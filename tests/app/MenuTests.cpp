...
- 27:08:2026 - 11:44:42: СЛУЧАЙ «ни один пункт не крупнее соседа». Роль
  «пункт-акцент» снята владельцем 27.08 («слово ПРОДОЛЖИТЬ бОльшим шрифтом
  написано, чем остальные»), и предыдущая раскладочная проверка её пропускала
  по построению: она утверждала НЕПЕРЕСЕЧЕНИЕ коробок, а коробка в 1.6 раза
...
- 27:08:2026 - 20:16:03: ДВА СЛУЧАЯ ЗВУКА (заказ владельца через музыкальную
  ...
  бы это ушами.
- 27:08:2026 - 20:28:48: третья ручка (речь) в случае про громкости, с тем же
  утверждением независимости от ДВУХ соседей сразу. Строка настоящая, хотя
  голосов ещё нет: ползунок, который «пока не считается», придётся чинить
  вместе с первой репликой.
- 27:08:2026 - 20:33:38: СЛУЧАЙ «страница настроек занимает страницу».
  Заведён по кадру, показавшему сорвавшуюся подгонку кегля, и заведён потому,
  что соседний раскладочный случай не мог этого увидеть ДВАЖДЫ: он утверждает
  НЕПЕРЕСЕЧЕНИЕ и нахождение в кадре — чему съёжившийся до нечитаемых букв
  список удовлетворяет с запасом (правило 41: прибор мерил не ту величину), —
  и он вообще не смотрел на страницу настроек, только на Root и Pause. Новый
  случай утверждает ИСХОД: список доходит до середины кадра. Он же нашёл
...
TEST_CASE("две громкости — две ручки, и ни одна не трогает чужую") {
    ...
    // ТРЕТЬЯ РУЧКА — РЕЧЬ. Голосов в движке ещё нет, и строка всё равно обязана
    ...
    for (int i = 0; i < VOLUME_RUNGS + 3; ++i) {
        m.adjust(+1);
        CHECK(legal_volume(m.settings().voice_volume));
        CHECK(m.settings().music_volume == doctest::Approx(music_kept));
        CHECK(m.settings().sfx_volume == doctest::Approx(sfx_kept));
    }
...
TEST_CASE("ни один пункт меню не крупнее соседа -- ни на одной странице") {
    // ЗАКАЗ ВЛАДЕЛЬЦА 27.08, дословно: «в меню текст разных кнопок разным
    // размером; слово ПРОДОЛЖИТЬ бОльшим шрифтом написано, чем остальные —
    // поправить и сделать одинаково везде». До этого первый пункт корня и
    // паузы рисовался ролью «пункт-акцент» (0.083 высоты кадра -> ступень 96
    // px против 60 px у прочих, ровно 1.6 раза), и ни один случай этого не
    // видел: раскладка проверялась на НЕПЕРЕСЕЧЕНИЕ коробок, а коробка вдвое
    // выше соседей не пересекается ни с кем.
    //
    // Мерится ВЫСОТА КОРОБКИ, а не кегль: коробка — то, по чему ходит мышь и
    // по чему рисуется строка, то есть единственная величина, в которой
    // «одинаково» имеет наблюдаемый смысл.
    MenuModel m;
    m.set_settings(MenuSettings{});
    for (const auto [w, h] : {std::pair{1920, 1080}, std::pair{640, 360},
                              std::pair{320, 180}}) {
        CAPTURE(w);
        CAPTURE(h);
        for (const MenuPage page : {MenuPage::Root, MenuPage::Pause}) {
            ...
            for (size_t i = 1; i < boxes.size(); ++i) {
                CAPTURE(i);
                CHECK(boxes[i].h == boxes[0].h);
            }
        }
    }

    // И КОНТРОЛЬ, без которого проверка выше проходит у ЛЮБОЙ раскладки,
    // рисующей все строки одним кеглем — даже неправильным: высота коробки
    // обязана быть высотой прописной РОЛИ «пункт», а не какого-то своего
    // числа. Именно эта половина ловит возврат акцента под другим именем.
    m.open(MenuPage::Root);
    const auto boxes = dfn::app::menu_row_boxes(1920, 1080, m);
    REQUIRE(!boxes.empty());
    const int item_cap = dfn::app::ui_cap_height(dfn::app::ui_px(1080, dfn::app::UiText::Item));
    REQUIRE(item_cap > 0); // шрифта нет -- случай не про размеры, а про актив
    CHECK(boxes[0].h == item_cap);
}
