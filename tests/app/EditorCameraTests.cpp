...
- 18:08:2026 - 11:50:28: правило «повторный запрос захвата не сбрасывает отсчёт смещения» —
  здесь, потому что оконный стенд может не измерить, а пропускающий рукав не
  охраняет ничего. Контрфакт: вернул безусловный сброс — 2 утверждения
  покраснели.
*/
...
// ЗАПРОС ЗАХВАТА, ПОВТОРЁННЫЙ С ТЕМ ЖЕ ЗНАЧЕНИЕМ, НЕ СОБЫТИЕ. Правило живёт
// выражением в IInput.h и проверяется ЗДЕСЬ, без окна, потому что сквозной
// стенд с настоящим окном (tests/platform/CursorCaptureTests.cpp) может не
// измерить: при захваченном курсоре система не отдаёт положение окну без
// фокуса, и на машине без переднего сеанса он пропускает себя вслух.
// Пропускающий рукав не охраняет ничего — поэтому правило продублировано сюда
// не копией кода, а ВЫЗОВОМ того же выражения, которое зовёт бэкенд.
TEST_CASE("повторный запрос захвата не сбрасывает отсчёт смещения") {
    using dfn::platform::capture_request_resets_delta;

    // СМЕНА режима — сброс нужен: захват телепортирует курсор, и первая
    // разность после телепорта мусор.
    CHECK(capture_request_resets_delta(false, true));
    CHECK(capture_request_resets_delta(true, false));

    // ТО ЖЕ ЗНАЧЕНИЕ — не событие. Ровно это и было сломано: App просит захват
    // КАЖДЫМ кадром, пока человек в редакторе, а безусловный сброс превращал
    // смещение мыши в ноль всегда, и камера не поворачивалась вовсе.
    CHECK_FALSE(capture_request_resets_delta(true, true));
    CHECK_FALSE(capture_request_resets_delta(false, false));
}
