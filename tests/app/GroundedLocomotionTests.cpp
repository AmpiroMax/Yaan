...
TEST_CASE("spinning_the_view_does_not_fly_the_body_across_the_map") {
    ...
    // ДО ДОРОЖКИ КОРНЯ клип поворота резался порогом на 10° от цели и тело
    // кончало у нуля; с дорожкой (§16, фаза 1) клип доигрывается (77° за
    // клип), и при цели ±120°, меняющейся каждые полсекунды, тело честно
    // догоняет её — три клипа в одну сторону. Порог — один оборот; прибор
    // непрерывности рыска (фаза 2, §16) заменяет эту проверку.
    CHECK(std::abs(body_yaw) < glm::radians(360.0f));
}
