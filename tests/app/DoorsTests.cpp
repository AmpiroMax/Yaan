...
- 27:08:2026 - 12:58:00: MenuEmblem.cpp в списке файлов, читающих двери
  (DFN_MENU_OAK_PHASE живёт там), и безлюдных дверей стало 20 —
  DFN_MENU_COST закрывает приложение, набрав свои кадры. Соседние двери
  герба в счёт НЕ вошли и не должны: обе меняют картинку, а не прогон.
  Число ПЕРЕСЧИТАНО по таблице.
- 27:08:2026 - 21:34:00: MenuArt.cpp в списке файлов, читающих двери —
  DFN_MENU_SPARK_PHASE живёт там (поле искр главного экрана). СЧЁТ БЕЗЛЮДНЫХ
  ДВЕРЕЙ ЭТА ЗАПИСЬ НЕ МЕНЯЕТ, и это утверждение, а не недосмотр: новая дверь
  меняет КАРТИНКУ, а не прогон, — на неё смотрят глазами ровно так же, как на
  соседнюю DFN_MENU_OAK_PHASE. Число в рукаве в этот же вечер выросло до 21,
  но по другой причине и чужой рукой (DFN_DOOR_AIM_PROBE — см. запись ниже
  или выше, смотря чей коммит лёг первым).
- 28:08:2026 - 12:20:00: безлюдных дверей 23 — DFN_SEAT_PROBE и DFN_SEAT_TAKE
  (механика поз). Соседка DFN_SEAT в счёт не вошла: это доза.
- 27:08:2026 - 21:40:30: безлюдных дверей 21 — прибавилась DFN_DOOR_AIM_PROBE
  (прибор прицела дверей). Её соседка DFN_DOOR_AIM в счёт НЕ вошла и не должна:
  это ДОЗА, она называет поведение мира, и человек законно играет на обоих её
  значениях. Число ПЕРЕСЧИТАНО по таблице, а не прибавлено на глаз.
  [ЗАПИСЬ ВОССТАНОВЛЕНА ТРЕТЬЕЙ РУКОЙ. Две волны правили этот файл в один
  вечер на общем дереве, и соседняя копия, лежавшая в рабочем каталоге,
  оказалась старше коммита — её запись вытеснила бы эту. Обе строки на месте,
  порядок хронологический; правило 17 нарушается не удалением намеренным, а
  вот таким.]
*/
...
const std::vector<std::string>& app_sources() {
    static const std::vector<std::string> v{
        ...
        // красный, который этот рукав обязан не выдавать.
        "engine/app/sources/MenuEmblem.cpp",
        // MenuArt.cpp дописан 27.08 вечером (поле искр главного экрана):
        // DFN_MENU_SPARK_PHASE читается там — по той же причине, что строкой
        // выше. Рисование меню теперь читает двери из ДВУХ файлов, и список,
        // знающий только один, объявил бы вторую дверь мёртвой.
        "engine/app/sources/MenuArt.cpp",
    };
    return v;
}
...
TEST_CASE("whether a run is unattended is the table's column, door by door") {
    ...
    // 20 с 27.08: DFN_MENU_COST — замер цены кадра главного меню. Она
    // ЗАКРЫВАЕТ приложение, набрав свои кадры, то есть за таким прогоном по
    // определению никто не сидит. Соседние двери герба (DFN_MENU_OAK и
    // DFN_MENU_OAK_PHASE) в счёт НЕ входят и не должны: обе меняют картинку,
    // а не прогон, и человек законно смотрит на них глазами.
    // 21 с 28.08: DFN_DOOR_AIM_PROBE — прибор прицела дверей. Он обходит все
    // створки карты четырьмя руками и печатает счёт; за таким прогоном никто
    // не сидит. Её соседка DFN_DOOR_AIM в счёт НЕ входит и не должна: это
    // ДОЗА, она называет поведение мира, и человек законно играет с нею на
    // обоих значениях.
    // 23 с 28.08: DFN_SEAT_PROBE (прибор прицела мебели, печатает счёт и
    // закрывается) и DFN_SEAT_TAKE (беспилотная рука на клавише E перед
    // лавкой — нажать её иначе умеет только человек). Их соседка DFN_SEAT в
