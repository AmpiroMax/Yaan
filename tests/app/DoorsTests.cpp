...
const std::vector<std::string>& app_sources() {
    static const std::vector<std::string> v{
        ...
        // AppPoses.cpp дописан волной «позы и переходы»: DFN_POSE и
        // DFN_POSE_TAPE читаются там — по той же причине, что и строкой выше.
        // Список поймал этот файл сам, ровно как когда-то поймал переезд
        // enter_world: обе двери мгновенно стали «описанными, но никем не
        // читаемыми», и рукав покраснел раньше, чем кто-нибудь их применил.
        "engine/app/sources/AppPoses.cpp",
        // AppViewer.cpp дописан волной «предпросмотр 3D-моделей» (01.09):
        // DFN_VIEWER_ITEM читается там — по той же причине, что и строкой выше.
        // Список поймал файл сам: пока его тут не было, дверь была «описанной,
        // но никем не читаемой», хотя её читают.
        "engine/app/sources/AppViewer.cpp",
        // AppCharGen.cpp дописан волной «редактор персонажа, шаг 2» (01.09):
        // DFN_CHARGEN_VIEW, DFN_CHARGEN_DONE и DFN_MORPH читаются там — по той
        // же причине, что и строкой выше. И рукав снова поймал файл сам, ещё
        // до первого прогона экрана: две новые двери мгновенно стали
        // «описанными, но никем не читаемыми».
        "engine/app/sources/AppCharGen.cpp",
    };
    return v;
}
...
TEST_CASE("whether a run is unattended is the table's column, door by door") {
    ...
    // законны).
    // 27 с 01.09: DFN_POSE (поставить фигуру в названную запись реестра поз) и
    // DFN_POSE_TAPE (перебрать весь реестр по своим часам). Обе — беспилотные
    // по тому же признаку, что и соседки выше: за прогоном, который снимает
    // двадцать три кадра подряд или ведёт ленту в полторы минуты, человек не
    // сидит — он бы просто нажал скобку. Клавиша у поз есть, и это как раз
    // отличает их от ДОЗЫ: доза называет поведение мира и законна на обоих
    // значениях, а эти две подменяют руку.
    CHECK(unattended_doors == 27);
