...
TEST_CASE("качание ограничено амплитудой и никогда не делает оборота") {
    float max_yaw = 0.0f;
    float max_tilt = 0.0f;
    // Проходим НЕСКОЛЬКО общих периодов обеих осей: 11 и 17 с несоизмеримы по
    // фазе, и короткая выборка застала бы только одну их комбинацию.
    ...
    // прошло бы оба утверждения выше. Пол — 6°, а не сегодняшние 13: пол
    ...
    CHECK(max_yaw > glm::radians(6.0f));
