...
TEST_CASE("дверной проём СКВОЗНОЙ: в полосе двери нет ни одного тела") {
    // 20.08: прорезь была отложена «до триангуляции с дырами», но пролёт-
    // прямоугольник режется на простенки и перемычку без неё. Без прорези в
    // дом нельзя войти — проходимость построек стоит на этом тесте.
    const auto occupied = [](bool with_door) {
        HouseGraph g;
        const VertexId a = g.add_vertex(Anchoring::OnGround, {0.0f, 0.0f, 0.0f});
        const VertexId b = g.add_vertex(Anchoring::OnGround, {4.0f, 0.0f, 0.0f});
        ElementId wall = 0;
        REQUIRE(g.add_element(ElementKind::Surface, {a, b},
                              with_door ? "frame;height=2.5;thickness=0.2;doors=1"
        ...
        // по углам прошёл бы и там, где прохода нет.
        int overlapping = 0;
        for (const auto& c : m.convex) {
            glm::vec3 lo{1e9f};
            glm::vec3 hi{-1e9f};
            ...
            const bool hit = lo.x < 2.35f && hi.x > 1.65f && lo.y < 1.8f
                          && hi.y > 0.2f && lo.z < 0.3f && hi.z > -0.3f;
            if (hit) {
                ++overlapping;
            }
        }
...
TEST_CASE("кровля рядами: скат собирается из дранок, а не красится") {
    ...
    // Износ кровли — ТОЛЬКО дрожь (правка после приёмки кадров 20.08:
    // выпавший кусок светил настилом и читался багом): кусков столько же.
    const int worn = count_pieces("frame;thickness=0.15;fill=7;wear=1");
    CHECK(worn == shingles);
    MESSAGE("дранок " << shingles << " (изношенных столько же)");
}
