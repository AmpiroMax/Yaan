...
TEST_CASE("§6.3 darkness is a FUNCTION OF THE PLACE, not of the last bit of a "
    ...
    int lit_under_open_sky = 0; // dark where the corridor's roof is in daylight
    float deepest_dark = 0.0f;
    for (int i = 0; i + 1 < tun.point_count; ++i) {
        const glm::vec3 a = tun.points[i];
        const glm::vec3 b = tun.points[i + 1];
        ...
        for (int s = 0; s < count; ++s) {
            ...
            prev = d_floor;
            deepest_dark = std::max(deepest_dark, d_floor);
            // THE USER'S OWN RULE, and the defect it was written for: «темнеет
            // снаружи, когда ещё крыши нет никакой». A corridor cut into a
            // hillside is an open TRENCH before it is a tunnel — its floor is
            // under the hill's surface while its CEILING is still in daylight —
            // and there the sky is directly overhead. Measured before the fix:
            // 61.3 % of such ticks on a live walk carried darkness, the worst
            // FULL BLACK with the ceiling standing 1.31 m above the hillside.
            const float surface = world::terrain_height(ctx, {floor_pt.x, floor_pt.z});
            const bool roof_in_daylight = floor_pt.y + tun.height >= surface;
            if (roof_in_daylight && d_floor > 0.0f) {
                ++lit_under_open_sky;
            }
        }
        arc += len;
    ...
    CHECK(lit_under_open_sky == 0);
    // ...and the tunnel does still go pitch black somewhere, or the three
    // checks above would all pass on a build that simply never darkens.
    CHECK(deepest_dark == doctest::Approx(1.0f));
}
