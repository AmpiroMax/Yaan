...
Dependencies:
- Uses: IPhysics.h, SkinnedCharacter.h (хитбоксы стоп, замки, якорь, скольжение),
...
- ДОРОЖКА КОРНЯ (LOCOMOTION_GROUNDED.md §16.6, фаза 5): стопы — ДАТЧИКИ.
  Постановка/отрыв — из расписания контактов клипа (LocomotionOut::planted),
  якорей нет; ход поставленного тела за тик — скольжение — уходит в корень.
  Ветка с замком остаётся прежнему пути (DFN_ROOT_TRACK=0) до сноса.
*/
#pragma once
...
private:
    ...
    /// Якорь замка относительно начала тела на постановке — чтобы ход тела
    /// переносить на якорь, а не подменять якорь центром коробки.
    std::array<glm::vec3, 2> anchor_offset_{};
    /// Якорь, который назвали МЫ на прошлом тике: если замок сдвинул его сам
    /// (перекат пятка → носок, перецеп), это не скольжение — пересинхронизация.
    std::array<glm::vec3, 2> anchor_seen_{};
