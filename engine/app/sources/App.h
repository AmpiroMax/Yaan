...
private:
    ...
    /// DFN_REST_POSE=legacy — прежняя (коробочная) рест-поза для тела игрока
    /// и тела экрана создания; рука «до» сравнения из того же бинарника.
    [[nodiscard]] static bool rest_pose_legacy_door();
    ...
    /// НАРОД ВЫБРАН: пересобрать список типажей, сузить нарисованные дорожки
    /// его полосой, поставить лорную строку. Тела НЕ трогает — выбор народа
    /// сам по себе не лепит, он показывает, где свои.
    void chargen_apply_people();
    /// ТИПАЖ ПРИМЕНЁН: все ручки встают в его центры (CHARGEN_UI.md, Р7 —
    /// «выбор типажа заполняет все ручки разом; дальше игрок правит»).
    void chargen_apply_archetype();
    /// «СЛУЧАЙНО»: усечённая нормаль в пределах народа, со скрытой связью
    /// «крупность»; лицо — усечённая нормаль вокруг нейтрали в полосе судьи
    /// (народных краёв у лица пока нет). БРОСАЕТ ТЕКУЩУЮ КАТЕГОРИЮ (Р5: игрок
    /// кидает нос, оставив телосложение); `all` — по всем категориям (Shift,
    /// доза DFN_CHARGEN_ROLL). Без народов тело не бросается.
    void chargen_random(bool all);
    /// «СЛУЧАЙНОЕ ИМЯ» по правилу народа.
    void chargen_roll_name();
    /// ПРИЗРАК «ДО»: снимок состояния на входе в категорию и возврат к нему,
    /// пока держат Пробел.
    void chargen_remember();
    void chargen_show_compare(bool on);
    /// Выбранный народ или nullptr, если народов в дереве нет.
    [[nodiscard]] const People* chargen_people() const;
    ...
    /// ЗЕРНО «СЛУЧАЙНО». Своё, потому что приёмка сравнивает прогон с
    /// прогоном (правило 13); дверь DFN_CHARGEN_SEED прибивает его.
    PeopleRng chargen_rng_{};
    /// СОСТОЯНИЕ НА ВХОДЕ В КАТЕГОРИЮ — точка отсчёта «Сравнить». Отсчёт
    /// берётся на ВХОДЕ В КАТЕГОРИЮ, а не в экран: сравнивать «нос до и
    /// после» полезно, «весь персонаж до и после» — нет (CHARGEN_UI.md, Р6).
    CharGenPreset chargen_before_{};
    /// Показан ли сейчас призрак (Пробел зажат).
    bool chargen_comparing_ = false;
    /// ДОЗА DFN_CHARGEN_COMPARE=1 держит его вместо руки. Отдельное поле, а не
    /// «включить один раз при входе»: кадр рисуется НЕ в момент входа, и
    /// первый же проход по кадру, увидев отпущенный Пробел, отпустил бы
    /// призрак — что первый прогон и показал (0.00 % разницы).
    bool chargen_compare_forced_ = false;
    /// Какая категория была на прошлом кадре — по её смене берётся отсчёт.
    std::size_t chargen_seen_category_ = 0;
    /// ОПИСЬ ЛИЦЕВЫХ РУЧЕК (манифест + калибровка), прочитанная на входе.
    FacePlan chargen_face_{};
    /// КУДА ЕДЕТ ПРИБЛИЖЕНИЕ (кадр категории, Р4); < 0 — не едет. Колесо
    /// игрока перебивает: кадр категории — предложение, а не запрет.
    float chargen_zoom_target_ = -1.0f;
    /// ДОЗА DFN_CHARGEN_VIEW ПРИБИЛА КАДР: категория его не переезжает.
    /// Иначе рецепт «вкладка 1, приближение 1» снимал бы фигуру целиком —
    /// кадр категории, а не кадр дозы.
    bool chargen_view_pinned_ = false;
    ...
    std::vector<render::RenderSystem::SkinnedDraw> skinned_draws_;
    /// То же для экрана создания: части предмета экрана (ScreenProp::parts).
    std::vector<render::RenderSystem::SkinnedDraw> chargen_part_draws_;
