...
- 28:08:2026 - 00:30:22: interior_light_dose() — доза DFN_INTERIOR_LIGHT
  (крит владельца 28.08 «в домах слишком темно»). Статическая функция, а не
  поле: свет комнаты ставится в enter_interior, а доза обязана быть одной на
  весь прогон — переключатель внутри прибора дал бы двум кадрам одного
  прогона разойтись в том, что именно они проверяли (AppDoors.h, DoorRead).
- 28:08:2026 - 11:30:00: interior_window_dose() и interior_day_factor() — СВЕТ ИЗ
  ОКОН (продолжение волны выше по её же §7: контраст 7..24x против 106x у
  референса). Доля дня стала функцией зоны, а не выражением внутри
  enter_interior, ровно потому, что читателей стало двое: общий свет комнаты и
  сила свечения оконных вставок (правило 35).
...
- 28:08:2026 - 19:30:00: Затвор беспилотного кадра стал ОДИН на тур и на дверь дозы:
  world_settled_ (латч вердикта), last_player_y_ (сходимость осадки капсулы),
  stream_focus_ и chunks_pending_. Раньше затвор жил внутри `if (tour_.active())`,
  и дверь, которой снимают города, не ждала мир вовсе.
*/
...
private:
    ...
    /// Собирает точки позы по УЖЕ ЗАЛИТОЙ локации (нужна геометрия).
    void spawn_furniture_seats();
    ...
    /// Исполняет заявку: садит либо поднимает.
    void take_seat();
