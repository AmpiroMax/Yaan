...
/// РУЧКА, КОТОРУЮ НАРОД НЕ НАЗВАЛ, БЕРЁТ ПОЛОСУ СУДЬИ ЦЕЛИКОМ. Дополняет
/// `limits` недостающими ручками из `canon`; уже названные не трогает.
///
/// ПОЧЕМУ ЭТО НУЖНО, И ПОЧЕМУ ЭТО НЕ УДОБСТВО. Три из четырёх файлов писали
/// «вся судейская полоса: лор её не сужает» и ПОВТОРЯЛИ её числа рядом с этим
/// комментарием. Повторённое число — это копия, и она разошлась с оригиналом
/// на первой же перепечке целей MORF: судья сузил `belly` с 0.45 до 0.43, и
/// два народа из четырёх были громко отвергнуты — правильно отвергнуты, за
/// полосу ШИРЕ судейской, которую никто не собирался расширять. Молчание в
/// файле выражает намерение «не сужаю» ТОЧНО, а число выражает его на день
/// написания.
///
/// СУЖЕНИЕ ПРИ ЭТОМ ОСТАЁТСЯ ЯВНЫМ: народ, который что-то сузил, пишет числа
/// и отвечает за них перед peoples_validate().
