...
//
// ЧЕМ ОТВЕЧАЕТ SKYRIM (artifacts/reports/interior-light-28-08.html, §1). Интерьер
...
/// СИЛА СВЕЧЕНИЯ ОКОННОЙ ВСТАВКИ — ЭТО ПЕРЕСВЕТ, И ЧИСЛО ВЫВЕДЕНО ИЗ ПЛИТКИ.
///
/// ПЕРВЫЙ ЗАХОД ПОСТАВИЛ 3.6 И ПРОМАХНУЛСЯ ВЧЕТВЕРО — записано, потому что
/// ошибка поучительная. Я прочитал pane_texel() (0.55..0.90 по светлоте) и
/// принял это за альбедо листа. Но texel — МНОЖИТЕЛЬ узора со средним около
/// единицы, а цвет плитки задаёт СТРОКА НАБОРА: parts_tile_base(Pane, Mid) =
/// (0.15, 0.10, 0.055), светлота 0.107. Глухое окно нарочно тёмное — снаружи
/// оно обязано читаться дырой с тёплой глубиной, — и произведение выходило
/// 0.078..0.14, то есть при силе 3.6 лист светил 0.28..0.50 и до пересвета не
/// доходил вдвое. Замер кадра поймал это сразу: медиана вставки 46 -> 83
/// байта вместо ожидаемого клипа в 255.
///
/// ЧИСЛО ЗАНОВО, ИЗ ИЗМЕРЕННОЙ ПЛИТКИ. Самый тёмный тексель листа (угол,
/// без тёплого пятна) светит 0.078; чтобы КЛИПОВАЛСЯ ДАЖЕ ОН, нужно 1/0.078 =
/// 12.8. Взято 12.0: угол выходит 0.94 и белым не становится, середина
/// (0.14 x 12 = 1.68) уходит в пересвет с запасом, а переплёт (плитка гасит
/// его на 35 %) остаётся видимой серой перекладиной. Ровно этот расклад — на
/// кадрах владельца: белый прямоугольник с различимым переплётом, а не
/// однородная заливка (images_examples/houses_indoors/image copy 3.png).
///
/// НОЧЬЮ 0.55, И ЭТО НЕ «ТО ЖЕ, НО ТЕМНЕЕ». Ночью за стеклом нет неба, и лист
/// светит ОТРАЖЁННЫМ светом комнаты: 0.078 x 0.55 = 0.043 против 0.077 у
/// стены рядом (альбедо 0.45 на ночном ambient 0.17). Окно обязано стать
/// ТЕМНЕЕ стены — тёмный прямоугольник ночью и есть окно, а светящийся
/// изнутри лист сделал бы дом фонарём наизнанку.
constexpr float PANE_GLOW_DAY = 12.00f;
constexpr float PANE_GLOW_NIGHT = 0.55f;

/// ТОН ВСТАВКИ — ТОТ ЖЕ СПОР, ЧТО У ОБЩЕГО СВЕТА, И ПОТОМУ СТОИТ РЯДОМ. Днём
/// холоднее ambient'а комнаты (0.82 синего против 0.90 у общего света): небо
/// в проёме — самый холодный предмет кадра, и именно на его фоне очаг
/// читается тёплым. Ночью тёплый и приглушённый: отражённый огонь дома.
constexpr glm::vec3 PANE_TINT_DAY{0.82f, 0.90f, 1.00f};
constexpr glm::vec3 PANE_TINT_NIGHT{1.00f, 0.86f, 0.70f};
...
/// ДОЗА СВЕТА ИЗ ОКОН (DFN_INTERIOR_WINDOW), 0..1.
///
/// ЧТО ЭТА ДОЗА ДЕЛАЕТ, ЕСЛИ ОДНОЙ ФРАЗОЙ: заводит в комнате ВТОРОЙ источник,
/// которого у неё не было. Прошлая волна подняла общий свет и честно записала,
/// чего не дотянула: контраст p95/p05 у нас 7..24x против медианных 106x у
/// девятнадцати кадров референса владельца, и весь разрыв — ПЕРЕСВЕТ. У него
/// в кадре горит само пламя и светится оконная вставка; у нас ярчайшая точка
/// кадра — пол под очагом. Днём ярчайшее пятно комнаты обязано быть окном.
///
/// РАЗБОР ПРИЧИНЫ, ПО КОТОРОЙ ОКНА НЕ БЫЛО (обе половины настоящие):
///   1. ВСТАВКА ГЛУХАЯ И БЕЗ ПРИЗНАКА. Лист остекления кладётся ПОДЧАСТЬЮ
///      стенного элемента (HouseWalls.cpp, материал 8), а самосвечение до
///      сих пор приходило свойством glow целого ЭЛЕМЕНТА — у подчасти его
///      быть не могло. Лечится признаком части (MeshPart.pane).
///   2. ПЕЧКА ВИДИМОСТИ ОКОН НЕ ЗНАЛА. Шестнадцать её лучей идут в верхнюю
///      полусферу и упираются в собственную кровлю, поэтому у запечатанной
///      комнаты видимость сидит РОВНО на полу 0.30 и у подоконника, и в
///      дальнем углу. Лечится апертурами (HouseMesh.windows) и форм-фактором
///      прямо на них.
///
/// ПОЧЕМУ НЕ ТОЧЕЧНЫЙ СВЕТ У ОКНА, хотя это дешевле на первый взгляд. Бюджет
/// точечных — шестнадцать на кадр на весь город, и зал с шестью окнами съел
/// бы больше трети; при этом окно светит ПЛОСКОСТЬЮ, а точка даёт круглое
/// пятно с центром в стене — ровно тот порок ванильного Skyrim, за который
/// его ругают модостроители («light emitting from the center of two sources,
/// light produced without a source»). Запечённая видимость же ничего не
/// стоит в кадре и умножается на ambient, который УЖЕ разный днём и ночью, —
/// то есть свет из окна получает суточный ход бесплатно и по построению.
float App::interior_window_dose() {
    static const float dose = [] {
        const char* e = door_value("DFN_INTERIOR_WINDOW");
        if (e == nullptr || *e == '\0') {
            return 1.0f;
        }
        return std::clamp(std::strtof(e, nullptr), 0.0f, 1.0f);
...
/// ДОЛЯ ДНЯ ДЛЯ СВЕТА ЛОКАЦИИ — ОДИН ХОЗЯИН НА ДВУХ ЧИТАТЕЛЕЙ (правило 35).
///
/// ЧАС СУТОК СЧИТАЕТСЯ ИЗ ЧАСОВ, А НЕ ЧИТАЕТСЯ ИЗ ОКРУЖЕНИЯ, И ЭТО ЗАМЕР, А
/// НЕ ПРЕДПОЧТЕНИЕ. Первая версия волны света брала env.sun_direction.y — и
/// получала 0.67 (полдень) при DFN_TIME_OF_DAY=0.0 (полночь). Причина названа
/// в самой петле кадра: apply_sky_time НЕ ЗОВЁТСЯ, пока мир подвешен (у
/// локации нет неба), а вход в локацию по двери DFN_INTERIOR случается ДО
/// ПЕРВОГО КАДРА — то есть окружение держит ещё умолчание структуры, и ночная
/// рука была бы неотличима от дневной. Здесь считается ровно то же, что
/// считает петля: доля суток из game_seconds_ и ТА ЖЕ функция
/// render::sun_direction_at.
///
/// Полоса 0..0.20 по высоте: солнце в 11.5° над горизонтом — уже день для
/// окна, ниже — сумерки.
float App::interior_day_factor() const {
    ...
    return std::clamp(sun_up / 0.20f, 0.0f, 1.0f);
}

/// СВЕЧЕНИЕ ОКОННОЙ ВСТАВКИ НА ТЕКУЩИЙ ЧАС. Доза 0 даёт силу 0, и это не
/// «почти прежний кадр»: при нулевой силе зона AppHouse не помечает вставку
/// самосветной вовсе, то есть лист остаётся в прежнем освещённом потоке
/// бит-в-бит (правило 47).
App::PaneGlow App::interior_pane_glow() const {
    const float dose = interior_window_dose();
    if (dose <= 0.0f) {
        return {};
    }
    const float day = interior_day_factor();
    PaneGlow g;
    g.tint = PANE_TINT_NIGHT + (PANE_TINT_DAY - PANE_TINT_NIGHT) * day;
    g.strength = (PANE_GLOW_NIGHT + (PANE_GLOW_DAY - PANE_GLOW_NIGHT) * day) * dose;
    return g;
}
...
bool App::enter_interior(const std::string& scene_path,
    ...
    const float amb = doc.air.set && doc.air.ambient >= 0.0f ? doc.air.ambient
                                                             : 0.10f;
    // ЧАС СУТОК — У ЗОНЫ, А НЕ ЗДЕСЬ (правило 35). Разбор, почему его нельзя
    // спрашивать у окружения подвешенного мира, переехал в шапку
    // interior_day_factor() вместе с самим счётом: читателей стало двое —
    // общий свет комнаты (ниже) и сила свечения оконных вставок (AppHouse), —
    // а окно, горящее полднем в комнате с ночным ambient, это не полутон, а
    // брак. Выражение перенесено буква в букву, кадр прежний.
    const float day = interior_day_factor();
    ...
    if (ilight > 0.0f) {
        std::fprintf(stderr,
                     ...
                     static_cast<double>(interior_window_dose()), lamp_count);
    }
