...
- 28:08:2026 - 11:35:00: СВЕТ ИЗ ОКНА (доза DFN_INTERIOR_WINDOW, 0 — бит-в-бит).
  Волна выше честно записала, чего не дотянула: контраст p95/p05 7..24x против
  медианных 106x у девятнадцати кадров референса, и разрыв — ПЕРЕСВЕТ, а
  ярчайшее пятно дневной комнаты обязано быть окном. Здесь — числа свечения
  вставки (PANE_GLOW_DAY/NIGHT, PANE_TINT_DAY/NIGHT) рядом с тоном общего
  света, потому что
  спорят они об одном небе за стеной; interior_pane_glow() отдаёт их зоне
