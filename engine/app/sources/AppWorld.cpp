...
void App::unload_world() {
    // СМОТРОВАЯ УХОДИТ ВМЕСТЕ С КАРТОЙ, и это ровно та утечка, которой она
    // иначе была бы: меш модели живёт под СВОИМ ключом россыпи, а ключ этот не
    // координата чанка — ни ChunkUnloaded, ни выгрузка чанков его не задевают.
    // Здесь он снимается явно, поэтому уход со смотровой на любую карту не
    // оставляет за собой ни одного резидентного меша (то же место и та же
    // причина, что у gallery_bodies_ ниже).
    viewer_leave();
    ...
    // И ЗВУК КАРТЫ — ТУДА ЖЕ. Рощи и русла принадлежат карте; выгрузка карты
    // без этой строки оставила бы играть шелест деревьев, которых больше нет,
    // — ровно та жалоба владельца, с которой началось правило хозяина
    // («вышел в главное меню — продолжили играть шумы фоновые»). enter_world
    // тоже снимает их у себя в начале, и это не дубль: снимать обязан КАЖДЫЙ
    // конец жизни карты, а их два — выгрузка и смена.
    if (audio_) {
        ambience_.silence(*audio_);
    }
...
bool App::enter_world(uint32_t stand) {
    ...
    // ИМЯ КАРТЫ, А НЕ ПУТЬ К ЕЁ КОМПОЗИЦИИ. Заголовок читает человек, и
    // «assets/scenes/whiterun.scene» он читает как ошибку. Путь остаётся
    // подзаголовком: он нужен тому, кто разбирается, почему грузится долго.
    loading_.begin(loading_map_name_.empty() ? std::string("Загрузка карты")
                                             : loading_map_name_,
                   gallery_scene_.empty() ? std::string("мир") : gallery_scene_);
    // ДЕВЯТЬ ЭТАПОВ: снос, тропы, свет, мир, сборка построек, их потоки в
    // видеопамяти, их коллайдер, заливка, готово. Три из девяти отмечает
    // upload_house_mesh — там же, где живут 75% времени загрузки Вайтрана
    // (1736 из 2319 мс), и разметить их отсюда было бы нечем.
    loading_.set_expected(9);
    // СНИМОК ЭКРАНА ЗАГРУЗКИ КАРТЫ — ПОД СВОИМ ИМЕНЕМ. Тот же прогон ниже
    // может войти в дом (DFN_INTERIOR), и его экран рецепты И15 знают как
    // loading_000.png; один ствол на два экрана означал бы, что город молча
    // затирает комнату, ради которой рецепт и запускался.
    loading_shot_stem_ = "loading_world";
    loading_frame_at_ = std::chrono::steady_clock::now();
    // ПЕРВЫЙ КАДР — ДО СНОСА, А НЕ ПОСЛЕ. Между «игрок выбрал карту» и первой
    // отметкой лежит вся выгрузка предыдущего мира; показать экран после неё
    // значило бы оставить чёрное окно ровно там, где человек ждёт ответа на
    // своё нажатие.
    present_loading_frame(loading_shot_stem_);
    auto load_mark = [&](const char* what) { load_step(what); };
    ...
    if (stand == static_cast<uint32_t>(world::StandId::Gallery)) {
        ...
        if (composed) {
            ...
            for (const world::Placement& p : doc.placements) {
                ...
                // ЗАПЕЧЁННАЯ ПОСТРОЙКА В ОБЪЕКТЕ (.dfo, секция HOUS) РОССЫПИ
                // НЕ КАСАЕТСЯ. Её рисует и в коллайдер кладёт upload_house_mesh
                // — теми же плитками, тем же светом и той же партией, что и
                // построенный по месту дом. Положить её ЕЩЁ и сюда значило бы
                // нарисовать одну кровать дважды: две одинаковые поверхности в
                // одной точке воюют за глубину и мерцают.
                //
                // ПОСЧИТАНА ОНА ВСЁ РАВНО (++placed): счётчик отвечает на
                // вопрос «встало ли всё, что просили», а не «сколько попало в
                // россыпь». Пропуск без счёта уже стоил зоне флоры сессии
                // поисков восьми объектов, которые никуда не пропадали.
                if (!obj.house.empty()) {
                    ++placed;
                    continue;
                }
            ...
            // ДВЕРЬ DFN_STAND_SPAWN: НАЗВАННАЯ точка сцены вместо заглавной.
            // Заведена ради пункта 3 приёмки («обе стопы стоят на предмете»):
            // поставить фигуру на ящик ходьбой значило бы вести её через
            // полстенда по счислению пути, а промах в полметра — это фигура
            // рядом с ящиком. Точка живёт в СЦЕНЕ (Rule 5), дверь только
            // выбирает её по имени, поэтому обе руки сравнения выходят из
            // одного бинарника и отличаются ровно дверью (Rule 47).
            if (const char* want = door_value("DFN_STAND_SPAWN");
                want != nullptr && want[0] != '\0') {
                bool found = false;
                for (const world::SceneSpawn& S : doc.spawns) {
                    if (S.name == want) {
                        scene_spawn_ = S.position;
                        scene_spawn_yaw_ = S.yaw;
                        found = true;
                        std::fprintf(stderr,
                                     "[scene] DFN_STAND_SPAWN=%s: (%.3f, %.3f, "
                                     "%.3f) yaw %.2f\n",
                                     want, static_cast<double>(S.position.x),
                ...
                if (!found) {
                    // ГРОМКО, А НЕ ТИХИМ ОТКАТОМ НА ЗАГЛАВНЫЙ СПАВН: доза,
                    // молча не сработавшая, даёт две руки сравнения, между
                    // которыми нет разницы, и это читается как «починка не
                    // помогла».
                    std::fprintf(stderr,
                                 "[scene] DFN_STAND_SPAWN=%s — такой точки в "
                                 "сцене нет, спавн заглавный\n", want);
                }
    ...
    // ПОЗА ДОЗОЙ — СРАЗУ ЗА ПОЯВЛЕНИЕМ ТЕЛА (DFN_POSE, AppPoses.cpp). Здесь, а
    // не в цикле кадра: беспилотный прогон обязан снять кадр позы с ПЕРВОГО
    // кадра, иначе снимок приёмки зависит от секунды, на которой его взяли.
    apply_pose_dose();
    ...
    if (!body_boxes_
        ...
        //
        // СЛОИ: ВСЁ, НА ЧЁМ МОЖНО СТОЯТЬ, И НИЧЕГО СВОЕГО. Статика (рельеф,
        // дома, мебель по чертежу, марши), предметы взаимодействия (дверная
        // створка) и УТВАРЬ (`LAYER_LOOSE`) — табурет, мешок, корзина, сундук:
        // ровно те предметы, на которые игрок встаёт, и единственный класс,
        // которого щуп не видел (заказ владельца 31.08, пункт 3).
        //
        // ЧТО ЗДЕСЬ ИЗМЕРЕНО, А ЧТО ПОЧИНЕНО. Заказ называл причиной «щуп
        // рейкастит только рельеф» — это НЕ ПОДТВЕРДИЛОСЬ: на стенде луч
        // честно находит и крышку ящика (25.780 при настиле 25.500), и матрас
        // кровати, и стопы стоят на них с зазором 0.8 мм. Дыра была ровно одна
        // и другая: `LAYER_LOOSE`. Утварь ВЫХОДИТ из общего статичного
        // коллайдера, когда получает своё динамическое тело (AppProps,
        // DFN_PROPS=1), то есть предмет, на который игрок влез, для щупа
        // ИСЧЕЗАЛ ровно в тот день, когда его стало можно двигать.
        //
        // И НИЧЕГО СВОЕГО: ни `LAYER_CHARACTER` (капсула), ни `LAYER_HITBOX`
        // (коробки частей тела, едущие за позой). Луч из-под собственной ноги,
        // нашедший собственную капсулу, поднял бы тело на собственный рост, а
        // нашедший собственную стопу — поставил бы стопу на саму себя.
        skinned_character_.set_ground_probe([this](const glm::vec3& p) {
            ...
            const platform::RayHit hit = physics_->raycast(
                ...
                REACH_M,
                physics::LAYER_STATIC | physics::LAYER_INTERACTABLE
                    | physics::LAYER_LOOSE);
    ...
    spawn_house_portals();
    open_decorative_doors();

    load_mark("мир готов");
    loading_.finish();
    // ПОСЛЕДНИЙ КАДР — ЭТО ИТОГ, А НЕ ФОРМАЛЬНОСТЬ. finish() останавливает
    // часы, и без кадра после него человек никогда не видит ни полной полосы,
    // ни числа, которым он мог бы пожаловаться на долгую загрузку.
    //
    // ДЕРЖИТСЯ ОН СТОЛЬКО ЖЕ, СКОЛЬКО ЭКРАН ВХОДА В ДОМ (DFN_INTERIOR_FADE,
    // штатно 0.15 с), и по той же паре причин: человеку нужен кадр, чтобы
    // прочитать итог, а беспилотному прогону — несколько кадров, чтобы
    // снимок успел вылиться. Бэкенд снимает ПОСЛЕ текущего end_frame и
    // требует нескольких кадров на слив (это уже знал снимок меню): один
    // кадр после finish() давал файл, на котором уже стоял ГОРОД, а не экран
    // загрузки, — и приёмка «экран жил всю загрузку» доказывалась картинкой
    // мира. Ствол имени второй: первый снимок ловит ПЕРВЫЙ этап из девяти, и
    // по нему одному «экран жил» и «экран мигнул» неразличимы.
    loading_shot_stem_ = "loading_world_done";
    hold_loading_screen();
    if (load_log) {
    ...
    if (const char* inside = door_value("DFN_INTERIOR");
        ...
        // ...И ВХОДИТ ОНА ТОЙ ЖЕ ДВЕРЬЮ, ЧТО РУКА (27.08). Дверь называет
        // локацию, а обратный адрес — свойство СТВОРКИ, и без него прогон
        // выходил бы туда, где игрок стоял до входа: точка выхода — ровно то,
        // на что владелец пожаловался, и проверять её путём, который её не
        // проходит, значит проверять что-то другое. Створка ищется по цели
        // перехода; локация без своей двери на карте (стенд, пилот) входит
        // по-старому, и это сказано вслух.
        PortalReturn back;
        std::string spawn;
        for (const PortalLink& link : portals_) {
            if (link.house && link.to == inside) {
                back = PortalReturn{link.back_at, link.back_yaw, link.back_set};
                spawn = link.to_spawn;
                break;
            }
        }
        if (!back.set) {
            std::fprintf(stderr,
                         "[интерьер] DFN_INTERIOR=%s: створки с такой целью на "
                         "карте НЕТ — выход пойдёт по позе игрока\n",
                         inside);
        } else {
            std::fprintf(stderr,
                         "[интерьер] DFN_INTERIOR=%s: своя створка найдена, "
                         "обратный адрес (%.1f %.1f %.1f), рыск %.2f\n",
                         inside, static_cast<double>(back.at.x),
                         static_cast<double>(back.at.y),
                         static_cast<double>(back.at.z),
                         static_cast<double>(back.yaw));
        }
        if (!enter_interior(inside, spawn, back)) {
            std::fprintf(stderr, "[интерьер] DFN_INTERIOR=%s — вход не удался\n",
                         inside);
        }
    }
    loading_.hide();
    // ХОЛСТ ГАСИТСЯ ЯВНО. present_loading_frame поднял hud_visible_, чтобы
    // экран вообще попал на кадр; оставить его поднятым значило бы показать
    // первый игровой кадр сквозь картинку загрузки (та же строка стоит в
    // конце enter_interior и по той же причине).
