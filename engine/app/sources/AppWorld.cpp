...
Last updated: 28:08:2026 - 16:40:00
Module: engine/app
...
- 27:08:2026 - 14:30:00: ЭКРАН ЗАГРУЗКИ ПОКАЗЫВАЕТСЯ, А НЕ ТОЛЬКО ВЕДЁТСЯ (заказ
  владельца 27.08: «нужен экран загрузки, пока грузятся города в момент запуска
  игры»). Модель этапов жила здесь с 24.08 и НИ ОДНОГО КАДРА не предъявляла:
  окно висело чёрным 2319 мс — ровно та ошибка, что уже случилась с полосой
  запекания ассетов и с холстом входа в дом, третий раз на одном пути. Кадр
  показывается ДО сноса прежнего мира (между нажатием и первой отметкой лежит
  вся выгрузка), на каждой границе этапа и после finish(); этапов стало девять,
  и три из них отмечает upload_house_mesh — там, где живут 75% времени
  (1736 из 2319 мс), а разметить их отсюда было нечем. Полоса внутри сборки
  построек идёт по ЧИСЛУ ПОСТРОЕК, кадры не чаще 80 мс. Заголовок — имя карты
  (loading_map_name_ ставит open_map ДО постройки: current_map_ присваивается
  после удачи и на первой загрузке назвала бы предыдущую карту).
- 27:08:2026 - 10:18:14: ОБЪЕКТ С ЗАПЕЧЁННОЙ ПОСТРОЙКОЙ РОССЫПИ НЕ КАСАЕТСЯ
  (волна кроватей 27.08). Такой объект рисует upload_house_mesh теми же
  плитками и тем же светом, что дом; положить его ещё и в россыпь значило бы
  нарисовать одну кровать дважды — две одинаковые поверхности в одной точке
  воюют за глубину. Счётчик «встало» его всё равно считает: пропуск без счёта
  уже стоил зоне флоры сессии поисков восьми объектов, которые не пропадали.
...
- 28:08:2026 - 16:40:00: unload_world молчит вместе с картой (второй конец
  жизни карты; первый — смена в enter_world).
*/
