...
enum class Action : uint8_t {
    ...
    /// ОРУЖИЕ В РУКАХ И ОБРАТНО (заказ владельца 31.08, пункты 5-6).
    ///
    /// T, А НЕ R: R уже занята курсором, и занята в области Anywhere, то есть
    /// перекрывает ЛЮБУЮ вторую строку. Из трёх свободных (T, H, Y) T — та,
    /// на которой это действие стоит в играх, откуда рука его помнит.
    ///
    /// ОБЛАСТЬ — ТЕЛО. Обнажать оружие в редакторе нечем и не для чего, а
    /// строка EditorOnly отняла бы T у будущего инструмента.
    WeaponToggle,      // T
    ...
    /// ПЕРЕБОР ПОЗ РЕЕСТРА, [ и ] (заказ ночной волны, пункт 5: «выбор позы из
    /// списка (циклично, DFN_POSE или клавиши [ ])»).
    ///
    /// ОДНА СТРОКА НА ДВЕ КЛАВИШИ, И ЭТО РЕШИЛА ПРОВЕРКА, А НЕ ВКУС. Я завёл
    /// две — «вперёд» и «назад», у каждой своя клавиша, — и набор app_controls
    /// покраснел: при 320x180 список уже стоит в двух колонках без подписей и
    /// без подвала, то есть на последней из своих запасных раскладок, и
    /// двадцать девятая строка с неё съезжает. Экран управления — потолок
    /// таблицы, и потолок этот настоящий.
    ///
    /// «Назад» отличает НАЖАТАЯ СКОБКА, а не модификатор: alias-клавиша строки
    /// доходит до обработчика физически, и он спрашивает, какая из двух лежит
    /// под пальцем. Тот же приём, что у отмены с Shift, только условие взято у
    /// самой пары клавиш, которую назвал заказ.
    PoseCycle,         // ] следующая, [ предыдущая

    /// СМОТРОВАЯ: ПЕРЕБОР МОДЕЛЕЙ, → и ← (заказ владельца 01.09: «режим
    /// предпросмотра разных 3D-моделей в игре — что накачал и что генерируешь»).
    ///
    /// ОДНА СТРОКА НА ДВЕ КЛАВИШИ, тем же приёмом, что у перебора поз: экран
    /// управления — потолок этой таблицы, и вторая строка стоила бы дороже,
    /// чем стоит различие «вперёд/назад», которое человек и так читает по паре
    /// стрелок.
    ///
    /// SHIFT ПЕРЕПРЫГИВАЕТ ЦЕЛУЮ ПОЛКУ, и это не украшение: на полке набора
    /// 2411 деталей, и добраться от неё до вывесок одной стрелкой значит нажать
    /// её две тысячи раз. Модификатор разбирается в обработчике — тот же приём,
    /// которым отмена отличает повтор, и по той же причине: это не вторая
    /// привязка, а то же нажатие с условием.
    ViewerCycle,       // → следующая, ← предыдущая; Shift — соседняя полка

    /// СМОТРОВАЯ: ПОВЕРНУТЬ МОДЕЛЬ, E и Q. Шагом, а не удержанием, и это
    /// вынужденно честно: модель показывается ЗАПЕЧЁННОЙ в мировые координаты
    /// (та же дорога, что у россыпи), поэтому поворот — это перепечка меша, а
    /// не матрица. Шаг в 15° стоит одну перепечку на нажатие; удержание стоило
    /// бы перепечки каждый кадр, и на модели в четверть миллиона треугольников
    /// это видно.
    ViewerTurn,        // E по часовой, Q против

    /// СМОТРОВАЯ: ВЕРНУТЬ РАКУРС, R. Клавиша названа заказом, и она освободилась
    /// честно: строка курсора (Action::CursorToggle) объявляла себя Anywhere, а
    /// её тело трогает ТОЛЬКО ящик инструментов редактора и печатает «[editor]
    /// курсор». Она сужена до EditorOnly в тот же заход — это не размен ради
    /// новой клавиши, а исправление подписи, которая была неверной: экран
    /// управления обещал игроку действие, которого в игре не происходило.
    ViewerReset,       // R
    /// ПЬЯНЕТЬ (владелец 03.09): H — из трёх свободных букв (T, H, Y) T уже
    /// у оружия; тело, не редактор.
    DrunkToggle,       // H
    Count,
};
