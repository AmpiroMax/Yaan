...
- Собрано по второму аудиту (docs/audits/AUDIT_EDITOR_TOOLS.md, долг 1): 33 прямых
...
void App::load_scene_houses() {
    ...
    // ТОЧКИ ПОЗЫ КАРТЫ — ПОСЛЕДНИМИ, И ИМЕННО ЗДЕСЬ, по тому же доводу, по
    // которому строкой выше стоит spawn_loose_props: раньше нельзя (мерится
    // геометрия ТОЛЬКО ЧТО прочитанных чертежей), позже негде — вызывающих у
    // этой функции четверо (вход в мир и три правки сцены редактором), и
    // забывший про строку получил бы карту, на которой мебель стоит, а
    // «Сесть» не горит. Сбор начинается с уборки, поэтому повторная правка
    // сцены не удваивает точки, а переписывает их.
    //
    // ПУСТАЯ КАРТА ПРОХОДИТ ЗДЕСЬ ЖЕ И МОЛЧА: ни одной вещи — ни одной точки,
    // ни одной строки, ни одной ошибки.
