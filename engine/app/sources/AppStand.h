...
struct StandStep {
    ...
    /// НА ЭТОМ СТЕНДЕ САДИТЬСЯ НЕ ВО ЧТО ВООБЩЕ — точки позы собирает
    /// spawn_furniture_seats(), а зовётся она только из enter_interior, и на
    /// уличной карте их ноль (прогон стенда: ни строки «точек на локации»,
    /// посадок 0). Настоящий жребий прицела существует, но живёт в другом
