...
struct StandStep {
    ...
    /// удалась, то же нажатие УСАЖИВАЕТ. Два прогона одной очереди давали
    /// разные позы на 35-й секунде, то есть приёмочные кадры волны зависели от
    /// жребия. Заявка «стоим» идемпотентна и жребия не имеет.
    ///
    /// А ВОТ ПРИЧИНА, КОТОРАЯ ЗДЕСЬ СТОЯЛА, БЫЛА НЕВЕРНОЙ, и стоит сказать это
    /// вслух, потому что она год выглядела объяснением. Тут было написано
    /// «прицел сиденья зависит от тика». Замерено волной подхода к мебели:
    /// НА ЭТОМ СТЕНДЕ САДИТЬСЯ БЫЛО НЕ ВО ЧТО ВООБЩЕ — точки позы собирает
    /// spawn_furniture_seats(), а звалась она только из enter_interior, и на
    /// уличной карте их было ноль (прогон стенда: ни строки «точек на
    /// локации», посадок 0). С волной «посадка под открытым небом» вторая
    /// ветка сбора есть, и точки на уличной карте заводятся; заявка «стоим»
    /// остаётся идемпотентной и по-прежнему нужна. Настоящий жребий прицела
    /// существует, но живёт в другом
    /// месте и в другом виде: вердикт выносится по тику ОТПУСКАНИЯ клавиши, а
    /// не нажатия. Заявка ниже закрывала СЛЕДСТВИЕ и закрывала его правильно;
    /// объяснение было выдумкой, и выдумка мешала искать.
    bool stand = false;
};
