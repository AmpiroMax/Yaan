...
enum class SettingsRow : size_t {
    ...
    MusicVolume,
    SfxVolume,
    // РЕЧЬ — ШИНА ВПЕРЁД ГОЛОСОВ (заказ владельца, дополнение того же дня).
    // Строка рисуется, хотя произносить ещё некому, и это ТА ЖЕ доктрина, что
    // у «Загрузить» на корневом экране (26.08): спрятанный пункт неотличим от
    // несуществующего, а игрок, который однажды услышит речь, будет искать её
    // громкость там, где она стоит сегодня. Ползунок настоящий — он крутит
    // настоящую шину и переживает перезапуск; просто на этой шине пока тихо.
    VoiceVolume,
