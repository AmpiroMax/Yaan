...
- 27:08:2026 - 20:28:48: ТРЕТИЙ ПОЛЗУНОК — РЕЧЬ (дополнение заказа того же
  дня: «три ползунка — музыка, эффекты, РЕЧЬ»). Голосов в движке ещё нет, и
  строка всё равно рисуется — та же доктрина, что у «Загрузить» на корневом
  экране: спрятанный пункт неотличим от несуществующего. Ползунок при этом
  НАСТОЯЩИЙ: он крутит настоящую шину и переживает перезапуск, так что первая
  же реплика попадёт в уже настроенную громкость, а не в единицу.
- 27:08:2026 - 21:36:43: НАСТРОЙКИ РАЗЛОЖЕНЫ ПО ГРУППАМ (заказ владельца
  28.08: «в настройках всё надо по группам разложить: видео, аудио, управление —
  как в любой нормальной игре по логике собери»).
  1. ПОДСТРАНИЦЫ, А НЕ ЗАГОЛОВКИ ВНУТРИ ОДНОГО СПИСКА, и довод измеримый, а не
     вкусовой: страница УЖЕ не помещалась. На сетке 320×180 двенадцать строк
     жались междустрочьем до высоты прописной (см. запись о сорвавшейся
     подгонке в Menu.cpp), а три подписи групп добавили бы ещё три строки к
     тому, что и так на пределе. Разложенная по группам, самая длинная
     страница — восемь строк, и на всех четырёх сетках она снова дышит.
  2. MenuPage::SettingsVideo и SettingsAudio; MenuPage::Settings стала
     ОГЛАВЛЕНИЕМ и рисуется тем же списком, что корень и пауза, — это и есть
     список пунктов, а вторая раскладка под ту же мышь разъехалась бы с первой.
  3. ТРИ ПЕРЕЧИСЛЕНИЯ СТРОК ВМЕСТО ОДНОГО (SettingsRow — группы, VideoRow,
     AudioRow). Одно общее на три страницы было бы ровно тем счётом нажатий,
     от которого эти имена и заведены, и оно молча уехало бы при первой же
     четвёртой группе.
  4. НАРУЖУ ВЕДЁТ РОВНО ОДНА ДВЕРЬ. Подстраницы возвращают в оглавление и
     НИЧЕГО не сохраняют: SettingsDone — это один ответ на «игрок закончил
     настраивать», и раздав его трём страницам, мы писали бы файл трижды за
     один поход в настройки. Калибровка при этом возвращается в ВИДЕО, откуда
     её открыли, а не в оглавление.
*/
...
enum class MenuPage : uint8_t {
    ...
    // ГРУППЫ НАСТРОЕК (заказ владельца 28.08: «в настройках всё надо по группам
    // разложить: видео, аудио, управление — как в любой нормальной игре»).
    // Settings стала ОГЛАВЛЕНИЕМ из трёх групп, а строки-значения переехали
    // сюда. Подстраницы, а не заголовки внутри одного списка, и довод
    // измеримый: страница уже не помещалась — на сетке 320x180 двенадцать строк
    // жались междустрочьем в высоту прописной, а три подписи групп добавили бы
    // к ним ещё три строки. Разложенная по группам, самая длинная страница
    // (видео) — восемь строк, и на всех сетках снова дышит.
    SettingsVideo = 10,
    SettingsAudio = 11,
};
...
// СТРОКИ СТРАНИЦ НАСТРОЕК, НАЗВАННЫЕ ТАМ ЖЕ, ГДЕ ИХ ИНДЕКСИРУЮТ. Ровно тот же
// урок, что записан над RootRow, и он повторялся дважды: страница росла, а
// рукав ходил к «яркости» счётом нажатий — и молча начинал жать «палитру»,
// сообщая об этом как «страница калибровки не открывается». Имя одно на код и
// на прибор: перестановка строк — ошибка компиляции, а не тихий зелёный.
//
// ТРИ ПЕРЕЧИСЛЕНИЯ, А НЕ ОДНО, потому что страниц стало три: оглавление групп и
// две страницы значений. Одно общее на всех было бы ровно тем счётом нажатий,
// от которого эти имена и заведены.
// ОГЛАВЛЕНИЕ НАСТРОЕК: три группы и выход. Порядок — от того, чем игрок
// пользуется чаще, к тому, что он открывает раз в жизни.
...
// ГРУППА «ВИДЕО». Порядок — это порядок картинки: сначала то, во что её
// показывают (окно, экран), потом в чём её считают (сетка, сглаживание,
// палитра), потом как она движется, и последним — насколько тёмной ей
// позволено стать.
enum class VideoRow : size_t {
...
// ГРУППА «АУДИО»: три шины, три строки — ровно потому, что это ТРИ разные
// жалобы: «музыка мешает слушать шаги», «в игре слишком громко» и «реплику не
// разобрать». Один общий ползунок не отвечает ни на одну из них.
//
// РЕЧЬ — ШИНА ВПЕРЁД ГОЛОСОВ. Строка рисуется, хотя произносить ещё некому, и
// это ТА ЖЕ доктрина, что у «Загрузить» на корневом экране (26.08): спрятанный
// пункт неотличим от несуществующего, а игрок, который однажды услышит речь,
// будет искать её громкость там, где она стоит сегодня. Ползунок настоящий — он
// крутит настоящую шину и переживает перезапуск; просто на этой шине пока тихо.
enum class AudioRow : size_t {
    MusicVolume = 0,
    SfxVolume,
    VoiceVolume,
    Back,
...
struct MenuSettings {
    ...
    // Речь. Единица, как у эффектов: реплика — это то, что игрок СЛУШАЕТ, и
    // приглушать её по умолчанию значило бы решать за него, что она фон.
