...
void App::on_undo_redo() {
    ...
    // Пока модель постройки не подключена к инструментам, отменять нечего, и
    // это ГОВОРИТСЯ вслух: молчащая отмена неотличима от сломанной, а сегодня
    // мы этот сорт молчания разбирали трижды.
