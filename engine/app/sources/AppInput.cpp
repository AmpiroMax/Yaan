...
void App::on_fullscreen() {
    ...
    if (window_->is_fullscreen() == want) {
        ...
        // И СТРАНИЦА НАСТРОЕК УЗНАЁТ ОБ ЭТОМ СРАЗУ (заказ владельца 27.08: «оно
        // в игре и меню должно синхронизироваться»). Полный экран — единственная
        // настройка с ДВУМЯ органами управления: клавиша F11 и строка на
        // странице. Без этой строки страница показывала бы «выкл» на полном
        // экране до следующего запуска, а следующее нажатие Enter на ней
        // ВЕРНУЛО БЫ окно — то есть строка не отставала бы, а врала и отменяла
        // то, что игрок только что сделал.
        menu_.set_live_fullscreen(want);
...
void App::on_toggle_body() {
    // КЛЮЧ ОТ РЕДАКТУРЫ ЛЕЖИТ В МЕНЮ, А НЕ НА КЛАВИШЕ (заказ владельца 27.08:
    // «в обычной игре могу войти в режим редактуры и строить, хотя должен это
    // уметь только в игровом режиме редактора»). Эта клавиша переключает ГЛАЗ —
    // тело или облёт, — и она осталась переключателем глаза; чем она быть
    // перестала, так это ВТОРЫМ входом в редактор. В игровой сессии её первая
    // половина (вернуться в тело) не нужна вовсе: тела никто не покидал.
    //
    // ОТКАЗ ГОВОРИТ ВСЛУХ. Клавиша, которая молча ничего не делает, неотличима
    // от сломанной, и следующий отчёт был бы «` перестала работать».
    if (!editor_session_) {
        std::fprintf(stderr,
                     "[режим] это игровая сессия: облёт и редактура недоступны. "
                     "Редактор открывается пунктом «Редактор» в главном меню\n");
        return;
    }
    if (mode_ == AppMode::Editor) {
...
void App::on_third_person() {
    ...
    // КАМЕРА ЗАВОДИТСЯ ЗА СПИНОЙ И СМОТРИТ ТУДА ЖЕ, КУДА СМОТРЕЛО ПЕРВОЕ ЛИЦО.
    // Азимут теперь мировой (App.h), поэтому «ноль» здесь означал бы «камера
    // на севере», то есть переключение вида швыряло бы кадр на пол-оборота.
...
void App::update_part_rotation() {
    ...
    if (house_.selected_vertex() != world::NO_VERTEX && nudge_selected_anchor()) {
        // РУКА ПЕРЕШЛА НА СТРЕЛКИ: призрак под прицелом гаснет до тех пор, пока
        // человек не тронет камеру (это решается в App::run, там же, где камера
        // и читает свой ввод).
        house_.set_nudging(true);
        return;
    }
    const IEditorTool* held = editor_ui_.toolbox().active();
    if (held == nullptr || !held->wants_part_rotation()) {
...
void App::on_quick_remark() {
    ...
    if (mode_ == AppMode::Editor) {
        // ENTER В РЕДАКТОРЕ — ЭТО ПОДТВЕРЖДЕНИЕ ВВОДА, И БОЛЬШЕ НИЧЕГО (заказ
        // 18.08: «убери скриншот или чё там на enter, пусть на enter ставится
        // якорь там, где я его держу, на ЛКМ и на Enter»).
        //
        // Быстрая заметка со снимком осталась, но только В ТЕЛЕ: там Enter
        // ничего не подтверждает, и клавиша свободна. В редакторе же человек
        // работает руками по многу часов, и клавиша, иногда делающая снимок
        // вместо постановки, — это не «две функции», а ловушка.
        if (IEditorTool* tool = editor_ui_.toolbox().active(); tool != nullptr) {
            if (tool->has_draft()) {
                tool->on_confirm(editor_ui_.tool_world());
                return;
            }
            // НЕТ ЧЕРНОВИКА — ЗНАЧИТ ENTER РАБОТАЕТ КАК ЛЕВАЯ КНОПКА: тем же
            // нажатием и отпусканием, по тому же прицелу. Не «похоже на клик», а
            // ровно он: второй путь постановки разошёлся бы с первым в тот день,
            // когда один из них поправят.
            const ToolAim aim = editor_aim();
            tool->on_press(aim, editor_ui_.tool_world());
            tool->on_release(editor_ui_.tool_world());
            return;
        }
        return;
    }
