    ...
    while (!window_->should_close()) {
        ...
        for (uint32_t i = 0; i < steps; ++i) {
            ...
            if (!tour_.active()) { // frozen player during the tour: deterministic frames
                ...
                // ЛЕНТА ПОЗ — СВОИМИ ЧАСАМИ И РЯДОМ С ОЧЕРЕДЬЮ СТЕНДА, но НЕ
                // внутри неё: очередь пишет НАМЕРЕНИЯ игрока (шаг, прыжок, E),
                // а лента объявляет позу. Слить их в одну очередь значило бы,
                // что кадр позы зависит от того, куда в ту секунду шли ноги.
                tick_pose_tape(static_cast<float>(timestep_.step_dt()));
                if (stand_seq_) {
                    if (auto* ps = world_.get<gameplay::PlayerState>(player_)) {
