...
- 27:08:2026 - 20:49:56: ЗВУК ЗАСТАВКИ (intro_sting.ogg, музыкальная сессия;
  финальный кусок заказа). Росчерк написан под КАДРЫ интро: гул 0.30, удар 1.65
  в момент полной яркости, хвост зала с 3.50.
  * СТАРТ — НА ПЕРВОМ КАДРЕ СТРАНИЦЫ ЗАСТАВКИ, а не в init(): между концом
    init и первым кадром лежит вся заливка окна, и удар на 1.65 пришёлся бы не
    на ту картинку. Тот же кадр рисует нулевой кадр видео — это и есть
    синхронность.
  * ЗВУК ДЛИННЕЕ ВИДЕО НА ДВА С ЛИШНИМ СЕКУНДЫ, И НЕ ОБРЕЗАЕТСЯ. 5.6 против
    3.5: хвост обязан звучать над уже открывшимся меню, где тема уже пошла —
    росчерк кончается пустой квинтой ре-ля, с которой тема начинается, и стоит
    на децибел тише. Поэтому естественный конец НЕ ЯВЛЯЕТСЯ событием: в нём
    делать нечего, и код там ничего не делает.
  * ПРОПУСК КЛАВИШЕЙ ГАСИТ ЗА 0.2 с; таймер не гасит. Это вся разница между
    двумя выходами из заставки, и она про обещание: досмотревший слышит хвост,
    нажавший «пропустить» просил тишины.
  * ШИНА — МУЗЫКАЛЬНАЯ. Хвост звучит ОДНОВРЕМЕННО с темой и сведён
    относительно неё; развести их по разным шинам значило бы дать игроку два
    органа управления одной музыкальной фразой.
  * menu_audio_allowed() — один ответ на тему и на росчерк вместо двух копий
    правила «в счётных прогонах тихо». Дверь DFN_MUSIC теперь гасит и его.
...
- 27:08:2026 - 21:40:30: filter_door_hover() СТРОГО МЕЖДУ update_hover и
  player_actions_step. Луч взаимодействия отвечает «в теле двери», а тело,
  начатое вокруг головы, отвечает так под любым углом: Jolt засчитывает луч из
  нутра выпуклого тела попаданием на нулевой доле, и подсказка «Выйти» горела с
  порога (крит владельца 28.08). Подсказка и клавиша E читают ОДИН HoverTarget,
  поэтому гасить его между ними — единственный способ не дать им разойтись.
- 27:08:2026 - 21:41:27: У КАЖДОГО ИЗЛУЧАТЕЛЯ ПОЯВИЛСЯ ХОЗЯИН (заказ
  владельца 28.08, дословно: «зашёл на Вайтран, поиграл, вышел в главное меню —
  продолжили играть шумы фоновые/ветер/вода. К чему привязаны эти шумы, если
  они сохранились после закрытия карты? Обязательно звук должен быть привязан к
  чему-то конкретному»).
  * ЭТО БЫЛ НЕ ЗАБЫТЫЙ ВЫЗОВ. Ветер заводился ОДИН РАЗ при старте приложения и
    жил до конца прогона: его хозяином было ПРИЛОЖЕНИЕ, а звучал он про МИР.
    Выход в меню мир не выгружает (так задумано — «Продолжить» обязано
    возвращать туда же), поэтому события, на котором ветру полагалось бы
    замолчать, не существовало вовсе. Строка «а ещё выключить ветер вот здесь»
    завела бы седьмое место, где кто-то забудет седьмой звук (правило 32).
  * ПРАВИЛО ВМЕСТО СЛУЧАЯ, и оно выражено ШИНОЙ: у каждого излучателя есть
    хозяин, звук замолкает вместе с ним, и выбрать шину — единственный способ
    завести звук, поэтому правило нельзя забыть, можно только нарушить вслух.
    Хозяев двое: МИР (world_bus_ под sfx_bus_) и МЕНЮ (music_bus_). Свод — в
    engine/platform/audio/docs/README.md, там же дерево шин.
  * ПАУЗА — ТОЖЕ ОСТАНОВЛЕННЫЙ МИР, и он тоже молчит: ветер над замершим
    кадром — это та же жалоба, только на две секунды раньше по времени игрока.
  * Пандус 0.35 с в обе стороны по своим стенным часам (audio_tick_prev_):
    часы меню на выходе в мир не идут, шага симуляции в меню нет — ни одни из
    двух существующих часов не тикают на ОБЕИХ сторонах перехода, ради
    которого пандус и заведён. Обрыв за кадр слышен как отвалившийся звук.
  * Две строки в лог на концах пандуса: «мир замолчал» / «мир снова звучит».
    «Почему в меню тихо» и «почему в меню шумит» — одна жалоба с двух сторон,
    и обе проверяются ушами, которых у прогона нет.
  * И ДВЕ СТРАНИЦЫ НАСТРОЕК СТАЛИ ДОСТИЖИМЫ ДВЕРЬЮ: DFN_MENU_PAGE принимает
    settings_video и settings_audio. Без них две страницы значений из трёх
    перестали бы быть доказательством в тот же час, когда появились.
- 27:08:2026 - 22:00:12: Шина мира начинает прогон там, где прогон стоит, а
...
- 28:08:2026 - 17:05:00: ЗВУК ОТ ИСТОЧНИКА, поправка первого живого замера:
  открытость двери в локации считается от ПОЛОТНА, а не от точки входа сцены.
  Прибор показал срез 420 Гц («вглубь дома») игроку, стоящему у самой двери, —
  точка входа не обязана лежать у порога, её ставит композиция. И боковой
  относ окклюзионных лучей перестал быть нормализацией вырожденного вектора:
  крона над головой давала почти вертикальный луч и NaN.
- 28:08:2026 - 18:50:00: НАСЕСТ СТРЕЛЫ ТРЕТЬЕГО ЛИЦА (второй хвост зоны поз, коммит ed8bf25):
  ветка third_person_ считает стрелу от точки НАД предметом, а не от глаза
  позы (у лежащего глаз в 0.25 м над матрасом, и щуп начинал ВНУТРИ
