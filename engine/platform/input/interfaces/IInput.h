...
/// СБРАСЫВАТЬ ЛИ «ПРЕДЫДУЩЕЕ ПОЛОЖЕНИЕ ИЗВЕСТНО» при запросе захвата.
/// Одно выражение, потому что именно оно было неверным и стоило пользователю
/// трёх заходов: захват телепортирует курсор, поэтому первая разность после
/// СМЕНЫ режима — мусор и её надо погасить; но App держит захват
/// УТВЕРЖДЕНИЕМ и просит его каждым кадром, а безусловный сброс превращал
/// смещение мыши в ноль НАВСЕГДА, и камера редактора не поворачивалась вовсе.
///
/// Живёт здесь, а не внутри бэкенда, по одной причине: сквозной стенд с
/// настоящим окном (tests/platform/CursorCaptureTests.cpp) МОЖЕТ НЕ ИЗМЕРИТЬ —
/// при GLFW_CURSOR_DISABLED система не отдаёт положение окну без фокуса, и на
/// машине без переднего сеанса он честно пропускает себя. Пропускающий рукав
/// не охраняет ничего, поэтому само правило вынесено туда, где его можно
/// проверить всегда и без окна.
[[nodiscard]] constexpr bool capture_request_resets_delta(bool was_captured,
                                                          bool now_captured) {
