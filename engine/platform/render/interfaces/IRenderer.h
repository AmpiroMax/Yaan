...
struct DrawParams {
    ...
    float aux1 = 0.0f;
    // --- ИЗ ЧЕГО СДЕЛАН ЭТОТ ДРО (зона МАТЕРИАЛЫ, волна 3, 28.08) -----------
    //
    // Альбедо этот контракт уже умеет передавать тремя способами (цвет
    // вершины, плитка в texture, лист рельефа в aux_texture). Чего он не
    // умеет вовсе — сказать, КАК поверхность отражает, и потому золото герба,
    // чугун петли и известковая штукатурка выходят из одного ламберта
    // неотличимыми по природе: у всех троих блика нет вообще.
    //
    // ЗДЕСЬ ЕДЕТ ИМЯ ВЕЩЕСТВА (его номер в реестре), А НЕ ЕГО ЧИСЛА, И ЭТО
    // ПОЧИНКА, А НЕ ПЕРЕИМЕНОВАНИЕ. Волна 0 положила сюда `roughness` и
    // `metalness` полями — то есть «вещество как свойство ВЫЗОВА». Пока
    // источник блика один (герб), разницы нет; как только их станет двадцать,
    // «шероховатость камня» будет записана в двадцати местах кода и разойдётся
    // в первой же правке, потому что у вызова нет владельца, которому задать
    // вопрос «а почему у гранита здесь 0.86, а через файл 0.9». Поля объявлены
    // ПЕРЕХОДНЫМИ с точкой смерти в этой волне (MATERIALS.md §2.7) и здесь
    // сняты: дро НАЗЫВАЕТ вещество, отражение бэкенд берёт из реестра сам.
    //
    // ПОЧЕМУ БЭКЕНДУ ЭТО МОЖНО. Реестр живёт в engine/core, а по слоям
    // (ARCHITECTURE.md, DAG) платформа смотрит в core — то самое, ради чего
    // реестр из engine/render и переезжал. Uniform u_matParams переезд пережил
    // без правки: он был нужен одинаково при обоих устройствах.
    //
    // 0 — «вещества не названо», и это ЛАМБЕРТ, то есть сегодняшний кадр
    // бит-в-бит. Не «безопасное умолчание», а контрольная рука (правило 47):
    // всякое утверждение про материалы измеряется против дро, который ничего
    // не сказал, и оба выходят из одного бинарника.
    //
    // ТИП НАМЕРЕННО ГОЛЫЙ uint16, А НЕ core::MaterialId. Контракт платформы
    // header-only и не тянет ничего (правило 1); значение приходит из
    // core::MaterialId, который есть тот же uint16, и бэкенд приводит его
    // обратно ровно в одном месте.
    uint16_t material = 0;
    ...
    // --- МАТЕРИАЛ ЧАСТИ ПЕРСОНАЖА (волна «части персонажа») ------------------
    //
    // ВЫРЕЗ ПО АЛЬФЕ ЛИСТА: тексели с альфой ниже порога не рисуются и не
    // отбрасывают тень (glTF alphaMode MASK / alphaCutoff). 0 — выреза нет,
    // и это контрольная рука: всякий дро, промолчавший, выходит из этого же
    // бинарника бит-в-бит прежним. Ненулевой порог на программе «skinned_cutout»
    // — прядь волос, ресница, бровь: карточка, у которой половина листа пуста.
    float alpha_cutoff = 0.0f;
    // ДВУСТОРОННИЙ СВЕТ (glTF doubleSided): нормаль разворачивается к глазу,
    // как у карточки листвы, — иначе изнанка пряди чёрная. Отсечение граней в
    // этом контракте и так выключено; поле говорит только про СВЕТ.
