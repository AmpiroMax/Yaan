...
struct DrawParams {
    ...
    float aux1 = 0.0f;
    // --- ОТРАЖАТЕЛЬНАЯ ПОЛОВИНА МАТЕРИАЛА (зона МАТЕРИАЛЫ, 28.08) ------------
    //
    ...
    //
    // ПОЧЕМУ СВОЙ uniform, А НЕ aux0/aux1. u_params — четыре слота, и все
    // четыре заняты (x лист, y растворение, z подсветка/эмиссия, w «лист
    // рельефа привязан»). w при этом УЖЕ перегружен: бэкенд кладёт туда aux0,
    // а потом затирает единицей, если аux-лист нашёлся, — то есть aux0
    // работает ровно до первой текстуры рельефа, а материалам она нужна
    // всегда. Второй vec4 стоит один setUniform на дро и снимает перегрузку,
    // вместо того чтобы добавить к ней третий смысл.
    //
    // 1.0 / 0.0 — ЛАМБЕРТ, то есть сегодняшний кадр бит-в-бит. Это не
    // «безопасное умолчание», а контрольная рука (правило 47): всякое
    // утверждение про материалы измеряется против дро, который ничего не
    // сказал, и оба выходят из одного бинарника.
    float roughness = 1.0f; // 1 = матовый ламберт, 0 = зеркало
    float metalness = 0.0f; // 0 = диэлектрик (белый блик), 1 = металл (блик цвета альбедо)
