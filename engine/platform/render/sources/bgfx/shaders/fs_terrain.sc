...
- 22:08:2026 - 23:48:50: тропа из фрагмента: маска мира (стадия 6, u_pathMask) заменяет вершинную альфу внутри композиции — кромка полотна с разрешением растеризации, а не сетки земли.
- 23:08:2026 - 02:45:45: анти-повтор мостовой — второй фетч клетки на несоизмеримом масштабе (как у травы), доза DFN_PATH_TILES_BOMB.
...
void main()
{
    ...
    if (u_pathMatDose > 0.5) {
        ...
        path_w = 1.0 - wi / 63.0;
        // ТРОПА ИЗ ФРАГМЕНТА (23.08, круг 5: «шевроны» — 1.8-метровые
        // полотна на 2-метровой решётке вершин рассыпались зигзагами).
        // Внутри прямоугольника композиции износ и класс берутся из
        // растеризованной маски мира (R — износ линейной фильтрацией, G —
        // класс чтением по центру текселя: интерполяция классов дала бы
        // мусорные клетки атласа на границах). За прямоугольником — прежняя
        // вершинная альфа: тропы долины живут на сетке, как жили.
        // u_pathMask.w = 0 — маски нет или доза DFN_PATH_FRAG = 0.
        if (u_pathMask.w > 0.5) {
            vec2 muv = (v_wpos.xz - u_pathMask.xy) * u_pathMask.z;
            if (muv.x > 0.0 && muv.y > 0.0 && muv.x < 1.0 && muv.y < 1.0) {
                path_w = texture2D(s_texPathMask, muv).x;
                vec2 cuv = (floor(muv * u_pathMask.w) + vec2(0.5, 0.5))
                         / u_pathMask.w;
                cls = floor(texture2D(s_texPathMask, cuv).y * 3.0 + 0.5);
            }
        }
        vec2 pcell = vec2(mod(cls, 2.0), floor(cls * 0.5));
        path_col = texture2D(s_texPath,
                             (pcell + fract(v_wpos.xz * u_pathTiles)) * 0.5).rgb;
        // АНТИ-ПОВТОР МОСТОВОЙ (волна 23.08): один штамп 9x9 повторялся
        // каждые 2.22 м, и на каждом изломе улицы ряды камней срезались об
        // мир — у травы разлом тайла есть (выше), у мостовой не было. Тот
        // же приём: второй фетч ТОЙ ЖЕ клетки на несоизмеримом масштабе,
        // выбор полем с периодом 11 м. Доза DFN_PATH_TILES_BOMB, 0 —
        // прежний одиночный штамп бит-в-бит.
        if (u_pathBombDose > 0.5) {
            vec3 path_col2 = texture2D(
                s_texPath,
                (pcell + fract(v_wpos.xz * u_pathTiles * 0.41
                               + vec2(0.19, 0.63))) * 0.5).rgb;
            float pbomb = dfn_gnoise(v_wpos.xz * (1.0 / 11.0) + vec2(37.0, 5.0));
            path_col = mix(path_col, path_col2,
                           smoothstep(0.30, 0.70, pbomb));
        }
