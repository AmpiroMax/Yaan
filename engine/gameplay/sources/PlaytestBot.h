...
struct PlaytestConfig {
    ...
    float arrive_m = 0.0f;
    /// МАСШТАБ ОБЗОРНОЙ РАЗВЁРТКИ ВНИЗ (GLANCE_MAX_RADIANS умножается на это).
    /// 1 — штатное поведение: бот качает взгляд вниз до ~26°, чтобы находить
    /// предметы ниже глаз. 0 — ровный взгляд для операторских прогонов: лента
    /// DFN_RECORD_EVERY с качанием полкадра смотрит в землю, и приёмка вида
    /// города по таким кадрам невозможна (претензия [9] критика 22.08 была
    /// ровно об этом — качание бота приняли за качание камеры игрока).
    float glance_scale = 1.0f;
};
