...
- 28:08:2026 - 18:20:00: у порога кроны записан ДОГОВОР С САЖАЮЩИМИ ЗОНАМИ и
  замер подроста ярусов — чтобы следующая волна флоры нашла ответ в коде, а не
  в переписке двух зон.
*/
...
///
/// ДОГОВОР С ЗОНАМИ, КОТОРЫЕ САЖАЮТ (28.08, волна ярусов флоры): отбор идёт
/// ПО ИЗМЕРЕННОЙ ВЫСОТЕ, а НЕ по полке объекта и не по его kind. Всё, что
/// выше этой отметки и несёт листву, зазвучит САМО, без правки кода здесь, —
/// поэтому предупреждать звук надо в ОБРАТНОМ случае: когда высокое растение
/// звучать не должно. Названный кандидат на такой разговор один — ВАЛЕЖНИК:
/// у лежащего ствола листва может оказаться при нулевой высоте, и порог его
/// отсечёт молча и правильно, но если валежник задумают стоячим, приходите.
///
/// Замер границы (обе боевые карты, 28.08): подрост ярусов ростом 1.86–2.88 м
/// проходит порог семью деревцами из одиннадцати в Вайтране и одним из шести
/// в Житнове — плюс три рощи. Слышны как −24…−20 дБ одиночки, и вытеснить
/// настоящую рощу из бюджета не могут: места достаются САМЫМ ГРОМКИМ.
