...
- 17:08:2026 - 17:49:29: DeckOnJoints смотрит на `deck-`, а не на `floor-`. Семейства `floor-`
  на полке НЕТ и не было: правило, написанное на несуществующее семейство, не
  может покраснеть ни на одной настоящей сцене — то есть не проверено, сколько
  бы тестов вокруг него ни стояло. Наведённое на настоящий настил, оно в тот же
  час нашло дефект в лестничной клетке houses/demo: оба несущих лежня стояли по
  одну сторону от середины настила (-1.75..-0.75 м), и дальняя половина висела.
*/
...
void check_house_rules(const SceneDoc& doc, const SceneWorld& world,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        // `deck-`, а не `floor-`: правило, написанное на семейство, которого на
        // полке НЕТ, не может покраснеть ни на одной настоящей сцене — то есть
        // не проверено по правилу 1, сколько бы тестов вокруг него ни стояло.
        // Настил появился вместе с проёмом (§9.1), и с этой строки правило
        // наконец смотрит на то, что в мире есть.
        if (!starts(p.object, "deck-")) {
            continue;
        }
        glm::vec2 blo{0.0f};
        glm::vec2 bhi{0.0f};
