...
- 23:08:2026 - 23:40:00: SceneAir.ambient — общий свет локации (И15 шаг 4: у интерьера нет
  неба, и ambient задаёт композитор, а не час суток снаружи).
- 27:08:2026 - 00:12:00: ScenePlacedHouse.sealed — ключ `sealed` у [house] (И15 волна Б,
  «дома снаружи болванками», заказ владельца 26.08). Створка запечатанной
  постройки входит в коллайдер: убранство уехало в локацию, и проём ведёт
  теперь не в комнату, а в пустую оболочку мимо перехода. Ключ у РАЗМЕЩЕНИЯ,
  а не у чертежа: city-house-s.dfh носят 11 домов Вайтрана и 6 Житнова, и
  portal=1 в общем чертеже запечатал бы каждый проём каждой карты, где он
  встречается. Только добавление (правило 26): сцена без ключа читается и
  пишется бит-в-бит как раньше.
*/
...
struct ScenePlacedHouse {
    ...
    std::string interior;
    /// БОЛВАНКА: створка этой постройки ВХОДИТ В КОЛЛАЙДЕР (ключ sealed, И15
    /// волна Б). Дверное полотно намеренно живёт вне коллайдера — оно
    /// качается, — и до волны Б это было верно: сквозь проём вели внутрь, там
    /// стояла мебель. С переездом убранства в локацию проём ведёт в ПУСТУЮ
    /// оболочку, а внутрь ведёт переход; незапечатанная створка тогда — дыра
    /// мимо всей механики.
    ///
    /// КЛЮЧ У РАЗМЕЩЕНИЯ, А НЕ У .dfh, И ЭТО НЕ ВКУС. Один чертёж
    /// (city-house-s.dfh) носят одиннадцать домов Вайтрана и шесть Житнова;
    /// portal=1 в общем чертеже запечатал бы КАЖДЫЙ проём каждой карты, где
    /// он встречается, включая стенды кузницы и демки, где сквозь дверь ходят
    /// намеренно. Ключ элемента portal=1 остаётся и работает как прежде — это
    /// второй, чертёжный способ сказать то же самое (пилот волны А).
    bool sealed = false;
    std::string note;
};
...
struct SceneAir {
    ...
    /// ОБЩИЙ СВЕТ ЛОКАЦИИ (ключ ambient, И15), 0..1; < 0 — не задан.
    /// У интерьера нет неба, и весь свет, кроме очага, — это ambient сцены:
    /// каменный подвал и мазанка с окнами живут при разном, и число это
    /// решение композитора, а не следствие часа суток снаружи.
    float ambient = -1.0f;
};
