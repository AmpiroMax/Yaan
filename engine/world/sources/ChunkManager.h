...
public:
    ...
    /// СКОЛЬКО ЧАНКОВ ЕЩЁ НЕ ПРИЕХАЛО в радиус загрузки вокруг `focus_position`
    /// — то есть очередь, которую было НЕЧЕМ увидеть. Приложение выводило
    /// «мир ещё грузится» из СОБЫТИЙ ChunkLoaded, увиденных за кадр: это
    /// верно для «что-то приехало» и слепо к «что-то стоит в очереди и не
    /// приехало». Затвор беспилотного кадра ждал именно второе, а спрашивал
    /// первое — и на кадре, где очередь была непуста, а за этот кадр ничего
    /// не admitted, мир читался успокоившимся.
    ///
    /// Считается ровно так же, как считает пропуск загрузки в update():
    /// клетки радиуса вокруг чанка фокуса, обрезанные ГЕНЕРИРУЕМЫМ ПРОТЯЖЕНИЕМ
    /// (клетка за краем мира не приедет НИКОГДА, и считать её ожидающей значит
    /// не дождаться никогда), минус резиденты. Ноль означает «в радиусе всё
    /// на месте», а не «работы больше нет»: крупная сетка дальней земли —
    /// отдельный счётчик, coarse_pending_count().
    [[nodiscard]] std::size_t pending_chunk_count(const glm::vec3& focus_position) const;
