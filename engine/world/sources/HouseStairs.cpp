...
void build_stairs(const Element& e, const ElementParams& p, glm::vec3 a, glm::vec3 b,
    ...
    // РАМПА-ОСНОВАНИЕ ПОД ОТКРЫТЫМИ СТУПЕНЯМИ. Капсула игрока (R=0.35) шире
    // проступи (0.35): между кромками соседних плит она оседает ниже верха
    // ступени, и следующий подступёнок оказывается выше лимита шага (0.35) —
    // персонаж намертво садится на ребро. Ловилось только на 60 Гц: за кадр
    // капсула смещается на 3 см и успевает осесть; крупный шаг симуляции
    // перелетает ребро (репродукция капсулой на городской лестнице Вайтрана,
    // 21.08: стоп в той же точке при любом положении марша). Непрерывная
    // наклонная опора в 2 см под линией марша не даёт осесть глубже: контакт
    // с рампой (26°, walkable) выталкивает к следующей кромке.
    if (p.open > 0.5f) {
        const glm::vec3 drop{0.0f, -0.18f, 0.0f};
        const glm::vec3 lo = a + drop;
        const glm::vec3 hi = a + dir * run + glm::vec3{0.0f, rise_total, 0.0f} + drop;
