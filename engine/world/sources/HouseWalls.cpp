...
static void build_wall_plate(const Element& e, const ElementParams& p,
    ...
    if (windows) {
        ...
        mb.set_material(8, -1);
        // ЛИСТ ОКНА СВЕТИТСЯ, И ЭТО НЕ УКРАШЕНИЕ (28.08). Приборный разбор
        // волны света интерьеров: у референса владельца контраст p95/p05
        // медианно 106x, у нас после подъёма ambient — 7..24x, и весь разрыв
        // это ПЕРЕСВЕТ, которого в кадре не было ни одного. Днём ярчайшее
        // пятно комнаты — окно, а не пол под очагом; чтобы оно им стало,
        // вставке нужен свой признак (MeshPart.pane), потому что она живёт
        // ПОДЧАСТЬЮ стенного элемента и параметра glow у неё быть не может.
        //
        // ФЛАГ ПОСЛЕ set_material, А НЕ ДО: смена материала закрывает часть
        // простенков, и поднятый заранее признак уехал бы на неё.
        mb.pane = true;
        for (const OpeningPlacement& op : holes) {
            ...
            // ПРОЁМ — ПЕЧКЕ ВИДИМОСТИ. Записывается ЗДЕСЬ, где раскладка
            // проёмов ещё жива: в собранном буфере от окна остаётся лист
            // остекления среди тысяч треугольников, и «где здесь было окно»
            // восстанавливалось бы угадыванием по материалу.
            const float w = op.u1 - op.u0;
            const float h = op.v1 - op.v0;
            if (w > HOUSE_GEOM_EPS && h > HOUSE_GEOM_EPS) {
                mesh.windows.push_back(
                    {a + dir * (op.u0 + w * 0.5f)
                       + glm::vec3{0.0f, op.v0 + h * 0.5f, 0.0f},
                     face_n, w * h});
            }
        }
        // И СНИМАЕТСЯ ПОСЛЕ set_material — по той же причине, по которой
        // ставился после неё: закрывает часть ЛИСТА именно эта строка.
        mb.set_material(-1, -1);
        mb.pane = false;
    }
}
