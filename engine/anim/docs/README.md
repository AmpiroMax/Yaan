...
- `Locomotion.h` — ОДИН ХОЗЯИН ДВИЖЕНИЯ (docs/design/LOCOMOTION_GROUNDED.md
  §16): машина состояний `LocoMachine`/`loco_step()` (Idle, TurnInPlace,
  Start, Cycle, Stop, Stagger, Air; решения на входе и по концу клипа, один
  анти-дребезг `LOCO_STATE_DWELL_S`), `loco_root_delta()` — ход и рыск за тик
  из дорожки корня клипа (`ClipEntry::root`), `loco_pick_turn()` — ближайший
  клип поворота с варпом рыска, `LocomotionOut` — заявка сима (смещение в
  системе тела, рыск, постановки и опора по расписанию, класс направления,
  владение рыском, вербатим), `rotate_root_joints()` — остаток варпа на позу.
  Прежний RootMotion.h (корень от опорной стопы) снесён фазой 6.
- `LocoTelemetry.h` — ПРИБОРЫ ЛОКОМОЦИИ В ИГРЕ: тик тела → детекторы с
  порогами реестра (ход опорной стопы, зазор, рывки суставов, перекрест,
  змейка, снос без ввода, разрыв фазы, скрутка, колено, ошибка скорости,
  рыск, смены клипа в секунду, варп), HUD `DFN_LOCO_HUD`, CSV `DFN_LOCO_CSV`.
