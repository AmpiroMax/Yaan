...
struct MeshGap {
    ...
    uint32_t bands = 0;
    /// ВЫСОТА ПОЛОСЫ, в которой боковой зазор наименьший, метры (середина
    /// полосы). NaN без бокового замера. Это плечо рычага для решателя
    /// рест-позы: у ног самая тесная полоса лежит у промежности, в нескольких
    /// сантиметрах от сустава бедра, а не у лодыжки, и рычаг «вся нога»
    /// недооценивал нужный угол в пятнадцать раз (RestFit.cpp).
    float worst_y = std::numeric_limits<float>::quiet_NaN();
