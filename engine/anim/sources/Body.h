...
struct BodyDrive {
    ...
    float view_yaw = 0.0f;
    /// Есть ли у этого тела взгляд вообще: у игрока — да (камера), у НПС,
    /// смотровой и экрана создания — нет, и поворот на месте им не стреляет.
    /// Без этого флага тело с view_yaw = 0 и лицом на восток крутилось бы
    /// вечно (замер 07.09: смотровая фигура переступала сама с собой).
