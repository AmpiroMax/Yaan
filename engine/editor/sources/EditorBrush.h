...
- StrokeRefresh: КОГДА ПОКАЗАТЬ ЗЕМЛЮ, ПОКА КНОПКА ЕЩЁ ЗАЖАТА — решение
  отдельным предметом, потому что «в реальном времени» на перестройке в 196 мс
  означает не «каждый кадр», а «так часто, как можно, не съев кадр».
...
- 18:08:2026 - 13:08:07: StrokeRefresh — ЗЕМЛЯ ДВИГАЕТСЯ, ПОКА ВЕДЁШЬ, а не одним скачком на
  отпускании (заказ 18.08: «хочу изменение ландшафта от инструмента высоты в
  реальном времени, сейчас высота меняется только когда я мышь отпущу, а мне так
  непонятно что происходит»). Мазок и раньше считался покадрово — не считался
  ПОКАЗ: rebuild_dirty звался только из finish_stroke.
  ЗАМЕР, ИЗ-ЗА КОТОРОГО ЭТО НЕ «КАЖДЫЙ КАДР» (12 прогонов на живом кольце 3x3,
  сид 123): перестройка ОДНОГО чанка под кистью — 196.4 мс в среднем (135.4
  лучшая, 259.9 худшая), потоковая загрузка того же чанка — 181.0 мс. То есть
  один показ стоит двенадцати кадров при 60 к/с. Частота выведена ИЗ ЭТОЙ ЦЕНЫ,
  а не назначена: следующий показ разрешается через REFRESH_COST_RATIO цен
  последнего, значит на перестройку уходит не больше 1/(1+4) = 20 % времени
  штриха на ЛЮБОЙ машине, а не столько, сколько выйдет на моей.
*/
...
// ЧИСЛА ПОКАЗА ЖИВУТ В docs/NUMBERS.md И БЕРУТСЯ ОТТУДА (правило 14):
// config::REFRESH_COST_RATIO — во сколько раз дольше ждать, чем стоил показ
// (доля времени штриха на перестройку ≈ 1/(1+4) = 20 %, и это число про
// УСТРОЙСТВО, а не про мою машину: цена измеряется каждым показом);
// config::REFRESH_MIN_PERIOD_S / _MAX_PERIOD_S — пол и потолок паузы. Довод
// целиком, вместе с замером 196 мс на чанк, записан там же одной таблицей.
// Копии здесь не будет: разошедшиеся числа дали бы кисть, которая в проверке
// показывает землю с одной частотой, а в игре с другой.

/// КОГДА ВЫВЕСТИ ИЗМЕНЁННУЮ ЗЕМЛЮ НА ЭКРАН, ПОКА ШТРИХ ИДЁТ.
///
/// Почему это отдельный предмет, а не два поля в App: App держит окно и не
/// заводится в проверке, а утверждение «за штрих земля изменилась БОЛЬШЕ ОДНОГО
/// РАЗА» невыразимо ни в одном кадре — кадр показывает одно состояние. Здесь оно
/// становится счётчиком, и рядом с ним встаёт его контроль: короткий штрих в
/// один кадр обязан дать РОВНО ОДИН показ, иначе проверка пройдёт и на коде,
/// который по-прежнему копит всё до отпускания.
///
/// ПЕРВЫЙ МАЗОК ПОКАЗЫВАЕТСЯ СРАЗУ. Ожидание считается ПОСЛЕ показа, а не до
/// него: иначе первые полсекунды штриха выглядели бы точно так же, как сломанная
/// кисть, — то есть жалоба осталась бы на месте в самый заметный момент.
struct StrokeRefresh {
    /// Сколько ждать до следующего показа. Ноль на старте — см. выше.
    float wait_s = 0.0f;
    float since_s = 0.0f;
    /// Земля изменилась с прошлого показа. Без этого флага отпускание давало бы
    /// второй показ там, где показывать нечего, и контрольный случай (один кадр
    /// = один показ) был бы ложью.
    bool pending = false;
    /// Сколько раз за этот штрих землю показали. Это и есть измеряемая величина.
    int pushes = 0;
    /// Сколько времени ушло на показы — вторая половина того же вопроса: цена.
    float spent_s = 0.0f;

    /// Один кадр штриха. `dabbed` — «в этом кадре земля действительно
    /// изменилась» (BrushDabReport::any), а не «кнопка зажата»: кисть, ушедшая
    /// за край мира, ничего не меняет, и перестраивать нечего.
    [[nodiscard]] bool step(bool dabbed, float dt_s);

    /// Во сколько обошёлся последний показ, в секундах. Отсюда берётся пауза —
    /// поэтому число не назначено в коде, а измеряется на той машине, где идёт
    /// работа.
    void note_cost(float rebuild_s);

    /// Штрих кончился. Правда наводится ПОЛНОСТЬЮ и здесь (caller drains the
    /// queue), поэтому этот вызов только закрывает счётчики.
    void end();
};
