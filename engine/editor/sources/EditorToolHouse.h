...
public:
    ...
    /// показал кадром — вместо стойки вышла длинная прямая, лежащая на траве.
    /// Это не оплошность инструмента, а свойство задачи: у мыши две координаты,
    /// у точки три, и без ограничения перевод из одного в другое неоднозначен.
    ///
    /// Ось задаёт НЕДОСТАЮЩЕЕ УСЛОВИЕ: конец обязан лежать на прямой, идущей
    /// из якоря вверх. Тогда движение мыши читается однозначно — как сдвиг
    /// вдоль этой прямой, а не как точка на земле.
    ///
    ...
    /// и она же единственная, кого App держит полем.
    enum class Axis : std::uint8_t {
        Ground = 0, ///< как раньше: точка там, где луч встретил землю
        Vertical,   ///< вертикаль через якорь: стойки, столбы, этажи
    };
    void set_axis(Axis a) { axis_ = a; }
    [[nodiscard]] Axis axis() const { return axis_; }
    /// Переключить: земля -> вертикаль -> земля. Одна клавиша, как у Blender.
    void toggle_axis() { axis_ = axis_ == Axis::Ground ? Axis::Vertical : Axis::Ground; }
    ...
    world::GraphResult mutate(std::string label, Fn&& fn) {
        ...
        if (r.ok) {
            // ВЕРСИЯ ГЕОМЕТРИИ РАСТЁТ ЗДЕСЬ, в единственной двери ко всем
            // мутациям: тело дома пересчитывает тот, кто заметил изменение, а
            // заметить его иначе можно только сравнением всего графа.
            ++version_;
...
public:
    ...
    [[nodiscard]] bool aims_without_ground() const override {
        return session_ != nullptr && from_ != world::NO_VERTEX
            && session_->axis() == HouseSession::Axis::Vertical;
    }
    [[nodiscard]] float max_reach_m() const override { return 60.0f; }
    [[nodiscard]] ToolStatus status(const ToolAim& aim) const override;
    void on_deselected(ToolWorld& world) override;
