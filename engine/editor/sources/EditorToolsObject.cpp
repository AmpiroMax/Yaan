...
- 20:08:2026 - 22:40:00: Щелчок мимо постройки снимает её выбор — панель выбора больше не залипает.
*/
...
void SelectTool::on_press(const ToolAim& aim, ToolWorld& world) {
    ...
    // ЩЕЛЧОК МИМО ПОСТРОЙКИ СНИМАЕТ ЕЁ ВЫБОР (UX-аудит 20.08: выбор части
    // постройки не сбрасывался НИКОГДА, и панель выбора навсегда показывала
    // стену — свойства сундука было не увидеть до удаления стены).
    if (house_ != nullptr) {
        house_->clear_selection();
    }
