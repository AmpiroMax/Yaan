...
- 18:08:2026 - 00:55:58: build_click_is_armed() и build_hand_wants_aim() — ЧТО ДЕЛАЕТ ЛЕВАЯ
  КНОПКА, выражением. Заказ 18.08: «я должен использовать только один инструмент
  одновременно, но почему-то и кисть высоты, и постройку объектов делаю».
  Разбор: обе руки висят на ЛЕВОЙ кнопке (кисть по is_down, постановка по
  was_pressed), а постановка взводилась условием «выбран режим постановки ИЛИ
  ОТКРЫТ СПИСОК ОБЪЕКТОВ». При открытом списке один щелчок и копал, и ставил.
  Над той строкой уже стоял комментарий «рука отвечает РЕЖИМУ, а не тому,
  открыто ли меню» — правило было записано словами и не записано кодом. Теперь
  оно выражение, и на него есть таблица в app_editor_palette.
- 18:08:2026 - 01:54:26: any_panel_open() / close_all_panels() — для ESC (заказ 18.08). Вопрос
  задаётся каркасу, а не списку идентификаторов: имена панелей знает тот, кто их
