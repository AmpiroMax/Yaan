/*
Module: engine/core/serialization
File: engine/core/serialization/sources/Sha256.cpp

Responsibility:
- Reference SHA-256 (FIPS 180-4): message schedule, compression, padding.

Dependencies:
- Uses: Sha256.h, std.
- Used by: dfn_core.

AI Agents Notice (must follow):
- Follow docs/ARCHITECTURE.md strictly.
- Checked against the FIPS vectors in tests/core/Sha256Tests.cpp; the
  algorithm is frozen (see the header).
*/

#include "engine/core/serialization/sources/Sha256.h"

#include <bit>
#include <cstdio>
#include <cstring>
#include <vector>

namespace dfn::serialization {
namespace {

constexpr std::array<uint32_t, 64> K = {
    0x428a2f98u, 0x71374491u, 0xb5c0fbcfu, 0xe9b5dba5u, 0x3956c25bu, 0x59f111f1u,
    0x923f82a4u, 0xab1c5ed5u, 0xd807aa98u, 0x12835b01u, 0x243185beu, 0x550c7dc3u,
    0x72be5d74u, 0x80deb1feu, 0x9bdc06a7u, 0xc19bf174u, 0xe49b69c1u, 0xefbe4786u,
    0x0fc19dc6u, 0x240ca1ccu, 0x2de92c6fu, 0x4a7484aau, 0x5cb0a9dcu, 0x76f988dau,
    0x983e5152u, 0xa831c66du, 0xb00327c8u, 0xbf597fc7u, 0xc6e00bf3u, 0xd5a79147u,
    0x06ca6351u, 0x14292967u, 0x27b70a85u, 0x2e1b2138u, 0x4d2c6dfcu, 0x53380d13u,
    0x650a7354u, 0x766a0abbu, 0x81c2c92eu, 0x92722c85u, 0xa2bfe8a1u, 0xa81a664bu,
    0xc24b8b70u, 0xc76c51a3u, 0xd192e819u, 0xd6990624u, 0xf40e3585u, 0x106aa070u,
    0x19a4c116u, 0x1e376c08u, 0x2748774cu, 0x34b0bcb5u, 0x391c0cb3u, 0x4ed8aa4au,
    0x5b9cca4fu, 0x682e6ff3u, 0x748f82eeu, 0x78a5636fu, 0x84c87814u, 0x8cc70208u,
    0x90befffau, 0xa4506cebu, 0xbef9a3f7u, 0xc67178f2u};

void compress(std::array<uint32_t, 8>& h, const uint8_t block[64]) {
    uint32_t w[64];
    for (int i = 0; i < 16; ++i) {
        w[i] = (static_cast<uint32_t>(block[i * 4]) << 24)
               | (static_cast<uint32_t>(block[i * 4 + 1]) << 16)
               | (static_cast<uint32_t>(block[i * 4 + 2]) << 8)
               | static_cast<uint32_t>(block[i * 4 + 3]);
    }
    for (int i = 16; i < 64; ++i) {
        const uint32_t s0 = std::rotr(w[i - 15], 7) ^ std::rotr(w[i - 15], 18) ^ (w[i - 15] >> 3);
        const uint32_t s1 = std::rotr(w[i - 2], 17) ^ std::rotr(w[i - 2], 19) ^ (w[i - 2] >> 10);
        w[i] = w[i - 16] + s0 + w[i - 7] + s1;
    }
    uint32_t a = h[0], b = h[1], c = h[2], d = h[3], e = h[4], f = h[5], g = h[6], hh = h[7];
    for (int i = 0; i < 64; ++i) {
        const uint32_t S1 = std::rotr(e, 6) ^ std::rotr(e, 11) ^ std::rotr(e, 25);
        const uint32_t ch = (e & f) ^ (~e & g);
        const uint32_t t1 = hh + S1 + ch + K[static_cast<std::size_t>(i)] + w[i];
        const uint32_t S0 = std::rotr(a, 2) ^ std::rotr(a, 13) ^ std::rotr(a, 22);
        const uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
        const uint32_t t2 = S0 + maj;
        hh = g;
        g = f;
        f = e;
        e = d + t1;
        d = c;
        c = b;
        b = a;
        a = t1 + t2;
    }
    h[0] += a;
    h[1] += b;
    h[2] += c;
    h[3] += d;
    h[4] += e;
    h[5] += f;
    h[6] += g;
    h[7] += hh;
}

} // namespace

Sha256Digest sha256(std::span<const uint8_t> bytes) {
    std::array<uint32_t, 8> h = {0x6a09e667u, 0xbb67ae85u, 0x3c6ef372u, 0xa54ff53au,
                                 0x510e527fu, 0x9b05688cu, 0x1f83d9abu, 0x5be0cd19u};
    std::size_t off = 0;
    while (bytes.size() - off >= 64) {
        compress(h, bytes.data() + off);
        off += 64;
    }
    // Padding: 0x80, zeros to 56 mod 64, then the bit length big-endian.
    uint8_t tail[128] = {};
    const std::size_t rest = bytes.size() - off;
    std::memcpy(tail, bytes.data() + off, rest);
    tail[rest] = 0x80;
    const std::size_t tail_len = rest < 56 ? 64 : 128;
    const uint64_t bit_len = static_cast<uint64_t>(bytes.size()) * 8u;
    for (int i = 0; i < 8; ++i) {
        tail[tail_len - 1 - static_cast<std::size_t>(i)] =
            static_cast<uint8_t>(bit_len >> (8 * i));
    }
    compress(h, tail);
    if (tail_len == 128) {
        compress(h, tail + 64);
    }
    Sha256Digest out{};
    for (int i = 0; i < 8; ++i) {
        out[static_cast<std::size_t>(i) * 4] = static_cast<uint8_t>(h[static_cast<std::size_t>(i)] >> 24);
        out[static_cast<std::size_t>(i) * 4 + 1] = static_cast<uint8_t>(h[static_cast<std::size_t>(i)] >> 16);
        out[static_cast<std::size_t>(i) * 4 + 2] = static_cast<uint8_t>(h[static_cast<std::size_t>(i)] >> 8);
        out[static_cast<std::size_t>(i) * 4 + 3] = static_cast<uint8_t>(h[static_cast<std::size_t>(i)]);
    }
    return out;
}

std::string sha256_hex(const Sha256Digest& digest) {
    static constexpr char HEX[] = "0123456789abcdef";
    std::string s;
    s.reserve(64);
    for (const uint8_t b : digest) {
        s.push_back(HEX[b >> 4]);
        s.push_back(HEX[b & 0x0f]);
    }
    return s;
}

std::string sha256_hex(std::span<const uint8_t> bytes) {
    return sha256_hex(sha256(bytes));
}

std::optional<std::string> sha256_file(const std::filesystem::path& path) {
    std::FILE* f = std::fopen(path.string().c_str(), "rb");
    if (f == nullptr) {
        return std::nullopt;
    }
    std::vector<uint8_t> bytes;
    uint8_t buf[1 << 16];
    for (;;) {
        const std::size_t n = std::fread(buf, 1, sizeof(buf), f);
        if (n == 0) {
            break;
        }
        bytes.insert(bytes.end(), buf, buf + n);
    }
    const bool bad = std::ferror(f) != 0;
    std::fclose(f);
    if (bad) {
        return std::nullopt;
    }
    return sha256_hex(bytes);
}

} // namespace dfn::serialization
