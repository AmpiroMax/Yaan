...
- SkinPart: ЧАСТЬ ПЕРСОНАЖА на том же скелете — волосы, глаза, брови,
  ресницы, зубы, язык, одежда: имя, флаги материала (вырез по альфе, порог,
  двусторонность), свой поток SKIN, свои листы и список вершин тела, которые
  часть закрывает (секция PART).
- Socket: точка на кости для жёсткой мелочи — кольцо, оружие, поклажа
  (секция SOCK).
...
/// ОДНА ЧАСТЬ ПЕРСОНАЖА НА ТОМ ЖЕ СКЕЛЕТЕ (.dfo, секция PART; волна «части
/// персонажа»): волосы, глаза, брови, ресницы, зубы, язык — отдельный меш со
/// своими весами к суставам тела, своими листами и своими флагами материала.
///
/// ПОЧЕМУ СПИСОК ВНУТРИ ОДНОГО ФАЙЛА, А НЕ ФАЙЛ НА ЧАСТЬ. Части печёт один
/// экспорт (HumanBase.parts.glb) на один риг; скелет у них ОДИН, и файл на
/// часть нёс бы шесть копий пятидесяти трёх суставов, каждую из которых
/// загрузчик обязан сверить с телом. Набор при этом остаётся открытым: файл
/// несёт 1..N частей, и причёска, испечённая отдельно (hair-short01.parts.dfo),
/// — это тот же формат с одной частью; тело прикрепляет столько файлов,
/// сколько ему назвали.
///
/// ПОЧЕМУ НЕ ШЕСТОЙ ПОТОК ТЕЛА. Часть отличается от тела не только мешем, а
/// МАТЕРИАЛОМ: у пряди волос вырез по альфе и свет с обеих сторон, у кожи —
/// ни того, ни другого. Это тот же довод, по которому куски постройки —
/// список HouseSubmesh, а не пятый MeshData: программ отрисовки конечное
/// число, а материалов — нет.
///
/// МОРФЫ ТЕЛА К ЧАСТЯМ ФОРМАТОМ НЕ АДРЕСУЮТСЯ: цель называет вершины потока
/// SKIN по номеру, у части свой поток. Часть следует за телом ПЕРЕНОСОМ
/// (engine/render MorphFollow.h: карта соседей по нейтральному телу, глаза и
/// зубы — жёстко за масками лица), и файл части об этом не знает — ни одной
/// дельты на часть в формате нет и не нужно.
struct SkinPart {
    std::string name; ///< "hair", "eyes", "eyebrows" — имя узла glTF; ключ дозы DFN_PARTS
    /// glTF alphaMode MASK: тексели с альфой ниже порога не рисуются и не
    /// отбрасывают тень; false — OPAQUE (зубы, язык).
    bool alpha_mask = false;
    float alpha_cutoff = 0.5f;
    /// glTF doubleSided: карточка освещается с обеих сторон (нормаль
    /// разворачивается к глазу), иначе изнанка пряди чёрная.
    bool double_sided = false;
    SkinMesh mesh;
    /// Листы части по ролям (albedo, normal, ao) — те же ссылки, что у тела.
    std::vector<TextureRef> textures;
    /// ВЕРШИНЫ ТЕЛА, КОТОРЫЕ ЭТА ЧАСТЬ ЗАКРЫВАЕТ (номера в потоке SKIN тела,
    /// к которому часть печётся; extras.hide_body_vertices у mhclo-прокси
    /// MakeHuman). Треугольник тела, все три вершины которого в списке,
    /// не рисуется, пока часть надета — иначе кожа просвечивает сквозь
    /// костюм на каждом сгибе. Пусто у волос и глаз: они ничего не закрывают.
    /// ДАННЫЕ, А НЕ ГЕОМЕТРИЯ: тело остаётся целым в файле, и снятый костюм
    /// возвращает кожу без перепечки.
    std::vector<std::uint32_t> hide_body_vertices;
    /// Первая ссылка названной роли или nullptr.
    [[nodiscard]] const TextureRef* texture(std::string_view role) const;
};

/// СОКЕТ — ТОЧКА НА КОСТИ, К КОТОРОЙ ЦЕПЛЯЕТСЯ ЖЁСТКАЯ МЕЛОЧЬ (.dfo, секция
/// SOCK): кольцо на пальце, браслет, амулет, оружие в ладони, поклажа за
/// спиной. Сустав и точка покоя В МОДЕЛЬНОМ ПРОСТРАНСТВЕ ЭТОГО ТЕЛА (то же
/// пространство, что у вершин SKIN: после осевой правки, масштаба и до
/// палитры) — то есть сокет позируется РОВНО КАК ВЕРШИНА с одним весом:
/// point_posed = palette[joint] * rest_point. Второй арифметики «где кость»
/// не заводится, и сокет не может отстать от кисти, к которой пришит.
///
/// ПОЧЕМУ В ФАЙЛЕ ТЕЛА, А НЕ В JSON РЯДОМ. Точка измерена в системе glb
/// (assets/characters/sockets.json), а тело в файле — в своей: осевая
/// правка, рыск, масштаб к росту. Перевод знает импортёр и только он; читать
/// json в игре значило бы завести вторую копию этой арифметики в app.
struct Socket {
    std::string name;     ///< "ring.L", "weapon.R", "back" — ключ прикрепления
    std::uint32_t joint = 0;
    glm::vec3 rest_point{0.0f};
};
...
struct RegistryObject {
    ...
    /// Первая ссылка названной роли или nullptr.
    [[nodiscard]] const TextureRef* texture(std::string_view role) const;
    /// ЧАСТИ ПЕРСОНАЖА (секция PART). Пусты у всего, что не набор частей, —
    /// и ровно поэтому их файлы не изменились ни на бит: пустой список в
    /// личность объекта не входит (шестой случай довода HOUS/MTRL/SKIN/MORF/
    /// TEX). Файл набора несёт SKEL (скелет, к которому части привязаны) и
    /// пустой SKIN: он не персонаж, а то, что персонаж носит.
    std::vector<SkinPart> parts;
    /// СОКЕТЫ ТЕЛА (секция SOCK). Пусты у всего, что не тело с сокетами, и
    /// в личность входят только непустыми — седьмой случай того же довода.
    std::vector<Socket> sockets;
    /// Сокет по имени или nullptr.
    [[nodiscard]] const Socket* socket(std::string_view name) const;
