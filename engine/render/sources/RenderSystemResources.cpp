...
- 27:08:2026 - 14:00:00: СЕТКА ИНТЕРФЕЙСА — FULL HD (UI_CANVAS_W вместо
...
void RenderSystem::publish_point_lights(std::vector<PointLightCandidate>& candidates) {
    ...
    for (const PointLightCandidate& c : candidates) {
        ...
        out.room_center_xz = c.room_center_xz;
        out.room_half_xz = c.room_half_xz;
...
void RenderSystem::set_internal_resolution(uint32_t width, uint32_t height) {
    if (width > 0 && height > 0) {
        ...
        //
        // И С 27.08.2026 ЭТА СЕТКА — FULL HD, А НЕ 640 (заказ владельца: «везде
        // качество поднимаем до базового fullhd» + «шрифт интерфейса слишком
        // угловатый»). Это одно решение, а не два. Пока интерфейс рисовался
        // блочным шрифтом 5×8, мелкий холст был ПРАВ: холст 640×360 на кадре
        // 1920×1080 давал целочисленный делитель 3, буквы выходили ровными
        // блоками, и увеличить их означало увеличить блок — то есть сделать
        // угловатость крупнее. Теперь интерфейс рисует настоящую антикву,
        // испечённую офлайн (assets/fonts/ui_serif.*), и ей нужны пиксели: на
        // боевых настройках делитель равен единице, холст интерфейса совпадает
        // с кадром пиксель в пиксель, и глиф в 60 px — это глиф в 60 px, а не
        // двадцать блоков по три. Число живёт в docs/NUMBERS.md (UI_CANVAS_W).
        //
        // РЕТРО-ПРЕСЕТЫ НЕ СЛОМАНЫ: при сцене 640×360 делитель тоже 1 (округление
        // 640/1920 даёт ноль, зажатый в единицу), то есть интерфейс просто
        // уменьшается вместе с кадром — а размеры шрифта заданы ДОЛЯМИ высоты
        // холста, не пикселями, так что пропорции держатся на всех пресетах.
        const auto design_w = static_cast<uint32_t>(config::UI_CANVAS_W);
