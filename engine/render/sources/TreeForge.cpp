...
- 28:08:2026 - 16:40:00: pack_wind/bark_tube ВЫНЕСЕНЫ в TreeBark.{h,cpp} — дословно,
  без единой правки тела, чтобы вторая итерация деревьев (TreeForgeV2) брала то
  же отображение тайла коры, а не свою копию. Логика forge_tree не тронута;
  сверка: полка .dfo перепечена после переноса, все 2563 файла байт в байт
  прежние (контрольный список хэшей — в отчёте волны). Это единственная правка
  файла в волне v2: железное правило владельца — первая итерация не меняется.
*/
