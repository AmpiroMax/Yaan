...
constexpr float MAP_SHADE_MIN = 0.55f;
constexpr float MAP_SHADE_SPAN = 0.85f;
...
void MapScreen::note_chunk(const math::HeightFieldView& field,
    ...
    const uint32_t cells = res - 1;                    // 128 at shipping numbers
    const uint32_t step = std::max(1u, cells / MAP_TILE_PX); // samples per map pixel
    const float max_height = static_cast<float>(config::WORLDGEN_MAX_HEIGHT);
    ...
    // Pass 1: block-averaged heights + water flags at map-pixel resolution.
    std::vector<float> heights(static_cast<size_t>(MAP_TILE_PX) * MAP_TILE_PX, 0.0f);
    std::vector<uint8_t> water(static_cast<size_t>(MAP_TILE_PX) * MAP_TILE_PX, 0);
    for (uint32_t pz = 0; pz < MAP_TILE_PX; ++pz) {
        for (uint32_t px = 0; px < MAP_TILE_PX; ++px) {
            float sum = 0.0f;
            uint32_t count = 0;
            bool wet = false;
            for (uint32_t dz = 0; dz < step; ++dz) {
                for (uint32_t dx = 0; dx < step; ++dx) {
                    const uint32_t sx = std::min(px * step + dx, res - 1);
                    const uint32_t sz = std::min(pz * step + dz, res - 1);
            ...
            heights[i] = sum / static_cast<float>(std::max(count, 1u));
            water[i] = wet ? 1u : 0u;
        }
    }

    // Pass 2: colorize (elevation ramp x hill shade; water flat and dark).
    Tile tile;
    tile.rgb.resize(static_cast<size_t>(MAP_TILE_PX) * MAP_TILE_PX * 3);
    for (uint32_t pz = 0; pz < MAP_TILE_PX; ++pz) {
        for (uint32_t px = 0; px < MAP_TILE_PX; ++px) {
            const size_t i = static_cast<size_t>(pz) * MAP_TILE_PX + px;
            Color color;
            if (water[i] != 0) {
                ...
                const float dx = heights[static_cast<size_t>(pz) * MAP_TILE_PX + xp]
                               - heights[static_cast<size_t>(pz) * MAP_TILE_PX + xm];
                const float dz = heights[static_cast<size_t>(zp) * MAP_TILE_PX + px]
                               - heights[static_cast<size_t>(zm) * MAP_TILE_PX + px];
                const float run = px_meters * static_cast<float>(xp - xm);
                const float run_z = px_meters * static_cast<float>(zp - zm);
                const glm::vec3 normal = glm::normalize(
                    glm::vec3(-dx / std::max(run, 1e-3f), 1.0f,
                              -dz / std::max(run_z, 1e-3f)));
                const float lambert = std::max(glm::dot(normal, MAP_LIGHT), 0.0f);
                color = shade(ramp_color(heights[i] / max_height),
                              MAP_SHADE_MIN + MAP_SHADE_SPAN * lambert);
            }
            tile.rgb[i * 3 + 0] = color.r;
            tile.rgb[i * 3 + 1] = color.g;
            tile.rgb[i * 3 + 2] = color.b;
        }
    }
...
void MapScreen::note_site(uint32_t mesh_asset_id, glm::vec3 position) {
    ...
    markers_.push_back({{position.x, position.z}, kind});
}
    ...
    for (const auto& [key, tile] : tiles_) {
        ...
        for (int j = 0; j < tile_px; ++j) {
            for (int i = 0; i < tile_px; ++i) {
                const size_t s = (static_cast<size_t>(j * down) * MAP_TILE_PX
                                  + static_cast<size_t>(i * down)) * 3;
                canvas_.put(tx + i, ty + j,
                            Color{tile.rgb[s], tile.rgb[s + 1], tile.rgb[s + 2]});
            }
        }
    }
