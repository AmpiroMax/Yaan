...
Last updated: 28:08:2026 - 19:04:53
-->
<!--
UPD:
...
- 13:08:2026 - 18:00:42: Правило 53 — величина, возвращающаяся к началу, невидима замеру по КОНЦАМ; считай ПЕРЕХОДЫ. Бот нажал глагол 32 раза, перепись дала 0 → 0 и это дословно совпало с жалобой пользователя — а на деле было шестнадцать открываний и шестнадцать закрываний. Третья форма одного семейства: между кадрами (рябь, мигание), между концами (переключатель), под лепестком (правило 50). Во всех трёх прибор исправен, уверен и отвечает НОЛЬ.
- 13:08:2026 - 22:20:15: Правило 54 — доказательство стареет вместе с деревом. Пользователю принесли выбор по кадрам, снятым до девяти коммитов и другого рельефа; он отверг сами кадры («хотя бы с текущего лучшего билда»). Вторая половина — принцип дня из истории берёзы: отвергнутый образец обязан быть воспроизводимым, а не помнимым; порог без живого образца — воспоминание, а не измерение.
- 13:08:2026 - 23:20:00: Правка ЗАПИСИ: предыдущая строка (правило 54) была датирована завтрашним числом (14:08 00:20 при стенных 22:20). Запись, датированная позже тех, что за ней последовали, делает порядок событий в блоке нечитаемым.
- 18:08:2026 - 12:27:18: ПРАВИЛО 21 — потолок файла поднят 800 -> 2500 решением пользователя,
- 24:08:2026 - 00:48:21: Rule 29 дополнен лестницей разделяемых ресурсов: общий файл (переплетение UPD), общий адресат работы (двойная маршрутизация находки, 25 секунд), авторство коммита — не данные.
- 24:08:2026 - 00:49:44: Rule 29 — третья часть правила кладущего («остальным сказано, что они не исполнители») и различие письмо/файл.
  дословно: «я думаю на больше узлы можно лимит до 1500-2500 поднимать...
  нормально когда есть большой файл и он много связывает». Прежний довод Q23
  (три параллельных агента, крупные файлы — источник конфликтов) не отменён, но
  он был не про ту величину: файл конфликтен числом РЕШЕНИЙ, которые правят
  разные руки, а не числом строк, и композиционный корень остаётся длинным,
  сколько его ни режь. Вместе с числом в правило записано то, что важнее числа:
  решение, которое нельзя проверить, не имеет права жить в файле, владеющем
  окном — потолок такого дефекта не ловит ни при 800, ни при 2500.
- 27:08:2026 - 12:03:42: Правило 27 дополнено ИМЕНОВАННЫМ СЛУЧАЕМ: критерий
  слеп к дефекту, который двигает измеряемую величину в БЕЗОПАСНУЮ сторону.
  Раскладка меню проверялась на НЕПЕРЕСЕЧЕНИЕ коробок строк — а дефектом был
  первый пункт В 1.6 РАЗА ВЫШЕ соседей, то есть коробка, которая не
  пересекается ни с кем ТЕМ ВЕРНЕЕ, чем крупнее дефект. Проверка была
  исправна, зелена и не могла покраснеть ни при каком размере акцента.
  Признак виден ДО прогона и стоит одного вопроса: в какую сторону дефект
  двигает то, что меряет критерий. Если в сторону прохождения — критерий
  меряет другую систему, сколько бы утверждений в нём ни стояло. Найдено на
  заказе владельца 27.08 («слово ПРОДОЛЖИТЬ бОльшим шрифтом написано»), где
  жалоба человека и зелёный набор описывали один и тот же кадр.
- 27:08:2026 - 21:52:00: ЖАЛОБА НА ДВИЖЕНИЕ МЕРЯЕТСЯ ЧАСТНЫМ, А НЕ СЛАГАЕМЫМ.
  Владелец сказал про качание герба в меню: «скорость сделать меньше, а то его
  сильно колбасит». У качания две ручки — амплитуда и период, — и обе по
  отдельности отвечают на жалобу неправильно: срезанная амплитуда при том же
  периоде просто ставит предмет анфас (уходит объём, ради которого качание и
  заведено), а растянутый период при той же амплитуде даёт тот же размах,
  только дольше. Глаз ловит УГЛОВУЮ СКОРОСТЬ, и у синуса её пик равен A·2π/T,
  то есть жалоба — про ЧАСТНОЕ. Правка сбавила обе ручки разом (13°/11 с ->
  7.5°/20 с, 4.5°/17 с -> 2.5°/29 с), и в мере жалобы это 7.43 -> 2.36 °/с и
  1.66 -> 0.54 °/с — ровно втрое, а не «на 42 % и на 82 %», как читались бы
  ручки порознь. Записано сюда, а не только у констант: тот же разбор нужен
  всякому «слишком быстро/дёргано» — у покачивания фонаря, у ветра листвы, у
  бега камеры, — и всякий раз соблазн покрутить одну ручку одинаков.
- 27:08:2026 - 22:09:50: Rule 29 — «коммит обязан собираться у того, кто его не писал» (два случая 28.08: Menu.cpp без Menu.h; App.h с чужим незакоммиченным DoorAim.h) + «--only берёт файл целиком, не diff» (едва не стёртая чужая запись UPD). Авторство: находка menufx, обобщение и замеры — волна музыки; вписано координатором — лидская запись вакантна (фальш-лид удалён владельцем 27.08).
- 27:08:2026 - 22:10:35: Rule 29 — рецепт чистого частичного коммита из общего файла: пламбинг (временный индекс + commit-tree + update-ref CAS), образец 95585eb волны прицела.
- 27:08:2026 - 22:10:55: Rule 29 — честная оговорка к проверке разреза: grep по именам не ловит устаревший заголовок; полная проверка — сборка из worktree коммита (замер menufx: его же проверка не ловила его же дефект).
- 27:08:2026 - 22:24:14: Rule 29 — застава tools/check_commit_builds.py (7bdb5c3): -fsyntax-only против дерева коммита за секунды; --range для унаследованной поломки (окно ee031b0 было шириной в три коммита, двое невиновных); строка про «12 минут» снята как протухшая.
- 27:08:2026 - 22:39:33: Rule 29 — итог вечера заставы (cd5be7d): три починки класса «зелёный ответ там, где ответа нет» (cwd, merge — до появления предмета); правила рукавов: рука печатает числа, не только приговор; вопрос — «что сломается следующим».
- 28:08:2026 - 14:25:46: Rule 47 — дополнение «одна сборка, одна камера, одна доза» для кадров до/после (два независимых свидетельства: фаска §8 и герб материалов); вписано координатором.
- 28:08:2026 - 19:04:53: Rule 47 — четвёртый член «один момент мира» (волна тура 7627041: шаги симуляции были функцией стенной дельты; затвор по готовности мира).
-->
