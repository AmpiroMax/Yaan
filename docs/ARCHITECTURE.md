...
Last updated: 27:08:2026 - 22:10:55
-->
<!--
UPD:
...
- **КОММИТ ОБЯЗАН СОБИРАТЬСЯ У ТОГО, КТО ЕГО НЕ ПИСАЛ** (28.08, два случая
  ...
  которого нет в HEAD (`git show HEAD:файл | grep -c имя`,
  `git ls-files путь`); `git stash` в общем дереве не годится. ЧЕСТНАЯ
  ОГОВОРКА (замер menufx): проверка по именам ловит зависимость от
  незакоммиченного ФАЙЛА, но НЕ ловит устаревший заголовок (Menu.h в
  ee031b0 существовал — в нём не было имён); полная проверка одна — сборка
  дерева коммита из `git worktree add --detach <sha>` (не бесплатно,
  холодная ~12 мин из-за bgfx; для рискованных разрезов — стоит того).
