...
Last updated: 10:08:2026 - 02:39:21
-->
<!--
UPD:
...
- 10:08:2026 - 01:54:37: TECH-DEBT WAVE task 1 — THE POND BECOMES A FLAT REACH OF THE RIVER (grill в23, §3.1 amendment ratified by design with amendments a-c). Monotone pass no longer descends through a pond: station-in-pond takes the pond's level; Pond::level = min(spill saddle, river entry level), settled to fixpoint (a trace can re-enter a pond it left); cells above the settled level DRAIN. §3.2 extension (design amendment c): the LAKE obeys the same entry rule — hydro.lake.surface_height = min(LAKE_LEVEL_TESTBED, entry); WorldgenWater reads the settled level, never the constant. Pond::spill_level (ADDITIVE field) records what the old construction drew — the Rule 30 control with the real rejected instance: measured 6 ponds clamped on seed-1 4x4/8x8, worst 8.44 m; drawn==swum gap now 0 m at all 34 in-pond stations. Invariant restated (flat-reach form): never gains height downstream, CONSTANT across any standing body, drawn == swum by construction, over-level pond UNCONSTRUCTIBLE (ok-backstop in build + suite tests with vacuity guard). All other water invariants re-verified green: C3 ford depth, WaterBed coverage, planes-never-flood-scatter, water-surface uniqueness, monotonic.
- 10:08:2026 - 02:39:21: TECH-DEBT WAVE tasks 2-4 + sim request. TUNNEL (task 2a, worldgen cause found): TWO defects — (1) the §2.8 massif re-buried the surveyed exit (terrain 67.9-74.3 m over the 61.9 m end floor; tunnel dead-ended), fixed by CarveCorridor::daylight_portals + open_daylight_portals() in build_world_context (endpoints DERIVED to open air along their own leg; barrow passage flagged false — it ends in its chamber on purpose); (2) legs 4->5 and 6->7 ran laterally OVERLAPPED (centers 3.3 m, boxes 4 m — a 0.7 m strip where the ramp floor is silently the lower corridor's floor; ASCENT_SCALE stretched the hidden drop 1.7 -> 3.7 m and the acceptance walker fell through and was trapped, measured at (803.7, 241.4)); wp[6] moved to hold the final leg >= 6 m off leg 4->5's line (2 x half_width + 2 m rock). Instrumented physics probe: walk completes 8/8 grounded at tick 4251 — the remaining sim_tunnel_walk red is ONLY its 3600-tick budget (test-side one-liner, requested from the lead; sim zone unowned). BARROW XFAIL (task 2b, design ruling §7.0a): mouth split into its own should_fail case with trigger-expiry (the §2.8.2 couloir work, owner design) — room/burial assertions keep guarding; suite green. JSON READER (task 3): core/serialization Json.h/.cpp — strict RFC 8259 recursive-descent parser into a DOM (order-preserving object members, total accessors), line/column errors, no exceptions across the boundary, depth-capped (JSON_MAX_DEPTH), duplicate keys/trailing garbage/comments/NaN = documented ERRORS; canonical writer (charconv shortest round-trip, guarded *_l fallback for the Apple clang 15 CI toolchain); suite: 27 rejection controls + depth bomb with passing neighbor + round-trip fixpoint + determinism (140 assertions). BIRCH (task 4): scatter lattice derives from BIRCH_BANKLINE_SPACING_MIN/MAX (design ruling: bank-line accent = crown x 1.1-1.5, NOT TREE_SPACING_FOREST; old 8.0 was in-band by luck). ChunkManager::surface_class_at(vec2) ADDITIVE for sim's footstep sound — nearest sample of the drawn field (asymmetry with the analytic water_surface_at documented on the method); test in ChunkManagerTests.
-->
