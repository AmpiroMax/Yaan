...
- 13:08:2026 - 18:05:00: БЕЛАЯ ЛИНИЯ ЛЕСА — ЭТО ВОЗДУХ, и ни один член не применён к
  листве отдельно. Полный разбор и рецепт: docs/acceptance/render-treeline-white-is-the-air.md.
  При выключенном воздухе кадр flora с 400 м — ОБЫЧНЫЙ ТЁМНО-ЗЕЛЁНЫЙ ЛЕС, то есть
  «осветительного члена на дальности» нет вовсе. b/r (лист 0.54, небо 1.13):
  отгруженное 1.035 / без покрытия 1.006 / без полосы мглы 0.847 / без дымки
  0.549 / без всего 0.540. НА ГЛАЗУ ИГРОКА (1.7 м) дымка — ВЕСЬ дефект: 0.943 ->
  0.639, а полоса мглы двигает РОВНО НОЛЬ, потому что лежит на 54-86 м. Покрытие
  (alpha-to-coverage) — третье и малое: 0.943 -> 0.908; его настоящий эффект в
  РАСПРЕДЕЛЕНИИ (доля пикселей в середине линии лист->небо 6.6 % -> 16.0 % при
  СОХРАНЁННОМ среднем полосы), то есть запись 12.08 указала верный рычаг и
  ошиблась в его ВЕСЕ. НИЧЕГО ЛИСТВЕННОГО: при выключенном покрытии в обеих
  руках полоса мглы тянет к небу землю и полог ОДИНАКОВО (0.25 -> 0.47 и
  0.38 -> 0.72, тот же множитель 1.9); полог просто ПОКАЗЫВАЕТ ту же долю сильнее,
  будучи тёмной дырчатой массой на светлом небе. ПОЧЕМУ ОБА ПРЕЖНИХ ДИАГНОЗА
  ПРОМАХНУЛИСЬ: flora сравнивала ДАЛЬНЮЮ полосу с БЛИЖНЕЙ вместо контрольной
  руки, а g-b — нечувствительная величина (у листа +64, у неба -12); b/r видит
  дефект сразу. КОДА НЕ МЕНЯЛ: рычаги — HAZE_SCALE_LENGTH 600 м и
  MIST_BAND_* (70 м / 32 м / 4), обе строки NUMBERS и решение ведущего (правило
  14). Цифры для него: дымка кладёт 28 % неба в полог на 200 м и 37 % на 400 м;
  внутри полосы мглы воздух впятеро плотнее, то есть видно 120 м, и эта полоса
  уже съела два прогона (проба неба на 84 м и кадр flora на 70 м).
  ЛОВУШКА ПРИБОРА: DFN_HAZE — ДЛИНА, а не доза (по умолчанию 600), DFN_HAZE=0 —
  это БЕСКОНЕЧНАЯ дымка; выключается 1e8, и при этом ГАСИТ И ПОЛОСУ МГЛЫ, потому
  что оба члена делятся на один и тот же L.
- 13:08:2026 - 18:55:00: НЕОСВЕЩЁННАЯ СТОРОНА И ПОЛЗУЩАЯ ТЕНЬ — две правки, оба
  числа и оба контроля в docs/acceptance/render-unlit-side-and-shadow-jitter.md.
  (1) ЗАЛИВКА ПОЛУЧИЛА НАПРАВЛЕНИЕ. Замер ДО: разброс яркости поперёк ствола в
  тени кроны p90/p10 = 1.01x (21.8/21.8/22.1 на 228 пикселях, 16 оттенков) при
  3.26x у освещённой земли в том же кадре. Иначе и быть не могло: в
  dfn_surface_light каждый член привязан к солнцу, луне или лампе, поэтому днём
  теневое полупространство сводилось к `u_ambientColor * sky_vis` — константе БЕЗ
  НОРМАЛИ, и на экране оставалось альбедо, умноженное на число. Стало 1.16x, кадр
  целиком 3.75x -> 4.19x при удержанном среднем (92.34 -> 82.59... среднее ствола
  ВЫРОСЛО 22.18 -> 23.25). Форма членов: min(n.y,0) — теряют только НИЗЫ, и
  азимут солнца с убранной вертикалью — ровно 0 на горизонтальной земле.
  ДВЕ СОБСТВЕННЫЕ ОШИБКИ, ОБЕ ОТГРУЖЕНЫ И ОБЕ ПОЙМАНЫ ЗАМЕРОМ: (а) «нулевое
  среднее по СФЕРЕ нормалей» — кадр не сфера нормалей, он в основном земля, а
  земля смотрит вверх: среднее кадра 84.81 -> 87.27, земли 88.90 -> 98.18, то есть
  10 % осветления мира под видом сохранения; (б) деление на значение для нормали
  «вверх» это чинит честно, но тогда всё, что НЕ вверх, может только терять:
  ствол 22.18 -> 19.42, крона 68.21 -> 59.72 (на 12 % темнее) — физически
  защитимо и ОБРАТНО дефекту, потому что человек смотрит на деревья, которых
  «словно нет».
  (2) ПРИВЯЗКА НАПРАВЛЕНИЯ СВЕТА (SHADOW_DIR_SNAP_RAD 0.00182). Текселевая
  привязка была ПОЛОВИНОЙ починки: центр объёма округляется в СВЕТОВОМ
  пространстве, а оно поворачивается вместе с солнцем, и световая координата
  глаза — АБСОЛЮТНАЯ мировая позиция (~1050 м), поэтому один кадр солнца
  (1.8e-5 рад) сдвигает её на 19 мм, и floor() переходит границу раз в несколько
  кадров, двигая ВСЮ карту на тексель 0.156 м разом. Сдвиг сетки на кадр
  0.1720 текселя -> 0.0037, медиана ровно 0, событий «шаг >= полтекселя»
  11.5/с -> 0.1/с. Для масштаба: собственное движение солнца за кадр смещает тень
  20-метрового кастера на 0.36 мм — сетка ездила в СЕМЬДЕСЯТ ПЯТЬ раз больше.
  Квант ВЫВЕДЕН, а не подогнан: шаг при снапе равен кванту в УГЛЕ, то есть
  субтендирует его с любого расстояния, и полпикселя внутреннего кадра дают
  0.5*1.309/360 = 0.00182 рад; развёртка по 0.0005..0.005 согласна, а не выбирает.
  ПРИБОР: tools/measure_shadow_jitter.cpp, и он НЕ ПО КАДРАМ — дефект живёт МЕЖДУ
  кадрами ОДНОГО прогона, а все наши двери снимают один кадр за процесс. Свип на
  26 прогонов был построен и ВЫБРОШЕН, когда наконец запустился его контроль: два
  ОДИНАКОВЫХ прогона при одном положении солнца расходятся на 32.83 % маски тени
  (стриминг и LOD разные при каждом запуске), то есть вдесятеро больше эффекта.
  Контроль должен был идти первым, а не последним — то же семейство, что беговая
  рябь и мигание в подземелье.
  НЕ ТРОНУТО НАРОЧНО: «сильно большие квадратные блоки» — это SHADOW_TEXEL_M
  0.156 м, вопрос РАЗРЕШЕНИЯ с ценой кадра (ближний каскад: +22 %), а привязка
  бесплатна. Три дефекта в одной фразе, третий открыт.
  ЧУЖОЕ И СРОЧНОЕ: ствол — ПЯТИГРАННАЯ призма с плоскими нормалями
  (FloraBuild::tube_segment, «faces own vertices»), поэтому при DFN_FILL_SUN=1.0
  он даёт ровно два плато 21.1 и 28.2 со стыком. Никакой осветительный член не
  нарисует цилиндр на геометрии, в которой его нет; радиальные повершинные
  нормали стоят НОЛЬ треугольников. Передано flora.
- 13:08:2026 - 20:23:22: НЕБО ЦЕЛИКОМ: кучевые с кольца на плиту, звёзды, лунный свет,
  толщина среднего яруса. Полная приёмка: docs/acceptance/render-sky-cumulus-stars-moonlight.md.
  ДВА ПЕРЕДАННЫХ МНЕ ДИАГНОЗА ОПРОВЕРГНУТЫ КОНТРОЛЕМ, и это ценнее правок.
  (1) «Метрика LOD облаков дёргает небо при ходьбе» — заморозка cells_px даёт
  отличие РОВНО НОЛЬ (27.65 % против 27.65 %, максимум совпадает до 0.01/255):
  метрика есть экранная производная и инвариантна к переносу ПО ПОСТРОЕНИЮ, то
  есть догадка не могла быть верной. Сама болтанка — честный подпиксельный
  параллакс плоскости на 1.6 км: среднее 0.69 из 255, палитровый шаг пересекают
  0.07 % пикселей на 0.1 м ходьбы. Чинить нечего.
  (2) «Кольцо кучевых едет с игроком» — отклик кольца на перенос глаза ТАКОЙ ЖЕ,
  как у трёх плоскостей, и он КОРРЕКТЕН для объекта в 20 км. Дефект в ДАЛЬНОСТИ:
  каждый кучевой пиксель стоял на одной дальности (20.0/20.0/20.0 км по
  p10/медиане/p90), поэтому дифференциального параллакса не было НИГДЕ в кадре —
  «декорация», записанная геометрией. Плита между нижним и средним ярусом даёт
  3.2/5.6/21.2 км и поворот пеленга 5.45/3.07/0.81° на 300 м ходьбы; облако
  выросло с 2.8 % неба до 26.8 %. Горизонтный банк вернулся из ДЛИНЫ ПУТИ, а не
  из новых констант.
  ЗВЁЗДЫ: поле — функция только направления взгляда (ходьба двигает 0.00 %),
  двигает поворот, а диск был радиусом 0.55 пикселя — мельче шага дискретизации,
  отчего один кадр обзора 30°/с менял яркость ВСЕГО поля на −16.9 %. Радиус
  теперь из экранной производной, яркость делится на площадь (та же энергия,
  другая форма), 0.83 px СЧИТАН СО СВИПА по колену двух кривых.
  ЛУННЫЙ СВЕТ: полная луна была на 0.49 палитрового шага ярче безлунной ночи, а
  освещённая земля занимала 0.34 шага от p10 до p90 — ОДНА запись палитры, у
  земли не было формы. MOON_GROUND_GAIN 1.234 — решённое требование на два шага
  разделения; линейность проверена ПРЕДСКАЗАНИЕМ (37.8 предсказано, 37.79
  замерено). Рука дозы 0 возвращает кадр, побитово равный новолунию.
  ТОЛЩИНА СРЕДНЕГО ЯРУСА (правило 52): плита 300 м = полклетки поля, три
  среза, пропускание (1−a)^n, где n — число КЛЕТОК, пересечённых хордой. Небо
  сквозь ярус у горизонта 5.5 % → 0.7 % (ярус ЗАКРЫВАЕТСЯ вдоль слоя, чего
  плоскость не умеет ни при какой непрозрачности). Рука DFN_DECK_THICK=0
  возвращает прежний лист с точностью 0.002 % пикселей и 1/255 — и она же
  поймала первую форму пути, в которой ТОЛЩИНЫ НЕ БЫЛО ВОВСЕ (доза, не входящая
  в формулу, — не доза).
  ТРИ СТРОКИ РЕЕСТРА заведены и утверждены: MOON_GROUND_GAIN, CLOUD_CEILING_MIN_M,
  CLOUD_CEILING_MAX_M. При них — замер РЕАЛЬНОГО разброса потолка: ПОГОДА даёт
  2.70x видимого размера ячейки (1270 м ясно → 470 м сплошная облачность), а
  МЕСТО только 1.17x, НИЖЕ собственного порога 1.30x, потому что длина волны
  места — две ширины мира. Строка описывает возможность, поведение даёт пока
  только погода; лечится длиной волны, решение ведущего.
  ЧУЖОЕ, ОТВЕЧЕНО: ScatterBatcher.cpp:197 передаёт FloraLod::Full не «зашито», а
  потому что строка выполняется ОДИН РАЗ на чанк, из ChunkLoaded. Выбор уровня
  по дальности в этой точке заморозил бы дерево на той дальности, на которой
