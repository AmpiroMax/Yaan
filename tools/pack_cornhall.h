...
/// Шаг ступени движка (HouseStairs.cpp, HOUSE_STAIR_RISE_M). ЗДЕСЬ КОПИЯ
/// ЧИСЛА, И ЭТО НАЗВАНО ВСЛУХ: константа живёт в .cpp зоны world и другой
/// единице трансляции недоступна, а рецепту она нужна не чтобы ДЕЛИТЬ марш
/// (делит движок), а чтобы ПРЕДСКАЗАТЬ, на сколько ступеней движок его
/// разделит, и назначить ход под нужную проступь. Расхождение копии с
/// оригиналом ловится приёмкой: проступь = ход / steps печатается замером.
inline constexpr float CORNHALL_STAIR_RISE = 0.175f;

/// Проступь пакета (заказ владельца: не меньше 0.28).
inline constexpr float CORNHALL_TREAD = 0.29f;
...
/// ХОД МАРША ПОД ЗАДАННЫЙ ПОДЪЁМ: столько, сколько просит проступь при том
/// числе ступеней, на которое марш разделит движок.
inline float cornhall_run(float rise) {
    const int steps =
        std::max(1, static_cast<int>(std::lround(rise / CORNHALL_STAIR_RISE)));
    return static_cast<float>(steps) * CORNHALL_TREAD;
}

/// ДЛИНА ПРОЁМА НАД МАРШЕМ — HOUSES.md §9.2, дословно:
///     L = (H + thick)/t + R*t / (sqrt(1 + t^2) + 1)
/// H — рост капсулы, R — её радиус, thick — толщина настила, t — тангенс
/// уклона. Второй член — касательная полусферы-макушки к углу плиты; без
/// него проём выходит короче на 4.5 см, а голове не хватает 10.
/// Запас 0.10 сверху: проём обязан ПОКРЫТЬ выведенный отрезок, а не совпасть
/// с ним (там же, про округление ВВЕРХ).
inline float cornhall_well_len(float rise, float run, float th) {
    const float t = rise / std::max(run, 0.01f);
    return (1.8f + th) / t + 0.35f * t / (std::sqrt(1.0f + t * t) + 1.0f) + 0.10f;
}
...
/// ХОДИМЫЙ МАРШ С ТЕТИВОЙ И ПЕРИЛАМИ СО СТОРОНЫ КОМНАТЫ.
/// xa..xb — ширина марша; rail_x — та его грань, что смотрит В КОМНАТУ
/// (у стены перила не нужны, там стена). z_foot -> z_top — направление хода.
///
/// ПОРЯДОК ВЕРШИН — ТОТ ЖЕ, ЧТО У ВСЕХ МАРШЕЙ ПОЛКИ: нижняя пара, затем
/// верхняя. build_stairs_contour читает по нему низ, верх и полуширину.
inline void cornhall_flight(Forge& f, float xa, float xb, float z_foot, float z_top,
    ...
    (void)f.contour({f.v(xa, y_lo, z_foot), f.v(xb, y_lo, z_foot),
                     f.v(xb, y_top, z_top), f.v(xa, y_top, z_top)},
    ...
    // висящих порознь.
    (void)f.beam(f.v(rail_x, y_lo + 0.12f, z_foot), f.v(rail_x, y_top + 0.12f, z_top),
    ...
    for (const float s : {0.0f, 0.5f, 1.0f}) {
        const float pz = z_foot + (z_top - z_foot) * s;
        const float py = y_lo + (y_top - y_lo) * s;
        (void)f.beam(f.v(rail_x, py, pz), f.v(rail_x, py + 1.0f, pz),
                     {{"radius", "0.055"}, {"form", "square"}, {"mat", "1"},
                      {"tone", "1"}, {"wear", wear}});
    }
    (void)f.beam(f.v(rail_x, y_lo + 0.95f, z_foot), f.v(rail_x, y_top + 0.95f, z_top),
...
inline void cornhall_dwelling(Aging age, float W, float D, int floors, int steps,
    ...
    for (int s = 1; s < floors; ++s) {
        ...
        lvl = cornhall_storey(f, 0.0f, 0.0f, W, D, lvl, y, 0.12f, D - 0.45f,
...
inline void forge_cornhall_jetty(const char* file) {
    ...
    float lvl = PL + 0.12f;
    lvl = cornhall_storey(f, 0.0f, 0.0f, W, D2, lvl, Y2, 0.14f, D - 0.45f,
                          /*west=*/false, "1", "1", "0.34");
    ...
    lvl = cornhall_storey(f, 0.0f, 0.0f, W, D3, lvl, Y3, 0.14f, D2 - 0.45f,
                          /*west=*/true, "1", "1", "0.3");
...
inline void forge_cornhall_arcade(const char* file) {
    ...
    cornhall_storey_dogleg(f, 0.0f, 0.0f, W, DG, 0.14f, Y2, 0.16f, D - 0.5f,
...
inline void forge_cornhall_brewery(const char* file) {
    ...
    cornhall_storey_dogleg(f, 0.0f, 0.0f, W, D, 0.16f, Y2, 0.16f, D - 0.5f,
...
inline void forge_cornhall_inn(const char* file) {
    ...
    for (const auto& t : {std::pair<float, float>{Y2, H2},
        ...
        lvl = cornhall_storey(f, 0.0f, 0.0f, GX0, D, lvl, y, 0.16f, D - 0.5f,
