...
- 27:08:2026 - 23:30:00: ТЕЛО cornhall_flight_ax ПЕРЕЕХАЛО В ОБЩУЮ РУКУ house_flight_ax
  (tools/forge_houses.cpp): пока правил было две, житновская правка перил
  Вайтрана не касалась, а крит владельца общий. Здесь остался перевод
  соглашения о стороне. ДВОРОВЫЙ МАРШ ПОСТОЯЛОГО ДВОРА ПОЧИНЕН, и починило его
  не усилие, а свойство rise, которого не было в день записи 24.08 о его
  нечинимости: 47.4 гр. / 28 ступеней / проступь 0.161 -> 45.00 / 18 / 0.276,
  ценой 0.47 м двора вместо посчитанных тогда 3.6 (полный разбор — у самого
  марша). Амбарному трапу и хвостовой лестнице ветряка добавлен nosing=1:
  уклон и открытый вид у них заказаны, а фантомная ступенька 0.155 у порога —
  нет.
...
- 28:08:2026 - 11:51:19: ВХОД КАЛАНЧИ: СТВОЛ В УГОЛ, ГОРОДСКАЯ СТВОРКА — В ВОРОТА (волна
  хвостов). Судья локаций держал одну известную находку на весь проект —
  «точка входа int/cornhall/x280z306 непроходима», — и она прятала ещё семь.
  Причин было две. (1) Городской дверью тела была дверь СТВОЛА, поэтому
  точка входа локации ложилась в 0.8 м за порогом, ВНУТРИ лестничной клетки
  и в 0.30 м от второго марша (капсула 0.35). (2) Ствол стоял посреди депо,
  и от комнаты оставалось кольцо 1.03/0.73 м при капсуле 0.70 — непроходимое
  по устройству, отчего всё убранство депо было недостижимо. Ствол ушёл в
  северо-восточный угол (комната 2.07 x 3.10 плюс проезд 6.00 x 1.50), а
  створка повешена в середине воротного проёма: второй разрыв наземной
  кладки завести нельзя — контур пятна стен в манифесте замыкается только
  при ОДНОМ разрыве, и калитка в простенке уносила wpoly в контур ствола.
  Пятно депо не изменилось. Замер: 8 предметов, 0 находок судьи.
*/
...
///
/// ТЕЛО РУКИ ПЕРЕЕХАЛО В ОБЩУЮ (28.08, house_flight_ax в forge_houses.cpp).
/// Довод — тот же «правило 39», которым эта рука и заведена, только этажом
/// выше: у Вайтрана марш выписывался ГОЛЫМ контуром пять раз, перил не имел
/// вовсе, и аудит 28.08 снял с обоих городов один и тот же кадр «пожарный
/// трап». Пока правил две, житновская правка перил Вайтрана не касается —
/// а крит владельца общий. Здесь остался ТОЛЬКО перевод соглашения о стороне
/// (bool rail_plus -> int rail_side): его знает вызывающий пакета.
inline void cornhall_flight_ax(Forge& f, float fx, float fz, float tx, float tz,
    ...
    house_flight_ax(f, fx, fz, tx, tz, half, sx, sz, y_lo, y_top,
                    rail_plus ? +1 : -1, th, wear);
}
...
inline void forge_cornhall_windmill(const char* file) {
    ...
    (void)f.contour({f.v(DOOR_X - 0.75f, 0.0f, BZ1 + 4.6f),
                     ...
                     f.v(DOOR_X - 0.75f, BODY_Y, BZ1 + 0.14f)},
                    // nosing=1 (28.08). Уклон 33.9 гр. остаётся: это ХВОСТОВАЯ
                    // лестница козловой мельницы, она же рычаг разворота, и её
                    // длина — конструкция, а не небрежность. А ходимая
                    // поверхность при nosing=0 шла по задним углам ступеней и
                    // приходила на 0.155 выше и земли, и порога — ступенька,
                    // через которую перепрыгивают, ровно по криту владельца.
                    {{"thickness", "0.12"}, {"fill", "6"}, {"open", "1"},
                     {"nosing", "1"}, {"mat", "1"}, {"tone", "2"}, {"wear", "0.55"}});
...
inline void forge_cornhall_inn(const char* file) {
    ...
    // Настил, стойки до него и перила: ярус, с которого попадают в комнаты.
    {
        ...
        //
        // НАХОДКА, ЗАПИСАННАЯ 24.08 КАК НЕЧИНИМАЯ, ПОЧИНЕНА 28.08 — И ПОЧИНИЛО
        // ЕЁ НЕ УСИЛИЕ, А СВОЙСТВО, КОТОРОГО В ДЕНЬ ЗАПИСИ НЕ БЫЛО. Текст той
        // записи сохраняю целиком, потому что он и есть довод:
        //
        //   «Этот марш НЕ ХОДИМЫЙ: подъём 4.90 на ход 4.50 — уклон 47 гр.,
        //    движок делит его на 28 ступеней и проступь выходит 0.161 при
        //    норме 0.28. Чтобы стал ходимым, нужен ход 28 * 0.29 = 8.12 м, то
        //    есть марш уходит во двор на 8 м вместо нынешних 4.5 — А ЭТО
        //    ДВИГАЕТ ПЯТНО ТЕЛА (wz0 с -5.9 на -9.5) и пересчитывает посадку
        //    постоялого двора в чертеже города.»
        //
        // ГДЕ В ЭТОМ СЧЁТЕ ОШИБКА: «28 ступеней» — не свойство марша, а
        // следствие того, что подступёнок тогда был ОДИН НА ВСЕХ (0.175), и
        // число ступеней получалось делением 4.90 на него. С 27.08 подступёнок
        // — свойство марша (rise, HouseMesh.h): при 0.28 ступеней 18, и ход
        // под 45 гр. равен подъёму — 4.97, а не 8.12. Двор растёт на 0.47 м,
        // а не на 3.6, и пятно тела с ним. Считать надо было не «сколько
        // ступеней при нынешнем делении», а «какое деление я вправе заказать».
        //
        // ЗАМЕР ДО/ПОСЛЕ: 47.44 гр. -> 45.00; проступь 0.161 -> 0.276;
        // ступеней 28 -> 18. Перила по ОБЕИМ граням: марш стоит во дворе
        // свободно, стены рядом нет ни с одной стороны.
        //
        // ВЕРХ САДИТСЯ НА ВЕРХ НАСТИЛА (Y2 + 0.07), А НЕ НА ЕГО ПЛОСКОСТЬ:
        // плита растёт в обе стороны от своей плоскости, и прежний якорь на Y2
        // оставлял ступеньку в полтолщины настила ровно там, где с марша
        // сходят на галерею.
        // РАЗРЫВ ПЕРИЛ ГАЛЕРЕИ ВЫШЕ (x 0.6..2.2) ОСТАЁТСЯ ВЕРНЫМ: ширина марша
        // не менялась, менялся только его вынос во двор.
        house_flight_z(f, 0.6f, 2.2f, GZ + 0.1f - (Y2 + 0.07f), GZ + 0.1f, 0.0f,
                       Y2 + 0.07f, /*rail_x=*/1.4f, "0.45");
...
// каланчу не надо.
//
// ПОПРАВКА 28.08 К ДВУМ ЧИСЛАМ ВЫШЕ (волна хвостов): 1.23 и 0.95 меряны до
// ОСЕЙ стен депо, а ходок упирается в ГРАНИ — настоящие полосы были 1.03 и
// 0.73 при капсуле 0.70. Кольцо было непроходимо, и ствол переехал в угол;
// разбор и замер — у derivation TX0/TZ0 ниже. Пятно депо и здесь не тронуто.
// ---------------------------------------------------------------------------
inline void forge_cornhall_firetower(const char* file) {
    ...
    // КАЛИТКА В ПОЛОТНЕ ВОРОТ, А НЕ РЯДОМ С НИМИ, И ЭТО ВЫНУЖДЕННО-ВЕРНОЕ
    // РЕШЕНИЕ. Городская дверь тела обязана быть створкой (door=1), и она
    // обязана стоять в ПРОЁМЕ. Второй проём в наземной кладке депо завести
    // нельзя: манифест собирает контур пятна стен (house_manifest.h,
    // wall_rings) замыканием цепи, и цепь замыкается ТОЛЬКО при двух
    // свободных концах — то есть при ОДНОМ разрыве кольца. Разрыв уже занят
    // воротами; калитка в простенке дала бы четыре конца, кольцо развалилось
    // бы, и в манифест уехал бы контур одного ствола вместо контура депо —
    // город положил бы фундамент под каланчу, а не под депо (проверено: wpoly
    // после такой правки стал «2.491/0.425,5.975/0.425,...»).
    // Плюс ось двери манифест берёт по СМЕЩЕНИЮ от центра габарита: створка в
    // западном простенке (0.75) уезжала от центра по X на 2.45 при 2.40 по Z,
    // и тело объявляло себя дверью НА ЗАПАД. В середине ворот смещение по X
    // ровно ноль, и ось выходит +Z — та, что и есть у фасада.
    // В натуре это самая обычная вещь: у пожарного депо в полотне ворот
    // прорезана дверь для человека.
    const float WICKET_X = 3.2f;   // ось калитки = ось ворот
    ...
    // ---------- СТВОЛ ПРИЖАТ К СЕВЕРНОЙ И ВОСТОЧНОЙ СТЕНАМ ДЕПО -----------
    // ЭТО ПОЧИНКА, А НЕ ВКУС, И ВОТ ЗАМЕР (28.08, волна хвостов). Ствол стоял
    // ПОСРЕДИ депо, и от комнаты оставалось КОЛЬЦО: полосы 1.03 м с запада и
    // востока и 0.73 м с севера и юга (числа сняты с карты проходимости
    // dfn_interior_check --map, а не выписаны из шапки: прежняя шапка звала их
    // 1.23 и 0.95, потому что мерила до ОСЕЙ стен депо, а ходок упирается в
    // ГРАНИ). Капсула игрока 0.70 в поперечнике; в полосе 0.73 полоса
    // допустимых ЦЕНТРОВ равна 0.03 м, то есть кольцо было непроходимо, а с
    // бочками в полосах — непроходимо дважды. Судья локаций печатал одну
    // находку «точка входа непроходима», и под ней пряталось СЕМЬ: всё
    // убранство депо было недостижимо.
    // КОЛЬЦО ШИРИНОЙ С КАПСУЛУ — ЭТО НЕ КОМНАТА, И ШИРЕ ЕГО НЕ СДЕЛАТЬ, не
    // трогая пятно: ствол 3.93 x 3.10 в депо 6.00 x 4.60 (в чистоте) забирает
    // почти всё. Значит остаток надо не размазывать по четырём сторонам, а
    // СОБРАТЬ В ОДНО МЕСТО: ствол уходит в северо-восточный угол, и вместо
    // кольца получается Г — комната 2.07 x 3.10 у западной стены и проезд
    // 6.00 x 1.50 вдоль южной, от ворот к двери ствола. Это ещё и правдивее:
    // у пожарного депо каланча стоит в углу, а не посреди проезда.
    // ПЯТНО ДЕПО ПРИ ЭТОМ НЕ ИЗМЕНИЛОСЬ НИ НА САНТИМЕТР — условие прежней
    // волны соблюдено, городу перекладывать каланчу не надо.
    const float BWALL = 0.4f;                    // стена депо
    const float SX1 = BW - BWALL * 0.5f;         // наружная грань ствола = грань депо
    const float TX1 = SX1 - SWALL * 0.5f;        // оси стен ствола
    const float TX0 = TX1 - (IN_X + SWALL);
    const float TZ0 = BWALL * 0.5f + SWALL * 0.5f;
    ...
    // ...И ЭТОТ ЖЕ РАЗРЫВ СЛУЖИТ КАЛИТКЕ (28.08): городская створка тела
    // повешена в середине воротного проёма, где цоколя уже нет. Порог 0.45 м
    // перед створкой был бы теми же запертыми воротами, только уже.
    ...
    parquet_floor(f, 0.0f, 0.0f, BW, BD, FLOOR_Y);
    {
        ...
        //
        // КАЛИТКА В ПОЛОТНЕ ВОРОТ — ЭТО ГОРОДСКАЯ ДВЕРЬ КАЛАНЧИ, И ОНА
        // ЗАВЕДЕНА ВМЕСТО ДВЕРИ СТВОЛА (28.08, волна хвостов). Прежде
        // единственная створка тела стояла в южной грани СТВОЛА, и город
        // читал её как вход: болванка запечатывалась по ней, а точка входа
        // локации ложилась в 0.8 м за порогом — то есть ВНУТРИ ЛЕСТНИЧНОЙ
        // КЛЕТКИ, в 0.30 м от второго марша. Судья локаций мерил там капсулу
        // (радиус 0.35) и объявлял саму точку входа непроходимой; на этой
        // находке разбор сцены обрывался, и под ней пряталось ещё семь —
        // всё убранство депо, до которого из клетки не дойти.
        // ВХОДЯТ В ДЕПО, А НЕ В КЛЕТКУ: калитка ведёт в проезд, из проезда
        // открыт и западный отсек, и дверь ствола, и марш. Дверь ствола
        // осталась проёмом (doors=1 без створки): она внутренняя, и вторая
        // створка сделала бы у тела два городских входа.
    ...
    f.door_leaf(WICKET_X, 0.0f, BD);   // городская дверь — калитка депо, не ствол
...
inline void forge_cornhall_granary(const char* file) {
    ...
    (void)f.contour({f.v(W * 0.5f - 0.7f, 0.0f, D + 2.0f),
                     ...
                     f.v(W * 0.5f - 0.7f, FLOOR, D + 0.13f)},
                    // nosing=1 (28.08): та же фантомная ступенька 0.155 у
                    // порога, что у крылец мельницы и усадьбы. Уклон 28.6 гр.
                    // и открытый вид остаются — «настоящий амбарный трап»
                    // назван видом выше, и норма внутридомового марша к нему
                    // не относится.
                    {{"thickness", "0.1"}, {"fill", "6"}, {"open", "1"},
                     {"nosing", "1"}, {"mat", "1"}, {"tone", "1"}, {"wear", "0.55"}});
    f.save(file);
}
