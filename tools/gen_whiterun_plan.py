...
# Last updated: 23:08:2026 - 19:08:08
# Module: tools
...
# - 23:08:2026 - 13:20:00: ДВА ОРИЕНТИРА СОШЛИ С ЧУЖОГО МЕСТА (претензии
#   [N25] и [24] круга 4). Оба ставятся ПРЯМО ИЗ ЧЕРТЕЖА, мимо разведения
#   тел — потому только они и могут встать посреди улицы или внутри дома,
#   и никакая проверка расстановки их не ловит (её список — PLACED).
#   (1) ГИЛДЕРГРИН (108,118) -> (115.5,118). Дерево стояло в МЕТРЕ от осевой
#   главной улицы: ствол-коллайдер 107.1..108.9 по x при осевой x=107.0
#   (перепись тел Jolt в прогоне), кольцо радиусом 2.6 со стенкой 0.5 м
#   лежало поперёк проезжей части. Критик читал это как «террасный уступ,
#   откос 66 гр.» — уступа нет, профиль по осевой даёт 0.12 м/м, а 66 гр.
#   это СТЕНА ДОМА (98.5,126.6), в 0.15 м от которой бот и стоял, свернув с
#   улицы у дерева. Новое место выбрано перебором полуметровой сеткой:
#   центр не ближе 4.2 м к осевым и 3.3 м к телам; ближайшая такая точка —
#   7.5 м на восток, у начала улицы Ветров.
#   (2) КОЛОДЕЦ (110.5,153) -> (120,151.5). Прошлый заход увёл его с осевой,
#   но привёл в тело лавки (107,156.5): та отодвигается от дороги 6 (грунт
#   через её схемное пятно) и заходит на плиту рынка на 5.9х6.8 м, накрывая
#   колодец с трёх сторон. На плите (109..123 x 146..157) есть РОВНО ОДНО
#   место, где тело колодца целиком на камне и не в полосе дороги, — угол
#   (120,151.5). Заодно: чертёж РИСОВАЛ колодец в (116,151.5) и после
#   прошлого переноса, то есть картинка и город разошлись; теперь у обоих
#   одна константа.
# - 24:08:2026 - 00:10:00: ТРИ ДОМА СОШЛИ С ЧУЖИХ МЕСТ РАДИ ЗАКАЗА ВЛАДЕЛЬЦА
#   22.08 («окультурить пространство вокруг дерева» и «оживить рынок»). Все
#   три переноса куплены ЗАМЕРОМ, а не вкусом, и ни один не трогает дерево,
#   колодец или улицы — двигается только рядовая застройка.
#   (1) ДОМ (113.5,124.5) -> (112,131). Стоял в 3.5 м к югу от ствола
#   Гилдергрина. Кольцевая скамья кузни-3 (сиденье 3.52..4.18 от центра)
#   садилась ОДНИМ сегментом из восьми: три отказывала проба по телу этого
#   дома, четыре — полосы главной улицы и улицы Ветров. Дерево между двумя
#   улицами и домом стоит в проходе, а не на площадке, и точки притяжения из
#   такого места не делает никакая расстановка. После переноса сегментов 4 из
#   8 — половина кольца, открытая на обе улицы; остальные четыре не встают по
#   ПОЛОСАМ УЛИЦ, и это уже честная композиция, а не теснота.
#   ПЕРЕНОСИТСЯ ДОМ, А НЕ ДЕРЕВО: место Гилдергрина выбрано прошлой волной
#   пробой по двум требованиям ([N25], бот застревал) и держит гайд §11 —
#   улица Ветров упирается в дерево. Сдвиг дерева отменил бы и то и другое.
#   (2) ЛАВКА (107,156.5) -> (104,161). Отодвиг от грунтовой дороги уносил её
#   телом 6.8x9.0 в центр (110.4,153.4), то есть на прямоугольник рынка:
#   карта свободы площади (прибор этой волны, сетка 1 м) показывала ровно две
#   свободные полосы — северо-западный угол и восточную кромку у колодца.
#   Отсюда «рынок: 0 предметов оживления» на первом прогоне: оживлять было
#   НЕГДЕ, а не нечем. Стало 15 предметов и 3 куста.
#   (3) ЛАВКА (107,147.5) -> (103.5,146.5). Заходила на прямоугольник рынка на
#   1.5 м по x, а пад под домом ВНУТРИ рынка не режется (площадь говорит
#   последней — иначе под ней вырастала ступень 1.18 м): заходящая полоса
#   оставалась на отметке площади, и земля протыкала пол на 0.23 м.
#   check_floors: 25 из 26 -> 26 из 26.
# - 23:08:2026 - 19:08:08: ТРИ РАССТАНОВОЧНЫХ НЕДОЧЁТА ВЛАДЕЛЬЦА (23.08), ЧАСТЬ
#   ЧЕРТЕЖА (вторая часть — в tools/gen_whiterun.py).
#   (1) «ГЛАВНЫЙ ЗАМОК СКВОЗЬ СТЕНУ ГОРОДА ПРОХОДИТ» — и это была правда
#   чертежа, а не расстановки: северная грань кольца шла (110,60)->(90,64),
#   то есть ПО ПЯТНУ ЗАМКА (стены 25.0x13.2 от (87.5,60) до (112.5,73.2)).
#   Прибор новой приёмки check_ring_bodies: взаимопроникновение 27.87 м оси
#   кладки, глубина 4.16 м. Грань переложена на КРОМКУ ЗАМКОВОГО ПЛАТО —
#   (128,74)->(118,59)->(86,57)->(74,84): профиль земли по новой линии
#   35.9..35.6 (ровное плато) против прежнего 35.9, то есть стена встала на
#   бровку обрыва, а замок остался внутри с честным зазором. Башня вершины
#   (90,64) переехала в (86,57) вместе с гранью.
#   (2) ДОНЖОН СТОЯЛ ЗА СТЕНОЙ НА ОБРЫВЕ. Его схемное пятно (112,60)
#   налезало на замок, и разведение тел выносило его на северо-восток за
#   кромку плато — фундамент 9.8 м, тело в 0.7 м от новой грани кольца.
#   Перенесён во фланг парадного двора: (115,70), дверь на запад.
#   (3) УЛИЦА ВЕТРОВ МОСТИЛА ПОДНОЖИЕ ГИЛДЕРГРИНА. Отрезок
#   (107,118)->(116,113) проходил в 4.23 м от ствола, а полотно шириной 4 м
#   рисует камень до 2.23 м — то есть улица и была той «каменной плиткой»,
#   в которой стоит дуб. Излом разнесён на два узла (111,113),(117,112.5):
#   до ствола 5.36 м, при этом осевая у фасадного ряда x=117 осталась на
#   z=112.5 — там между улицей и просёлком Гребня ровно один дом ширины, и
#   сдвиг улицы на метр выдавливал его на просёлок (проверено прогоном).
import math
...
- 23:08:2026 - 13:20:00: Гилдергрин (108,118) -> (115.5,118): ствол стоял в
  метре от осевой главной улицы, кольцо лежало поперёк проезжей части.
  Колодец (110.5,153) -> (120,151.5): на западной половине плиты его накрыло
  тело лавки, отодвинутой от грунтовой дороги.
- 24:08:2026 - 00:10:00: три дома сошли с чужих мест ради сквера Гилдергрина и
  рынка: дом (113.5,124.5)->(112,131) стоял в 3.5 м от ствола и отказывал
  кольцевую скамью, лавка (107,156.5)->(104,161) телом накрывала площадь
  («оживлять негде»), лавка (107,147.5)->(103.5,146.5) заходила на площадь и
  протыкала свой пол на 0.23 м.
- 23:08:2026 - 19:08:08: три расстановочных недочёта владельца. Северная грань кольца
  (110,60)->(90,64) шла ПО ПЯТНУ ЗАМКА (стены от (87.5,60) до (112.5,73.2)) —
  переложена на кромку замкового плато (128,74)->(118,59)->(86,57)->(74,84),
  башня вершины (90,64)->(86,57). Донжон (112,60)->(115,70): прежнее пятно
  налезало на замок, и разведение тел выносило его за стену на обрыв.
  Улица Ветров (107,118)->(116,113) проходила в 4.23 м от ствола Гилдергрина
  и мостила его подножие — излом разнесён на (111,113),(117,112.5), при этом
  осевая у фасадного ряда x=117 осталась на z=112.5.
-->
