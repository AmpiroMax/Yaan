...
# Last updated: 22:08:2026 - 23:55:00
# Module: tools
...
#
# UPD:
# - 22:08:2026 - 01:10:00: Создан из итеративной сессии со схемой (v4.0-v4.6):
#   река одним рукавом рвом вдоль стены, гора-исток, мост на воде, дома
#   параллельны улицам, дорожки с бордюрами, дуб-поляна, огороды, нормативы
#   ширин, 8 башен вместо 14, постоялый двор за мостом.
# - 22:08:2026 - 01:35:00: Экспорт docs/WHITERUN_PLAN.json (единая правда для
#   генератора игры) и заголовок-контракт в html-выводе.
# - 22:08:2026 - 02:00:00: Сведение с игрой: кромки зон 28-32 сняты с рынка
#   (полигон Т2 краем накрывал площадь — высота рынка выходила 29.4 вместо
#   27), дорога Восточных ворот входит в площадь севернее таверны, лавки
#   раздвинуты под реальную глубину, тропа к дубу шириной 2 м, тесный дом
#   юго-востока рынка снят.
# - 22:08:2026 - 23:55:00: КОЛОДЕЦ УБРАН С ОСЕВОЙ ГЛАВНОЙ УЛИЦЫ (претензия
#   [N8] критика, тяжесть 2 — единственный жёсткий стоп бота за прогон).
#   Колодец стоял в ЦЕНТРЕ прямоугольника рынка (116, 151.5), а главная улица
#   проходит ЧЕРЕЗ рынок, и её узел (116,152) лежал ВНУТРИ тела колодца:
#   замер city-well.dfh даёт пятно 2.80x2.74 от угла (-0.55,-0.52), то есть
#   115.45..118.25 по x и 150.98..153.72 по z. Бот упирался в северную грань
#   и стоял: (116.17, 153.67), скорость 0.03-0.18 м/с, вотчдог снял прогон;
#   узел маршрута был недостижим в принципе, а не «проход между прилавками
#   узок». Колодец сдвинут на западную половину площади: (110.5, 153) — до
#   ближайшей осевой 4.92 м (было 0.5), до ближайшего прилавка 4.43 м, тело
#   целиком на плите рынка (109..123 x 146..157). Восточная половина
#   оставлена свободной намеренно: в неё входит тракт Восточных ворот.
#   Тем же заходом РАЗВЕДЁН ТОРГОВЫЙ РЯД: осевая пересекает линию прилавков
#   (z=149.2) в x=114.8, а прилавок 2 стоял центром в 116.5 при полуширине
#   1.35 — проход 0.35 м, капсуле не пройти. Ряд раздвинут по обе стороны
#   улицы: 111.3 на западе, 118 и 121.3 на востоке; коридор шириной 3.2 м
#   (+-1.6 от осевой) держится свободным, тела прилавков в него не входят.
import math
...
wall = [(80,168),(102,177),(122,174),(138,160),(148,142),(147,118),(140,94),(128,74),(110,60),(90,64),(74,84),(66,110),(65,138),(71,156)]
...
TOWERS = [(102,177),(138,160),(148,142),(147,118),(128,74),(90,64),(66,110),(71,156)]
for x, z in TOWERS:
...
stone(MAIN)
GRAVEL_Y = [(107,118),(116,113),(124,110),(130,107)]
...
house(112, 60, 4, 4, 0, "#7a7065", "донжон", "S", walk=(104,66), kind="donjon")
...
svg.append(f'<circle cx="{108*SC}" cy="{118*SC}" r="{7*SC}" fill="none" stroke="#b58f52" stroke-width="2" stroke-dasharray="4,4"/>')
svg.append(f'<circle cx="{108*SC}" cy="{118*SC}" r="{4.4*SC}" fill="#e8a7c4" stroke="#b06a8c" stroke-width="2"/>')
svg.append(f'<text x="{108*SC-32}" y="{118*SC+36}" font-size="11" font-weight="bold" fill="#8c4a68">ГИЛДЕРГРИН</text>')
...
house(113.5, 124.5, 7, 6, along(MAIN,109,124)+90, "#c8a06a", None, "W", walk=(109.8,124.5))
svg.append(f'<rect x="{109*SC}" y="{146*SC}" width="{14*SC}" height="{11*SC}" fill="#cfc5a8" stroke="#8f855f"/>')
svg.append(f'<circle cx="{116*SC}" cy="{151.5*SC}" r="4" fill="#4d698a"/><text x="{116*SC+6}" y="{151.5*SC+3}" font-size="9" fill="#33435a">колодец</text>')
...
house(107, 147.5, 6, 7, 0, "#c8a06a", "лавка", "E", walk=(111,148), kind="shop")
house(107, 156.5, 6, 7, 0, "#c8a06a", "лавка", "E", walk=(111,156), kind="shop")
...
house(127.5, 162.5, 6, 6, along(MAIN,118,163)+90, "#c8a06a", None, "W", walk=(118.7,162.5))
...
UPD:
- 21:08:2026 - 23:30:00: Создан (схема v4.0).
- 22:08:2026 - 01:35:00: v4.6 — правки пользователя и самопроверки: река одним
  рукавом рвом вдоль стены, гора-исток, мост на воде, дома параллельны улицам,
...
- 22:08:2026 - 23:55:00: колодец убран с осевой главной улицы (116,151.5) ->
  (110.5,153): узел улицы лежал внутри тела колодца, бот вставал намертво.
-->
...
open("docs/WHITERUN_PLAN.json", "w", encoding="utf-8").write(json.dumps({
    ...
    "roads": ROADS_X,
    "houses": HOUSES_FULL,
    "market": {"rect": [109,146,14,11], "well": [110.5,153], "stalls": [[111.3,149.2],[118,149.2],[121.3,149.2]]},
    "gildergreen": [108,118], "oak_glade": {"oak": [52,190], "trees": [[42,180],[60,176],[68,184],[66,199],[44,202],[36,188],[58,204]]},
    "gardens": [[92,74],[100,76],[120,72],[127,78],[112,68]],
    "trees": TREES_X,
