...
Last updated: 28:08:2026 - 01:35:48
Module: tools
...
- 28:08:2026 - 01:35:48: ТО, ЧТО ЭТОТ ПРИБОР СНИМАЕТ, ПЕРЕЕХАЛО ИЗ /tmp В
  ДЕРЕВО: tools/cities/<город>.natural, рядом с паспортом города (движок эту
  землю не читает ни разу, поэтому не в assets/). Причина — инцидент 28.08: macOS
  вычистила /tmp, оба дампа исчезли, и города выпустились БЕЗ ЕДИНОЙ строки
  дельт (60 278 у Вайтрана, 255 473 у Корнхолла). Замер, ради которого нужен
  собранный движок, не может жить там, где его стирает уборка системы;
  подробный довод и полный прогон — в шапке tools/gen_city.py. Заодно
  измерено, что прибор ВОСПРОИЗВОДИТСЯ: перезамер 28.08 сегодняшним деревом
  дал рельефы обоих городов побайтово теми же, что в HEAD.
*/
