...
- 22:08:2026 - 15:25:00: В шапку INDEX.md — строка о том, что паспортная полоса W/H
  §2.5 РЕЦЕПТУРНАЯ, а измеренная выше на свес лап (великан 1.56 при рецептурных
  0.87, Гилдергрин 1.85 при 1.30), и что выход за полосу осознанный. Без неё
  разницу двух метрик через месяц прочтут как расползание.
- 22:08:2026 - 15:45:00: ГИЛДЕРГРИН УРОНЕН ПОД ЗАМОК: 14.0/9.1 -> 11.4/7.28, измеренная
  высота 20.3 -> 16.7. Приёмка гайда §11 — высотой доминирует Драконий Предел,
  дерево формой и пустотой круга; при перепаде террас 3.48 м первая выпечка
  ставила верхушку на 52.9 против конька замка 50.5 и донжона 51.5. Стало 49.3:
  на 1.2 м ниже замка, на 2.2 м ниже донжона. Крона НЕ ужата пропорционально
  (заказ «ниже, но ШИРЕ»): Ø 31.3, ниже 26 пришлось бы двигать city-treering.
...
int main(int argc, char** argv) {
    ...
    }
    {
        ...
        //   W/H = 2*7.28/11.4 = 1.28  против 0.87 у великана;
        //   нижние сучья на 0.22*11.4 = 2.5 м — под ними ходят.
        //
        // ВЫСОТУ ДИКТУЕТ НЕ ДЕРЕВО, А ЗАМОК. Гайд §11: доминировать высотой
        // обязан Драконий Предел, дерево — формой и пустотой круга. Первая
        // выпечка (14.0 / 9.1) мерилась 20.3 м и с учётом перепада террас
        // 3.48 м ставила верхушку на 52.9 — ВЫШЕ конька замка (50.5) и донжона
        // (51.5). Терраса не спасала: чтобы уйти под замок, перепад должен
        // быть 6-7 м, а поднять площадку замка нельзя — под ней парадный марш
        // и подпор плато. Поэтому уронено до измеренных 16.7: верхушка 49.3,
        // на 1.2 м ниже замка и на 2.2 м ниже донжона.
        // Крону при этом НЕ ужимал пропорционально росту: заказ владельца —
        // «значительно ниже, но ШИРЕ», и Ø 31.3 держит круг площади (ниже 26
        // пришлось бы двигать кольцо city-treering).
        ...
        // число дало бы вдвое мелкие полотна. 0.09*(20.0/7.28) = 0.2473 — то же
    ...
    // ОСОЗНАННЫЙ ВЫХОД ЗА ПАСПОРТНУЮ ПОЛОСУ — СКАЗАН ВСЛУХ, а не оставлен на
    // догадку. docs/TREE_PASSPORTS.md §2.5 держит W/H в 1.1-1.3, но это полоса
    // РЕЦЕПТУРНАЯ (2*crown_radius / height), а мерится тут ГЕОМЕТРИЯ, у которой
    // лапы свисают за crown_radius. Измеренная метрика систематически выше
    // рецептурной: у великана 1.56 при рецептурных 0.87, у Гилдергрина 1.85 при
    // рецептурных 1.30. Без этой строки через месяц кто-то прочтёт разницу как
    // расползание.
    index += "# W/H в паспортах — РЕЦЕПТУРНОЕ (2*crown_radius/height); измеренное\n"
