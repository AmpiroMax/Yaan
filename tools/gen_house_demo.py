...
def stairwell(scene, sx, sz, group):
    ...
    d0 = run - hole_u * GRID         # начало проёма от подножия марша
    ...
    scene.place(deck, (sx + d0 - 0.5, deck_y, sz - 1.5), 0.0, group,
    ...
    far_x = sx + d0 - 0.5 + 3.75
    scene.place(joist, (far_x, axis_y - joist_r, sz - 1.0), yaw_for(0, 1), group)
    scene.place(joist, (sx + d0 - 0.75, axis_y - joist_r, sz - 1.0), yaw_for(0, 1),
                group)
    for x in (sx + d0 - 0.75, sx + run, far_x):
        for z in (sz - 1.0, sz + 3.0):
            scene.place(post, (x, PAD_Y, z), 0.0, group)
    return steps, hole_u, d0, L
...
def build():
    ...
    steps, hole_u, d0, L = stairwell(scene, STAIR_X, STAIR_Z, "stairwell")
    ...
    PASSPORTS["stair"] = [
        ...
        f"проём начинается в {d0:.3f} м от низа",
    ]
...
SIGNS_HEADER = [
    ...
    f"# - {STAMP}: Паспорт ЛЕСТНИЧНОЙ КЛЕТКИ (заказ C): длина проёма ВЫВЕДЕНА,",
