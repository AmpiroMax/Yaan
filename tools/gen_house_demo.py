...
# шарниру, верх к другому. С такой конструкцией можно сделать навес... получая
# не просто треугольник, а разные арки по форме».
#
# ВОТ ВСЯ МЕХАНИКА В ОДНОЙ ФРАЗЕ: скат — это ЗВЕНО, а не крыша. У звена две
# параллельные грани, каждая садится на свой горизонтальный шарнир, и уклон
# звена — это НЕ свойство постройки, а разность двух точек. Поставь шарниры по
# прямой — получишь двускатную крышу; поставь их по дуге — получишь арку; дай
# звеньям разные уклоны — получишь свод любой формы. Треугольник (фронтон,
# вальма) — это ВЫРОЖДЕННЫЙ случай, у которого верхней грани нет вовсе, и он
# умеет висеть только вершиной в точку.
#
# ПРОФИЛЬ ВЫВОДИТСЯ ИЗ ПОЛКИ, А НЕ РИСУЕТСЯ. Звено обязано быть НАСТОЯЩЕЙ
# деталью: её имя несёт run x depth x rise, и разность соседних шарниров обязана
# сойтись с этими числами ровно, иначе грань повиснет. Полка держит скаты
# run 8/12/16u при rise 4/6/8/12u, и rise 4u — только при depth 8u. Отсюда и
# профиль ниже: два уклона, 45° у пяты и 18.4° у замка. Так и кладут арку —
# круто у опоры, полого наверху, — но здесь это не стилизация, а единственные
# два звена, которые полка умеет сомкнуть без зазора.
...
def arch(scene, ax, z_near, y_spring, cover, group):
    ...
    Симметричная: восходящая половина идёт от ближней пяты к замку, нисходящая
    — зеркало от дальней. Каждое звено кладётся ВОСХОДЯЩИМ (низ на нижний
    шарнир, верх на верхний) — тем же движением, что скат дома, потому что это
    и есть один и тот же скат.

    Возвращает (пролёт, подъём) — числа, которыми навес описан в паспорте."""
    depth = ARCH_BAY * ARCH_BAYS
    purlin = worn("sleeper-timber-d50-nr-16u")
    purlin_r = 0.25
    post = require("joint-timber-d50-n4-h13-w03")

    # Точки шарниров восходящей половины, от пяты: (сдвиг по -Z, подъём).
    rising = [(0.0, 0.0)]
    for run, rise in ARCH_RUN_RISE:
        dz, dy = rising[-1]
        rising.append((dz + run, dy + rise))
    span = rising[-1][0] * 2.0   # ЗАМОК посередине: полный пролёт вдвое больше
    crown = rising[-1][1]

    # ШАРНИРЫ. Пять на четыре звена: у арки на один шарнир больше, чем звеньев,
    # и это не совпадение — звено требует ДВУХ, а соседние делят общий.
    for dz, dy in rising:
        for z in {z_near - dz, z_near - span + dz}:  # set: замок кладётся один раз
            note = None
            if dy == crown:
                note = ("замок свода: на нём сходятся верхние грани обоих "
                        "верхних звеньев")
            elif dy == 0.0:
                note = "пята: лежень на стойках, с него свод начинается"
            scene.place(purlin, (ax, y_spring + dy - purlin_r, z), yaw_for(1, 0),
                        group, note)

    # СТОЙКИ под пятами. Свод стоит на них, а не на земле; между стойками
    # пусто — это навес, под ним ходят.
    for z in (z_near, z_near - span):
        for k in range(ARCH_BAYS + 1):
            scene.place(post, (ax + k * ARCH_BAY, PAD_Y, z), 0.0, group)

    # ЗВЕНЬЯ. `sign` = +1 для восходящей половины (лезет к -Z), -1 для
    # зеркальной. У зеркальной глубина уходит в -X, поэтому её начало
    # смещено на глубину пролёта — та же поправка, что у северного ската дома.
    for sign, base_z, yaw, x_off in ((1, z_near, yaw_for(0, -1), 0.0),
                                     (-1, z_near - span, yaw_for(0, 1), ARCH_BAY)):
        for seg, (run, rise) in enumerate(ARCH_RUN_RISE):
            dz, dy = rising[seg]
            piece = worn(f"roof-{cover}-{int(run / GRID)}x{int(ARCH_BAY / GRID)}"
                         f"x{int(rise / GRID)}")
            for bay in range(ARCH_BAYS):
                x = ax + bay * ARCH_BAY + x_off
                scene.place(piece, (x, y_spring + dy, base_z - sign * dz), yaw,
                            group,
                            f"звено {seg + 1}: уклон {math.degrees(math.atan2(rise, run)):.1f}°"
...
# пустоты своим ИМЕНЕМ (deck-...-hole<x>x<z>x<l>x<w>-...) и крепится к лежням
# как сплошной. Иначе дырка неотличима от ЗАБЫТОЙ панели — ровно того дефекта,
# ради которого весь свод и написан.
#
# ДЛИНА ПРОЁМА ВЫВОДИТСЯ, А НЕ ВЫБИРАЕТСЯ (HOUSES.md §9.2). Это КАЛЬКУЛЯТОР;
# СУДЬЯ — правило stair-headroom, которое ставит капсулу игрока на носок каждой
# ступени и меряет. Они выведены разными дорогами НАРОЧНО. При расхождении прав
# судья, и чинить надо эту формулу.
PLAYER_H = 1.8          # PLAYER_CAPSULE_HEIGHT, docs/NUMBERS.md
...
def opening_length_m(t, thick, height=PLAYER_H, radius=PLAYER_R):
    ...
    Второй член — НЕ «плюс R»: игрок не линия, его макушка это полусфера
...
def stairwell(scene, sx, sz, group):
    ...
    Возвращает числа, которыми клетка описана в паспорте."""
    steps = 13                       # 13 x 1u = 3.25 м = этаж 13u
    t = 1.0                          # крутой марш 45°: проступь 1u при подъёме 1u
    run = steps * GRID               # 3.25 м — d1, горизонталь до верхнего уровня
    L = opening_length_m(t, DECK_T)
    # ВВЕРХ по сетке: проём обязан ПОКРЫТЬ выведенный отрезок, а не совпасть с
    # ним. Округление вниз укоротило бы его на разницу.
    hole_u = int(math.ceil(L / GRID - 1e-6))
    # d0 БЕРЁТСЯ ТОЛЬКО ИЗ ТОЖДЕСТВА d0 = d1 - L, и отдельной формулы для него
    # здесь нет НАРОЧНО (замечание лида 17.08): три записанных числа, из которых
    # одно выводится из двух других, — это три шанса разойтись вместо одного.
    d0 = run - L
    # А это ДРУГАЯ величина, и путать их нельзя: где начинается сама ДЕТАЛЬ
    # после округления вверх. Она всегда не позже d0 — на том округление и
    # стоит, — и разница есть ЗАПАС пустоты, а не ошибка.
    detail_start = run - hole_u * GRID
    joist = worn("sleeper-timber-d50-nr-16u")
    joist_r = 0.25
    post = require("joint-timber-d50-n4-h13-w03")
    deck = require(f"deck-timber-16x16x1-hole2x6x{hole_u}x4-w03")

    # МАРШ. Начало у подножия, лезет вдоль +X, ширина 1.0 м вдоль +Z.
    scene.place(require("stair-steep-timber-1x4x13-w03"), (sx, PAD_Y, sz), 0.0,
                group, f"марш 45°: {steps} ступеней по 1u, этаж за свою длину")
    # ЛЕЖНИ ПОД КОНЦАМИ МАРША — то самое «крепить к пол-потолок». Верхний идёт
    # осью по СЕРЕДИНЕ ТОЛЩИНЫ настила (§3.2, одна конвенция на все ориентации):
    # тогда носок верхней ступени лежит от оси на 0.125 м, внутри допуска 0.23.
    deck_y = PAD_Y + STOREY - DECK_T
    axis_y = deck_y + DECK_T * 0.5
    scene.place(joist, (sx, PAD_Y - joist_r, sz - 1.0), yaw_for(0, 1), group,
                "лежень нижнего уровня: на него садится подножие марша")
    scene.place(joist, (sx + run, axis_y - joist_r, sz - 1.0), yaw_for(0, 1), group,
                "лежень верхнего уровня: на него садится верх марша")
    # НАСТИЛ С ПРОЁМОМ. Панель полки объявляет пустоту начиная с 2u, поэтому её
    # угол отступает на 0.5 м назад от начала проёма; по Z пустота шириной 4u
    # ложится ровно на ширину марша.
    scene.place(deck, (sx + detail_start - 0.5, deck_y, sz - 1.5), 0.0, group,
                f"настил с ОБЪЯВЛЕННЫМ проёмом {hole_u}u = {hole_u * GRID:.2f} м")
    # Второй лежень под дальним краем настила, и стойки под концами лежней.
    far_x = sx + detail_start - 0.5 + 3.75
    scene.place(joist, (far_x, axis_y - joist_r, sz - 1.0), yaw_for(0, 1), group)
    # ВНУТРИ следа настила, а не в 0.25 м перед ним: настил начинается в
    # detail_start - 0.5, и лежень, положенный раньше этой отметки, его НЕ
    # НЕСЁТ. Правило deck-on-joints нашло это в тот же час, когда его навели на
    # настоящее семейство деталей: оба несущих лежня оказались по одну сторону
    # от середины настила (-1.75..-0.75 м), и дальняя половина висела.
    scene.place(joist, (sx + detail_start - 0.25, axis_y - joist_r, sz - 1.0),
                yaw_for(0, 1), group,
                "лежень под ближним краем настила: без него настил висит")
    for x in (sx + detail_start - 0.25, sx + run, far_x):
        for z in (sz - 1.0, sz + 3.0):
...
def house(scene, ox, oz, span, storeys, style, upper, cover, gable_mat, group):
    ...
    if storeys > 1 or style.startswith("frame"):
        ...
        # ЛЕЖНИ ПОД КОНЦАМИ МАРША. Их тут не было, и правило stair-seat, едва
        # написанное, нашло это на обоих домах с лестницей: 4 находки по 2.2,
        # 5.3 и 8.6 м — марш висел обоими концами, потому что ближайшим лежнем
        # ему был лежень чужого этажа через полдома. Верхний идёт осью по
        # СЕРЕДИНЕ ТОЛЩИНЫ настила (§3.2), тогда носок верхней ступени лежит от
        # оси на 0.125 м — внутри допуска 0.23.
        joist = worn("sleeper-timber-d50-nr-16u")
        scene.place(joist, (ox + 0.5, PAD_Y - 0.25, oz), yaw_for(0, 1), group,
                    "лежень нижнего уровня: подножие марша садится на него")
        scene.place(joist, (ox + 0.5 + 13 * GRID, PAD_Y + STOREY - 0.375, oz),
                    yaw_for(0, 1), group,
...
def build():
    ...
    steps, hole_u, d0, L, detail_start = stairwell(scene, STAIR_X, STAIR_Z,
                                                   "stairwell")
    scene.place("PASSPORT:stair", (STAIR_X + 2.0, PAD_Y, STAIR_Z + 3.5), 0.0,
                "stairwell", "паспорт клетки: почему проём именно такой длины")
    PASSPORTS["stair"] = [
        "ЛЕСТНИЧНАЯ КЛЕТКА С ПРОЁМОМ",
        f"марш 45°, {steps} ступеней, этаж 3.25 м",
        "низ на лежень нижнего уровня,",
        "верх на лежень верхнего",
        f"проём {hole_u * GRID:.2f} м — ВЫВЕДЕН: {L:.3f}",
        "вверх по сетке 0.25",
        "макушка игрока полусфера,",
        "она цепляет УГОЛ плиты первой",
        f"нужен с {d0:.3f} м, деталь с {detail_start:.3f}",
        f"разница {d0 - detail_start:.3f} м — это ЗАПАС",
    ]
    span, crown, depth = arch(scene, ARCH_X, ARCH_Z_NEAR, ARCH_Y, "shingle", "arch")
    # Паспорт навеса стоит там же, где паспорта домов: со стороны прихода (+Z).
    scene.place("PASSPORT:arch", (ARCH_X + depth * 0.5, PAD_Y, ARCH_Z_NEAR + 2.5),
                0.0, "arch", "паспорт навеса: из чего он и почему такой формы")
    PASSPORTS["arch"] = [
        "НАВЕС-АРКА, СВОД ИЗ ЗВЕНЬЕВ",
        f"пролёт {span:.0f} м, подъём {crown:.0f} м, глубина {depth:.0f} м",
        "звено низом на один лежень,",
        "верхом на другой — уклон это",
        "разность двух шарниров",
        "у пяты 45°, у замка 18°",
        "шарниры по дуге = арка,",
        "по прямой = двускатная крыша",
    ]
    return scene
