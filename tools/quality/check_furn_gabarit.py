...
Last updated: 28:08:2026 - 15:50:00
...
Usage:
  ...
  НЕ РАБОТАЕТ КАК ПЛЕЧО ШИРИНА ФАСКИ: попросить у печи 60 мм вместо 10 нельзя
  — ширина приходит ОТ КУСКА (четверть толщины, четверть кратчайшего ребра), и
  на убранстве она уже упёрта в этот предел. Замер: полка при --bevel 0.06
  байт в байт та же, что при штатной. Это и есть доказательство, что
  «переусердствовать» фаска не может, но плечом оно быть не может тоже.
