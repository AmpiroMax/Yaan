...
def _kit(pattern):
    ...
    волнами, и любое записанное сюда число — будущая ложь."""
    out = {}
    for f in os.listdir(os.path.join(ROOT, "assets/houses")):
        m = re.fullmatch(pattern.replace("%02d", r"(\d\d)"), f)
        if m:
            xs, ys = [], []
            for line in open(os.path.join(ROOT, "assets/houses", f),
                             encoding="utf-8"):
                t = line.split()
                if t and t[0] == "vertex":
                    xs.append(float(t[3]))
                    ys.append(float(t[4]))
            out[max(ys)] = (f, max(xs) - min(xs))
    return out

def market_terrace(Hh, plate_top):
    """Строит перепад северной кромки рынка. Возвращает (маршей, отрезков)."""
    steps, kerbs = _kit(STEP_REC), _kit(KERB_REC)
    ...
    usable = {}
    for h, (rec, run) in steps.items():
        if rec not in STEP_FIT:
            print(f"терраса рынка: {rec} без замера поверхности — не сажаю")
            continue
        t_foot, t_top = STEP_FIT[rec]
        usable[h + t_top - t_foot] = (rec, run, t_foot, h)
    if not usable:
        return 0, 0
    walk_rise = min(usable, key=lambda v: abs(v - rise))
    rec_s, run, t_foot, h_anchor = usable[walk_rise]
