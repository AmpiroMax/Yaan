...
def cand_top(cx, cz, yaw, k, h0, sl, px, pz, clamp=False):
    """То же для ЕЩЁ НЕ УЛОЖЕННОГО куска: h0 — отметка его начальной кромки,
    k — уклон по ходу улицы (знак живой, разворот на 180 делается позже)."""
    c = math.cos(math.radians(yaw))
    sn = math.sin(math.radians(yaw))
    lx = (px - cx)*c - (pz - cz)*sn
    if clamp:
        lx = max(-sl/2, min(sl/2, lx))
    return h0 + k*sl/2 + PAVE_TOP + k*lx
...
def pave(pts, note):
    ...
    while total - s0 > 3.0:
        ...
        for sl, rec in COBBLE:
            ...
            # НАЛОЖЕНИЕ МЕРЯЕТСЯ НЕ ТОЛЬКО ПЛОЩАДЬЮ. Порог в 1 м2 пропускал
            # пары, лежащие РОВНО в одной плоскости (замер wr-forge: две пары
            # по 0.6 м2 с разностью верха 0.0000) — а мерцает именно
            # соплоскостная пара, и полметра мерцания под ногами хуже, чем
            # пятнадцать метров с честным сантиметром разницы. Уголок в
            # сантиметр-другой по высоте безвреден: верхняя плита просто
            # накрывает нижнюю.
            clash = False
            for p in PAVED:
                a = poly_clip_area(list(poly), rect_poly(*p[:5]))
                if a <= PAVE_SLIVER:
                    continue
                mpx, mpy = (cx + p[0])/2, (cz + p[1])/2
                # ТА ЖЕ МЕРА, ЧТО У ПРОВЕРКИ. Без прижатия к пятну укладчик
                # и check_paving расходились в оценке ОДНОЙ пары: у соседок на
                # изломе разность, продлённая до середины между центрами,
                # выходила ненулевой, и соплоскостный клин проскакивал мимо
                # укладчика, чтобы потом всплыть в отчёте.
                dy = abs(cand_top(cx, cz, yaw, k, h0, sl, mpx, mpy, clamp=True)
                         - slab_top(p, mpx, mpy, clamp=True))
                if a > PAVE_OVERLAP or dy < PAVE_FLICKER:
                    clash = True
                    break
...
def main():
    ...
    # ТЕРРАСА РЫНКА строится ПОСЛЕ мощения: марш встаёт в разрыв, который
    # мостовая оставила на нагорной кромке, и обязан знать, где она кончилась.
