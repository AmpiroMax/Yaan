...
# Last updated: 23:08:2026 - 05:00:00
# Module: tools
...
def light(x, y, z, color, radius, note, shadow=False, interior=False):
    LIGHTS.append((x, y, z, color, radius, shadow, note, interior))
...
def furnish(rec, ox, oy, oz, yaw_deg, note):
    ...
    for ff, lx, lz, lyaw in items:
        ...
        if ff == "furn-hearth.dfh":
            ...
            _c, _cz, rw, rd, _a, _o = recipe_box(rec)
            ...
            light(hx, fy + 0.55, hz, HEARTH_COLOR, rad,
                  "очаг: " + note, rec in HEARTH_SHADOW, interior=True)
...
def market_terrace(Hh, plate_top):
    ...
    def street(x):
        """Ходибельная отметка улицы против точки x кромки: земля плана плюс
        то, на чём лежит мостовая (вкоп 0.03 и верх камня)."""
        return hh_at(Hh, x, edge_z - 2.0) + 0.03 + PAVE_TOP
    ...
    oy = plate_top - t_foot + (rise - walk_rise) / 2.0
    ...
    grade_corridor(Hh,
                   [(ox + 3.0, oz, oy - 0.02),
                    (ox + 3.0, oz - run, oy + h_anchor - 0.02)],
                   3.4, 0.8)
    ...
    # уйдёт в relief.
    cs = [hh_at(Hh, px, pz) - (oy + h_anchor * lx / run)
          for px, pz, lx in ((ox, oz, 0.0), (ox + 6.0, oz, 0.0),
                             (ox, oz - run, run), (ox + 6.0, oz - run, run))]
    check_faces(Hh, kerb_faces)
    print(f"терраса рынка: марш {rec_s} (перепад {rise:.2f} м, ходибельный "
          f"подъём {walk_rise:.3f}, остаток по {abs(rise-walk_rise)/2:.3f} на "
          f"конец), кромка {n_k} отрезков")
    if cs:
        nm = ("ЮЗ подошвы", "ЮВ подошвы", "СЗ верха", "СВ верха")
        print(f"  ниша под маршем (поле этого прохода): отход углов от "
              f"подошвы {min(cs):+.3f}..{max(cs):+.3f} м  ["
              + ", ".join(f"{a}={b:+.3f}" for a, b in zip(nm, cs)) + "]")
    return 1, n_k
...
def main():
    ...
    for x, y, z, col, rad, shadow, note, inter in LIGHTS:
