...
# Last updated: 23:08:2026 - 00:55:00
...
# - 23:08:2026 - 00:55:00: top_at ПРИЖИМАЕТ точку к своему пятну. Поверхность
#   существует только НАД пятном, а прежде плоскость продлевалась в пустоту и
#   деталь врала на весь свой уклон за длину выноса: у марша (57 %) вынос 0.30 м
#   давал 0.17 м — против порога ступени 0.20, то есть ошибка ходила у самого
#   порога незамеченной. Нашёл wr-paver: у него тем же приёмом марш 2.1 м рядом
#   с восьмиметровой плитой дал «ступень 0.85 м» там, где кромки сходятся с
#   точностью 3 см.
#
...
def top_at(s, pt) -> float:
    ...
    ТОЧКА ПРИЖИМАЕТСЯ К СВОЕМУ ПЯТНУ. Поверхность существует только НАД
    пятном; за краем честное значение — отметка ближней кромки, а не плоскость,
    продлённая в пустоту. Без прижатия деталь врёт на весь свой уклон, взятый
    за длину выноса: у марша (57 %) вынос 0.30 м — уже 0.17 м, против порога
    ступени 0.20. Короткая крутая деталь рядом с длинной пологой давала так
    «обрыв 0.85 м» там, где кромки сходятся с точностью 3 см. Нашёл wr-paver.
    """
    stem, x, y, z, a, (rx0, rz0, rx1, rz1) = s
    r = math.radians(a)
    c, si = math.cos(r), math.sin(r)
    dx, dz = pt[0] - x, pt[1] - z
    # Мировая точка в собственные оси детали (обращение AppHouse.cpp:790).
    lx = dx * c - dz * si
    lz = dx * si + dz * c
    lx = min(max(lx, rx0), rx1)
    lz = min(max(lz, rz0), rz1)  # считается, но в высоту не входит: уклон вдоль +X
